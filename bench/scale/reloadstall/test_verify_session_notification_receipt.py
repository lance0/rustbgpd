...
class VerifyTests(unittest.TestCase):
    ...
    def test_full_receipt_is_fail_closed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(ValueError): V.verify(root)
            (root / "checkpoints.csv").write_text(lines()); (root / "harness.log").write_text(lines().replace("high_watermark=1", "high_watermark=2", 1)); reseal()
            with self.assertRaises(ValueError): V.verify(root)
            (root / "harness.log").write_text(lines()); (run / "status").write_text("PASS\n"); reseal()
            with self.assertRaises(ValueError): V.verify(root)
