...
Usage:
    gen-irr-scenario.py <cell> <n_members> <total_prefixes> <out_dir>
        ...
        [--competitor-generation historical|current]
...
def emit_bird(args, members: list[Member], out: pathlib.Path) -> list[str]:
    ...
    competitor_name, competitor_image = COMPETITOR_GENERATIONS[
        args.competitor_generation
    ]["bird"]
    for gen in ("a", "b"):
    ...
    config = [
        f"# IRR reload-matrix {competitor_name} route server - generated by gen-irr-scenario.py.",
        f"# Run: docker run -d --network=host -v <out_dir>:/etc/bird {competitor_image} \\",
...
def emit_obgpd(args, members: list[Member], out: pathlib.Path) -> list[str]:
    ...
    config = [
        f"# IRR reload-matrix {competitor_name} route server - generated by gen-irr-scenario.py.",
        f"# Run: docker run -d --network=host -v <out_dir>:/etc/bgpd {competitor_image}",
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=tuple(COMPETITOR_GENERATIONS),
        default="historical",
        help="closed BIRD/OpenBGPD receipt pair (default: historical)",
    )
    parser.add_argument(
        "--bmp-collector",
    ...
    if (
        args.competitor_generation != "historical"
        and args.cell not in {"bird", "openbgpd"}
    ):
        sys.exit("--competitor-generation=current applies only to competitor cells")
