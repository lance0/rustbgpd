...
Soak extension (route-reflector flagship soak): setting `GEN_IBGP_RR_ASN`
(1..=65535 — the harness stubs OPEN with it as a u16 `my_as`) emits an iBGP
route-reflector scenario instead: `asn = <value>` with `cluster_id` set to the
router-id, every neighbor `remote_asn = <value>` + `route_reflector_client =
true` (never `route_server_client`), and NO `[policy]` section — iBGP is
outside RFC 8212's default-deny, reflection needs no policy, and the RR soak
runs zero reloads (the `.rpol` files are still written to satisfy the
harness's positional arg contract; the daemon never loads them). Mutually
exclusive with the trip knobs. Absent (the default), the emitted config is
byte-for-byte the historical one.

Dual-stack extension: setting `GEN_DUALSTACK=1` gives every neighbor
...
rejects the named prefix-set `filtered` (base indexes `0..K` of every emitted
family, i.e. the head of member 0's slice) before tagging, while generation A
stays permit-everything. Matches the harness's `RELOADSTALL_FILTER_COUNT=K`
withdraw/leak/bystander accounting. Absent, both generations are
permit-set-preserving as before.
"""
import os
import pathlib
import sys
...
dualstack = os.environ.get("GEN_DUALSTACK") == "1"
filter_count = int(os.environ.get("GEN_FILTER_COUNT", "0"))
if filter_count < 0:
    sys.exit("GEN_FILTER_COUNT must be non-negative")
ibgp_rr_asn = os.environ.get("GEN_IBGP_RR_ASN")
if ibgp_rr_asn is not None:
    ibgp_rr_asn = int(ibgp_rr_asn)
    if not 0 < ibgp_rr_asn < 65536:
...
# Base-table prefix for global route index `idx`; MUST match the harness's
# `base_prefix` / `base_prefix6`.
def base_prefix(idx: int) -> str:
    return f"{20 + (idx >> 16)}.{(idx >> 8) & 0xFF}.{idx & 0xFF}.0/24"


def base_prefix6(idx: int) -> str:
    return f"3001:{idx >> 16:x}:{idx & 0xFFFF:x}::/48"


# Named filter set for the filtering generation: the first `filter_count`
# base indexes of every emitted family (member 0's slice head).
filtered_prefixes = [base_prefix(i) for i in range(filter_count)]
if dualstack:
    filtered_prefixes += [base_prefix6(i) for i in range(filter_count)]
...
""" if mixed_export_only else ""
    # Filtering generation (B only): reject the named set before tagging, so
    # observers must see those prefixes withdrawn and everything else re-tagged.
    filtering = fname == "gen-b.rpol" and filter_count > 0
    filter_set = (
        f"prefix-set filtered {{ {', '.join(filtered_prefixes)} }}\n\n" if filtering else ""
    )
    filter_term = (
        "    term drop-filtered { if route.prefix in filtered { reject } }\n" if filtering else ""
    )
...
{filter_set}policy member-in {{
    term drop-blocked {{ if route.prefix == {reject_prefix} {{ reject }} }}
    term default {{ accept }}
}}

policy member-out {{
{filter_term}    term tag {{ add community {community}; accept }}
}}
{stable_policy}
"""
    )
...
config = [
    ...
    if ibgp_rr_asn is None
    else "# reload-stall iBGP route-reflector scenario — generated by gen-scenario.py.",
    "# Stock daemon config: worker_threads unset => min(CPU, 8), matching the",
    "# receipt. Start load-gated (1-min loadavg < 2.0), --release.",
    "",
    # RR mode: explicit epoch-2 posture for a clean --check. RFC 8212
    # governs eBGP only, and this scenario is all-iBGP, so enforcement
    # is declared but exercises nothing. (The historical route-server
    # config stays byte-for-byte untouched.)
    *([] if ibgp_rr_asn is None else ["config_epoch = 2", ""]),
    "[global]",
    ...
    *([] if ibgp_rr_asn is None else ['cluster_id = "10.0.0.1"', "ebgp_requires_policy = true"]),
    f"listen_port = {port}",
    ...
    "",
    # iBGP-RR mode: no [policy] section at all. iBGP is outside RFC
    # 8212's default-deny, reflection needs no export chain, and the RR
    # soak runs zero policy reloads.
    *(
        [
            "[policy]",
...
for i in range(n_peers):
    neighbor = [
        ...
        f"remote_asn = {stub_asn(i) if ibgp_rr_asn is None else ibgp_rr_asn}",
        "route_server_client = true"
        if ibgp_rr_asn is None
        else "route_reflector_client = true",
...
rr_note = (
    f", ibgp_rr(asn={ibgp_rr_asn}, cluster_id=10.0.0.1, no policy section)"
    if ibgp_rr_asn is not None
    else ""
)
dualstack_note = ", dualstack(ipv4_unicast+ipv6_unicast)" if dualstack else ""
filter_note = (
    f", filter(gen-b rejects {len(filtered_prefixes)} named prefixes: base indexes 0..{filter_count} per family)"
    if filter_count > 0
    else ""
)
print(
    ...
    f"stable_peers={n_peers - changed_peers}, grpc_uds={rundir}/grpc.sock{trip_note}{rr_note}{dualstack_note}{filter_note}"
)
