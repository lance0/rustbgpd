...
| P0 | **gRPC security audit + authorization split** | v1.0 is blocked on an external security review. mTLS, bearer auth, UDS defaults, and listener-level `access_mode` exist; ADR-0064 now adds a checked 66-RPC risk-tier matrix plus audit-only runtime decision logs/metrics. Operator trust still needs real principal extraction, roles, listener tier caps, and deny-by-tier enforcement. | Security review complete; mutating/operator-only RPCs can be disabled or isolated from sensitive-read observability endpoints, and the method-tier matrix is enforced by the gRPC runtime. |
...
- [ ] **Finer-grained gRPC authorization** — ADR-0064 now has a checked
  method-tier matrix and audit-only runtime decision telemetry. Remaining
  work: mTLS principal extraction, explicit bearer/UDS principals,
  `[security.grpc.roles]`, listener `max_tier`, and enforcement.
...
- [ ] **Security audit of gRPC surface** *(v1.0 gate)* — independent
  ...
  model. The audit packet still needs the ADR-0064 threat model and
  external-review evidence bundle.
