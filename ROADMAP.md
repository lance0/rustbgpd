...
- [ ] **EVPN Phase 2 — VTEP mode** — local EVI/VRF/VNI state, MAC learning from kernel FDB, local route origination. Required for general-purpose routing; not required for RR-only deployments. Kernel integration design landed in ADR-0054 (Gate 7b foundation, PR #34); the remote-MAC FDB programmer is in flight on that branch. Local-MAC learning + Type 2/3 origination are the next slices on top of the Gate 7b foundation (tracked separately in `docs/evpn-enablement.md` Gate 7b "Still ahead").
