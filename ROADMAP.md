...
| P1 | **EVPN production-default decision point** | The EVPN dataplane is broad now; the missing evidence is sustained MAC churn with DF flips before turning enforcement defaults on. The MAC-churn harness exists and uses process-restart flips so containerlab links survive. | Gate 8b MAC-churn 24h soak with `apply_bum_enforcement=true` and `apply_aliasing_ecmp=true`; documented default decision. |
| P1 | **TCP-AO (RFC 5925)** | BIRD 3 has TCP-AO on Linux, and it is the modern replacement for TCP MD5. This is a visible route-server/security parity gap. ADR-0062 and the internal Linux socket primitive are now in place. | Remaining exit: static-neighbor config schema, inbound-listener socket refactor, outbound/listener apply, interop smoke, and fallback behavior documented. |
| P1 | **Real-time event/history surface** | Operators debugging policy and convergence need a timeline, not only snapshots. This is where an API-first daemon can beat CLI-first stacks. Bounded unicast route-event history has landed via `RibService.ListRouteEvents` and `rustbgpctl events`, including exact-prefix drilldown; bounded session lifecycle history has landed via `EventService.ListSessionEvents` and `rustbgpctl events sessions`; `EventService.WatchEvents` now exposes route events, structured session lifecycle events, BGP NOTIFICATION sent/received metadata, opt-in policy mutation summary events, and aggregate FIB / BLACKHOLE dataplane status changes through a unified typed stream, with session lifecycle delivery isolated from lossless TCP collision coordination. Route events carry process-local cursors, and `rustbgpctl events watch --backfill N` composes recent route history with a live tail. API/operations docs now classify live streams, bounded history, snapshots, and metrics as separate observability surfaces. | Remaining exit: richer per-route / per-MAC dataplane and EVPN event categories; retention decisions if operators need durable history beyond the in-memory ring. |
...
- [ ] **TCP-AO authentication** (RFC 5925) — modern replacement for TCP MD5; BIRD 3 supports it on Linux. rustbgpd has ADR-0062 plus an internal Linux socket primitive/capability probe; runtime config, listener/outbound apply, and interop smoke remain.
- [ ] **Real-time BGP observability** — first slices shipped: unicast best-path events now land in a bounded 4096-event RIB ring with `RibService.ListRouteEvents` and `rustbgpctl events`, including exact-prefix drilldown; session lifecycle events now land in a bounded 4096-event peer-manager ring with `EventService.ListSessionEvents` and `rustbgpctl events sessions`; `EventService.WatchEvents` now exposes route events, structured session lifecycle events, BGP NOTIFICATION sent/received metadata, opt-in policy mutation summary events, and aggregate FIB / BLACKHOLE dataplane status changes through a unified typed live stream with peer/family/prefix/type filters where those filters apply. Route events carry process-local cursors and `rustbgpctl events watch --backfill N` can print recent route history before tailing the live stream. Session lifecycle delivery is bounded and separate from lossless TCP collision coordination, with `stream_lagged` warning events when a live subscriber misses bounded route/session source events. API/operations docs now distinguish live streams, bounded history, snapshots, and metrics so operators know which surface to script. Remaining work: policy_filtered, richer per-route/per-MAC dataplane and EVPN event categories; durable retention decisions; WatchRoutes missed-event signaling if/when it gains an envelope; optional subscription-side indexing or a dedicated event bus if subscriber count or event rate makes post-broadcast filtering expensive; foundation for TUI live event view
...
and the Docker `fdb_nhg` / `fib_runtime` netns selectors now run in the
protected self-hosted `kernel-dataplane` workflow; the remaining
VTEP / DF-election smokes stay manual until they are assigned to that
runner or explicitly kept as reviewer-run checks.
