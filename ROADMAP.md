...
- [ ] **EVPN VTEP alpha-soak slate.** With Gates 7a / 7b / 7b+1 /
  ...
  confidence is completing the last ADR-0063 runtime convergence shapes:
  `ip_vrf` relink, L3VNI/device/table IP-VRF redefine, and non-teardown mixed
  edits (an add combined with a delete or redefine in the same request).
  The daemon actor converger already commits single L2VNI add/delete/redefine,
  single IP-VRF add/standalone delete/redefine with unchanged
  L3VNI/device/table identity, single Ethernet Segment add/delete/redefine
  (including ES add/redefine over a member VNI added by an earlier live L2VNI add
  when the segment actor exists), and atomic tenant teardown — a delete-only plan
  that drops an ES-member L2VNI with its Ethernet Segment and/or a linked IP-VRF
  in one pass, validated for the M47 interop smoke.
...
- [ ] **EVPN DF election + single-active follow-ups** — non-blocking polish on the RFC 8584/9785 + single-active work that shipped on `main`: (1) **cross-vendor preference-DF interop smoke** against FRR — FRR exposes `es-df-pref`, but its on-the-wire DF Election Extended Community / DP-bit behavior is unverified, so the smoke must packet-capture the Type 4 ES route to confirm `DF Alg = 2/3` + the preference + DP bit rather than masking a silent fall-back to default modulo (RFC 9785 Highest-/Lowest-Preference election is currently unit-tested only; cross-vendor HRW is not possible since FRR implements preference-DF, not HRW); (2) **dedicated dataplane test for the single-active `(next-hop, ESI)` duplicate fold** — the receiver ORs the ESI Label Single-Active flag so single-active wins on conflicting/transient duplicate EAD-per-ES rows; currently covered indirectly via the decode + suppression unit tests; (3) RFC 9785 **local Don't-Preempt / non-revertive origination** — rustbgpd reads peers' DP bit for tie-breaking but always advertises DP=0 (revertive) today; (4) proactive **single-active backup-path pre-install** on the receive path.
