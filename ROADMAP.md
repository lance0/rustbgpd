...
- [ ] **EVPN Gate 9 follow-ups — overlay-index IRB** — Gate 9 slice 6 ships the Interface-less symmetric IRB model, auto-derived Route Targets (RFC 8365 §5.1.2.1) are available as an explicit config opt-in for `[[evpn_instances]]` and `[[evpn_ip_vrfs]]`, and non-zero Type 5 Gateway Address routes are now detected and fail-closed as unsupported overlay-index IRB. Remaining standards-completeness item: recursive overlay-index resolution (RFC 9135 §9.2), needed for broader tenant-routing topologies after the first-class runtime/API work is settled.
