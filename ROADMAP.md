...
| ✓ | **BFD (RFC 5880/5881/5882) — single-hop async — shipped** | Sub-second peer-failure detection wired to BGP so a BFD-down event tears the neighbor down before the hold timer. Design is ADR-0067; shipped end-to-end across five slices: (1) `crates/bfd` pure sans-IO crate, (2) config (`[[bfd_profiles]]` + `[neighbors.bfd]`) + in-process no-GC actor + status/metrics, (3) operator surface (gRPC `GetBfdSessions` + `rustbgpctl bfd` + events), (4) RFC 5882 coupling — **non-strict** (BFD-down tears BGP down) and **strict** (withhold BGP establishment until BFD Up, via the `add_peer` create/start split), (5) M51 FRR `bfdd` interop. v1 is IPv4 + IPv6 **global**, static neighbors only; `COMPARISON.md` now marks BFD `Yes`. This closes the BFD differentiator gap vs GoBGP (the other gRPC-first daemon, which still lacks it). | Done — M51 (`tests/interop/m51-bfd-frr.clab.yml`) shows BGP + BFD Up from both sides (`GetBfdSessions` + FRR `show bfd peers`), a killed `bfdd` drops BGP faster than the hold timer, and recovery reconnects; wired into `kernel-dataplane` CI. **Deferred follow-ups:** multihop (RFC 5883), echo/demand mode, authentication, C-bit/GR-aware nuance, static-route BFD tracking, dynamic-neighbor BFD, hardware/offload, and **IPv6 link-local / unnumbered → v1.1** (gated on link-local BGP peering, which the daemon lacks today — confirmed by the BFD spike). |
