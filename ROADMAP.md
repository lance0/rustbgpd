...
| P2 | **EVPN runtime instance mutation implementation** | `[[evpn_instances]]`, `[[evpn_ip_vrfs]]`, and `[[ethernet_segments]]` are still startup-pinned. ADR-0063 now defines the safe mutation contract: a single command-driven EVPN coordinator, validation-first model updates, and explicit drain/replay across IMET, Type 2, Type 5, DF/ES, and Linux owned state. The foundation now has a generationed runtime model, read-only `GetEvpnRuntime` / `rustbgpctl evpn runtime` status, and the coordinator core / commit gate that accepts fully validated candidates and publishes only after a convergence backend accepts the plan. | Implement the public read/write API slice and daemon actor convergence without direct shared-table swaps; keep SIGHUP restart-required until the coordinator can safely converge edits ([#210](https://github.com/lance0/rustbgpd/issues/210)). |
