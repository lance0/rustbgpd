...
| P0 | **Privileged runner expansion for remaining dataplane gates** | The self-hosted `kernel-dataplane` workflow now covers M36, M37, M37+IP, M38, M39, M40, M42, M43, and the Docker `fib_runtime` / `fdb_nhg` selectors behind protected-environment approval. The remaining high-value local-only dataplane gates are long soak variants and platform-diversity checks that intentionally do not run on every PR. | Protected CI covers the repeatable EVPN dataplane / DF-election / TCP-AO smokes; any future long-soak gate either gets a protected-runner job or a documented reason for staying manual. |
...
M36 / M37 / M37+IP / M38 / M39 / M40 / M42 / M43 and the Docker `fdb_nhg` /
