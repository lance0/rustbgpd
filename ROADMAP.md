...
- [ ] **EVPN Phase 3 — Multi-homing enforcement (Gate 8b residual) + Gate 9 overlay-index IRB** — Gate 8b residuals: aliasing dataplane ECMP per [ADR-0059](docs/adr/0059-evpn-aliasing-fdb-nexthop-groups.md) (FDB nexthop groups via `NDA_NH_ID` + `NHA_FDB`; raw-netlink construction because `rtnetlink 0.21` has no nexthop API). Slice 1 (portable intent — `RemoteMacEntry::alias_group_key`), slice 2 (`nexthop_raw` netlink primitive), slice 3a (state types + apply primitive + CVE-2025-39851 guard, infrastructure-only), and slice 3b (PR #88, the operational-behavior-change slice — diff Pass 1b + reconcile actor coordinator + `NexthopOps` impls + startup NHID adoption + Docker-runnable netns test + actor-level FDB-NHG coverage) have all shipped on `main`. **Slice 4 (M40 manual containerlab smoke against FRR) is in flight, pending merge** — `tests/interop/m40-evpn-aliasing-ecmp-frr.clab.yml` validates that rustbgpd consumes real FRR EVPN-MH routes and programs an FDB-NHG on the receiving VTEP; gating evidence for flipping aliasing ECMP from "shipped, manual-smoke green" to "production-default". Slice 3.5 deferred follow-ups: periodic `RTM_GETNEXTHOP` drift recovery (research §8 confirms forced NHG deletion doesn't emit RTNLGRP_NEIGH events), `apply_aliasing_ecmp` operator toggle, IPv6 alias members. Gate 9 follow-ups: overlay-index IRB (RFC 9135 — Gate 9 slice 6 ships the Interface-less variant only), auto-derived Route Targets (RFC 8365 §5.1.2.1), privileged-runner CI gate for the M39 + M40 smokes. Needed for active-active ToR deployments and broader tenant-routing topologies.
...
L3VXLAN FDB programming validated end-to-end. Manual only; same
hosted-runner gap as M30b (Azure kernel lacks `vrf` module).
