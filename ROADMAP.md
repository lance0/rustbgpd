...
- [ ] **Real-time BGP observability** — first slices shipped: unicast best-path events now land in a bounded 4096-event RIB ring with `RibService.ListRouteEvents` and `rustbgpctl events`, including exact-prefix drilldown; `EventService.WatchEvents` now exposes route events and structured session lifecycle events through a unified typed live stream with peer/family/prefix/type filters. Ordinary non-collision lifecycle delivery is bounded and separate from lossless TCP collision coordination. Remaining work: policy_filtered, notification sent/received, dataplane, EVPN, and other operator event categories; retention/backfill decisions; explicit missed-event/slow-consumer signaling for lagged watchers; optional subscription-side indexing or a dedicated event bus if subscriber count or event rate makes post-broadcast filtering expensive; foundation for TUI live event view
