...
|----------|------|---------|-----------------------------|
| P0 | **Read-only gNMI / OpenConfig telemetry adapter** | Cloud and whitebox operators already aggregate device state through gNMI/OpenConfig tooling. rustbgpd has typed gRPC snapshots and events; a narrow read-only adapter can plug into that ecosystem without claiming full router-model coverage or adding config mutation risk. | **ADR-0070 accepted.** `Capabilities`, `Get`, and `Subscribe` (ONCE/POLL/STREAM SAMPLE) for a deliberately small OpenConfig BGP subset (global + neighbor state); `Set` returns a stable `Unimplemented`; BFD / FIB / EVPN telemetry and per-AFI-SAFI counters deferred; verified with `gnmic`; no `Set` until a daemon-wide transaction model exists. |
