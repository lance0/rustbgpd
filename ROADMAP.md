...
- [x] gRPC API — 10 services: Global, Config, Neighbor, Policy, PeerGroup, RIB, Event, Injection, Control, Evpn (all IPv6-capable)
...
|----------|------|---------|-----------------------------|
| ✓ | **Cut v0.23.0 — shipped** | Includes ADR-0064 gRPC authorization (matrix + audit-only runtime + threat model + roles config + listener tier caps), ADR-0062 static-neighbor TCP-AO runtime + BIRD 3.2.1 interop smoke, the EVPN production-default flip for `apply_bum_enforcement`, filtered FIB-status queries, peer-group MD5 redaction with tri-state preserve/clear, and the FDB nexthop raw-netlink parser hardening. Gating soaks: Gate 8b MAC-churn 24h (2026-05-16) and M37 local-origination 24h (2026-05-19). | Done — workspace bumped to 0.23.0, CHANGELOG rolled, tag pushed, CI + interop green, GitHub release notes match CHANGELOG. |
| P0 | **Privileged runner expansion for remaining dataplane gates** | The self-hosted `kernel-dataplane` workflow now covers M36, M37, M37+IP, M38, M39, M40, M42, M43, and the Docker `fib_runtime` / `fdb_nhg` selectors behind protected-environment approval. The current protected runner advertises `CONFIG_TCP_AO=y`, so M43 runs the real BIRD 3.2.1 TCP-AO topology; the workflow keeps its kernel-feature probe so future runner drift skips loudly instead of producing a false negative. The remaining high-value local-only dataplane gates are long soak variants and platform-diversity checks that intentionally do not run on every PR. | Protected CI covers the repeatable EVPN dataplane / DF-election smokes and TCP-AO; any future long-soak gate either gets a protected-runner job or a documented reason for staying manual. |
| P0 | **ADR-0061 FIB hardening** | GoBGP and FRR both expose kernel route programming. rustbgpd now has the minimal configured-table path, peer / peer-group guardrails, route-count caps, exact-match crash-restart recovery via persisted owned-state, explicit live-drift preservation, and **unicast multipath/ECMP install** (ADR-0066: per-table `maximum_paths`, kernel `RTA_MULTIPATH`, owned-state v2, M50 FRR interop). | Decide whether operators need a paginated/detail API for over-cap rejected routes beyond the current sampled `route_limit_exceeded` status rows; remaining future work is hot-swap `[[fib_tables]]` and the ECMP follow-ups operators may ask for: `AS_PATH`-length-only `multipath-relax`, weighted/unequal-cost multipath (the kernel encode is equal-weight `hops=0` today), per-class `maximum_paths_ebgp` / `maximum_paths_ibgp` (FRR parity), and an incremental equal-cost sibling index so wide full-table multipath avoids the `O(best_routes × peers)` per-reconcile rescan. |
| P1 | **BFD (RFC 5880/5881/5882) — single-hop async** | BFD gives sub-second peer-failure detection and RFC 5882 wires it to BGP so a down event tears the neighbor down before the hold timer. `COMPARISON.md` marks it `No` for rustbgpd, FRR/BIRD `Yes`, **GoBGP/OpenBGPd `No`** — so it is a differentiator vs the other gRPC-first daemon and pairs with the just-shipped ECMP work (fast detect → fast reconverge). In-process, no-GC (a Rust advantage vs Go's GC jitter → false flaps). Design is ADR-0067; staged: (1) `crates/bfd` pure sans-IO crate **[shipped]**, (2) config + actor + status/metrics — sessions run and report Up/Down, no BGP effect yet **[in progress]**, (3) operator surface (gRPC `GetBfdSessions` + `rustbgpctl bfd` + events) — observability before behavior, (4) RFC 5882 coupling (strict + non-strict), (5) M51 FRR `bfdd` interop + `COMPARISON.md` `No → Yes`. v1 is IPv4 + IPv6 **global**, static neighbors only. | Exit: M51 shows BGP + BFD Up, induced failure drops BGP faster than the hold timer, recovery reconnects, asserted via FRR `vtysh show bfd peers` **and** `GetBfdSessions`. **Deferred:** multihop (RFC 5883), echo/demand mode, authentication, C-bit/GR-aware nuance, static-route BFD tracking, dynamic-neighbor BFD, hardware/offload, and **IPv6 link-local / unnumbered → v1.1** (gated on link-local BGP peering, which the daemon lacks today — confirmed by the BFD spike). |
| ✓ | **gRPC security audit + authorization split — implementation complete** | ADR-0064 shipped end-to-end across v0.23.0 and v0.24.0: checked 66-RPC risk-tier matrix, runtime decision logs/metrics, threat-model packet, principal-to-role config, per-listener `max_tier` caps, mTLS certificate principal extraction with per-connection cache, result-aware audit outcomes with credential-masked request summaries, JSON method-tier inventory export, opt-in role enforcement (slice 4a) with migration prep (slice 4b prep), operations runbook for `grpc_authz` audit collection and resource guardrails, and the `enforcement = "tier"` default flip (slice 4b, closes #164). Mutating/operator-only RPCs are now isolated from sensitive-read observability endpoints out of the box; the method-tier matrix is enforced by the gRPC runtime; operators who need the prior posture set `enforcement = "legacy"` explicitly. Remaining v1.0 work for this item is the external security review itself. | Done. External review packet is at `docs/adr/0064-threat-model.md`. |
| ✓ | **EVPN production-default decision point — shipped** | Gating soaks: Gate 8b MAC-churn 24 h (2026-05-16, postmortem `docs/soak-gate8b-mac-churn-24h.md`; 69 flip cycles, ~478 K FDB ops, PE1 RSS plateau 17.23–18.93 MB, 0 ADR-0059 drift events) and M37 local-origination 24 h MAC-churn (2026-05-19, postmortem `docs/soak-m37-local-origination-churn-24h.md`; 17 174 churn cycles, 430 400 inject == 430 400 withdraw, RSS plateau 19.13 → 23.53 MB, 10/10 gates green). `apply_bum_enforcement` now defaults to `true` in `src/config/schema.rs`; `apply_aliasing_ecmp` already defaulted to `true` since ADR-0059 slice 3.5 (PR #91). Explicit `false` is still honored for operators who want the prior observe-only / single-dst posture. | Done. |
| P1 | **TCP-AO (RFC 5925)** | BIRD 3 has TCP-AO on Linux, and it is the modern replacement for TCP MD5. This is a visible route-server/security parity gap. ADR-0062 now covers the Linux socket primitive, read-only host capability status, static-neighbor TOML, startup key installation on active-open and passive-listener sockets, and protected M43 BIRD 3.2.1 interop with a mismatched-key fail-closed leg. The protected workflow still probes `CONFIG_TCP_AO=y`, but the current runner satisfies it and runs M43 instead of skipping. | Remaining exit: dynamic-neighbor wildcard-MKT design, runtime key rotation / multi-key rollover, and richer accepted-socket inspection/observability. |
| P1 | **Real-time event/history surface** | Operators debugging policy and convergence need a timeline, not only snapshots. This is where an API-first daemon can beat CLI-first stacks. Bounded unicast route-event history has landed via `RibService.ListRouteEvents` and `rustbgpctl events`, including exact-prefix drilldown; bounded session lifecycle history has landed via `EventService.ListSessionEvents` and `rustbgpctl events sessions`; bounded policy mutation history has landed via `EventService.ListPolicyEvents` and `rustbgpctl events policy`; `EventService.WatchEvents` now exposes route events, structured session lifecycle events, BGP NOTIFICATION sent/received metadata, opt-in policy mutation summary events, and FIB / BLACKHOLE dataplane status-row summary changes through a unified typed stream, with session lifecycle delivery isolated from lossless TCP collision coordination, plus Prometheus visibility for lagged consumers and active subscribers. Route events carry process-local cursors, and `rustbgpctl events watch --backfill N` composes recent route history with a live tail. API/operations docs now classify live streams, bounded history, snapshots, and metrics as separate observability surfaces. Route-level `policy_filtered` events (explicit source/target peer + reason), typed EVPN route events (`EVENT_CATEGORY_EVPN`, `ListEvpnEvents`, `rustbgpctl events evpn`), and per-route FIB dataplane route-outcome events (`DataplaneRouteEvent` for ADR-0061 install/withdraw/apply-failure) have now landed. | Remaining exit: richer per-MAC EVPN event categories, retention decisions if operators need durable history beyond the in-memory ring, and precomputed dataplane summary counters or watch channels if snapshot polling cost shows up at large table scale. |
| P1 | **Mega-module splits — engineering velocity** | Several source files grew past comfortable rebase boundaries and serialize every contributor. The EVPN runtime converger has been extracted to `src/evpn_runtime_converger.rs`, materially shrinking `src/main.rs` (the converger was its largest single block). `src/peer_manager.rs` and `src/evpn_originator.rs` have each been split into concern-based module directories (`mod` root + per-concern siblings + `tests.rs`), dropping both roots well under the target. That clears the large `src/` mega-modules; the remaining candidate is `crates/api/src/event_service.rs` (~2.1k, borderline). Every multi-PR batch in the last two release cycles has hit at least one conflict here, and the `clippy::too_many_lines` suppression count is up to ~94 workspace-wide. Splitting them lets parallel PRs land cleanly and unblocks the cleanups in the sustainability subsection below. | Each oversized module either drops below ~2k LoC or gains a clear top-level module structure (sub-modules per concern) so future PRs can land in narrower files; `clippy::too_many_lines` suppression count trends down rather than up. |
| P2 | **EVPN runtime instance mutation implementation** | `[[evpn_instances]]`, `[[evpn_ip_vrfs]]`, and `[[ethernet_segments]]` are partially live via `ApplyEvpnRuntime` (file/SIGHUP edits remain restart-required). ADR-0063 defines the safe mutation contract: a single command-driven EVPN coordinator, validation-first model updates, and explicit drain/replay across IMET, Type 2, Type 5, DF/ES, and Linux owned state. The foundation has a generationed runtime model, `GetEvpnRuntime` / `rustbgpctl evpn runtime` status, the coordinator core / commit gate, the public `EvpnService.ApplyEvpnRuntime` API contract wired through daemon-owned parsing, full candidate validation, validate-only planning, no-op apply, credential-redacted audit summaries, the IMET controller with per-VNI originate/withdraw, and an Ethernet Segment owner/control surface that keeps Type 1/4 state under `src/evpn_segment.rs`. A daemon actor converger now commits these live shapes — a **single L2VNI add**, a **single L2VNI delete** when the VNI is not an Ethernet Segment member (including IP-VRF deployments where only derived link metadata changes), a **single L2VNI redefine** including Ethernet Segment members when `ip_vrf` link metadata is unchanged, a **single IP-VRF add**, a **single standalone IP-VRF delete** with no L2VNI links, a **single IP-VRF redefine** with unchanged L3VNI/device/table identity, and a **single Ethernet Segment add/delete/redefine** — via ordered IMET origination/withdraw + IP-VRF metadata / effective-table / desired-ES-snapshot republish to the dataplane supervisor, Type 2 / Type 5 originators, SVI task, and segment actor, with rollback on partial failure. L2VNI redefine re-originates the per-VNI Type 3 IMET and republishes the candidate instance table to the level-triggered watch consumers (which drain/re-derive the content-changed VNI); for ES members, the segment actor also drains/rebuilds Type 4 / EAD-per-ES / EAD-per-EVI routes under the candidate route identity while preserving the stable ESI label. This makes `apply_aliasing_ecmp` runtime-drivable via the dataplane `FdbNhg → SingleDst` transition. IP-VRF redefine drains and replays local Type 5 routes for changed route/policy/egress fields. The segment actor consumes runtime instance snapshots, so ES add/redefine can bind a member VNI added by an earlier live L2VNI add when the actor already exists. **Atomic tenant teardown** also commits live: a delete-only plan that drops an ES-member L2VNI together with its Ethernet Segment (delete or member-shrink) and/or a linked IP-VRF in one pass, closing the runtime tenant lifecycle (validated end-to-end by the M47 interop smoke). **`ip_vrf` relink** commits live too — an L2VNI re-homed to a different IP-VRF (or its link added/removed), a dataplane-only republish of the moved reference (the link drives only RFC 9135 overlay-index recursion; RD is unchanged so no Type 3 re-origination). Other non-noop shapes still fail closed without advancing or degrading the committed generation. | Remaining: **L3VNI/device/table IP-VRF redefine** is restart-required by design (kernel VRF identity lifecycle — destroy + recreate; `router_mac` stays live-redefinable); **non-teardown mixed edits** (an add combined with a delete/redefine in one request) fail closed with an operator-actionable "apply each as a separate request" error, pending a generalized converge-to-candidate follow-up ([#210](https://github.com/lance0/rustbgpd/issues/210)). |
...
- [ ] **EVPN VTEP alpha-soak slate.** With Gates 7a / 7b / 7b+1 /
  ...
  PR B import + M39 protected self-hosted smoke against FRR 10.3.1 + sub-second
...
- [ ] **Finer-grained gRPC authorization** — ADR-0064 now has a checked
  method-tier matrix, audit-only runtime decision telemetry, staged
  `[security.grpc.roles]`, explicit non-mTLS listener principal labels, and
  enforced listener `max_tier` caps. Remaining work: mTLS certificate
  principal extraction, deny-by-role/default tier enforcement, and hardened
  result/request audit records.
...
- [ ] **BGP Roles + OTC** (RFC 9234) — **strongest current routing-security gap for the IXP / route-server audience (2026 market signal).** OPEN BGP Role capability (code 9) negotiating role (provider / customer / peer / RS / RS-client), the Only-To-Customer (OTC) attribute set/check for automatic route-leak prevention, and the role-mismatch NOTIFICATION. MANRS-recommended, observed in the wild, shipping in Junos and at IXP route servers, and the natural companion to the ASPA path verification already shipped. Low cost relative to impact (one capability + one path attribute + one policy/validation rule).
- [ ] **ASPA verification — complete scope** — ASPA path verification ships (VRP/ASPA `ValidationSnapshot`, `match_aspa_validation` import policy). Extend toward the full draft-ietf-sidrops-aspa-verification scope (downstream / customer-cone in addition to the upstream path) as ASPA reaches GA across RIRs (ARIN Jan 2026, RIPE production, APNIC Q2 2026), matching BIRD / OpenBGPD router-side verification. Pairs with BGP Roles as the 2026 route-leak-prevention bundle for the MANRS / IXP audience.
...
- [ ] **TCP-AO authentication** (RFC 5925) — modern replacement for TCP MD5; BIRD 3 supports it on Linux. rustbgpd has ADR-0062, an internal Linux socket primitive/capability probe, read-only host capability status in `GlobalService.GetGlobal` / `rustbgpctl global`, static-neighbor TOML, startup key installation on active-open and passive-listener sockets, and M43 protected BIRD 3.2.1 interop with a mismatched-key fail-closed leg. Dynamic neighbors, runtime key rotation, multi-key rollover, and accepted-socket inspection remain.
- [ ] **Real-time BGP observability** — first slices shipped: unicast best-path events now land in a bounded 4096-event RIB ring with `RibService.ListRouteEvents` and `rustbgpctl events`, including exact-prefix drilldown; session lifecycle events now land in a bounded 4096-event peer-manager ring with `EventService.ListSessionEvents` and `rustbgpctl events sessions`; policy mutation summaries now land in a bounded 4096-event peer-manager ring with `EventService.ListPolicyEvents` and `rustbgpctl events policy`; `EventService.WatchEvents` now exposes route events, structured session lifecycle events, BGP NOTIFICATION sent/received metadata, opt-in policy mutation summary events, and FIB / BLACKHOLE dataplane status-row summary changes through a unified typed live stream with peer/family/prefix/type filters where those filters apply. Route events carry process-local cursors and `rustbgpctl events watch --backfill N` can print recent route history before tailing the live stream. Session lifecycle delivery is bounded and separate from lossless TCP collision coordination, with `stream_lagged` warning events when a live subscriber misses bounded route/session source events. Prometheus metrics now also expose lagged live-stream consumers, active stream subscribers, and route-history depth/capacity. API/operations docs now distinguish live streams, bounded history, snapshots, and metrics so operators know which surface to script. Remaining work: route-level `policy_filtered`, richer per-route/per-MAC dataplane and EVPN event categories; durable retention decisions; WatchRoutes missed-event signaling if/when it gains an envelope; precomputed dataplane summary counters or watch channels if full status-snapshot polling becomes expensive; optional subscription-side indexing or a dedicated event bus if subscriber count or event rate makes post-broadcast filtering expensive; foundation for TUI live event view
...
- [ ] **gNMI telemetry adapter (exploratory — no current demand)** — thinking-about-the-space only, not committed. Shape: read-only gNMI (`Capabilities` / `Get` / `Subscribe` ONCE+POLL+SAMPLE) as a thin adapter over rustbgpd's existing typed snapshots and event streams, exposing a *small, exactly-implemented* OpenConfig BGP subset (global / neighbor / afi-safi `state`) rather than claiming full OpenConfig coverage. Distinct from the deprioritized YANG/NETCONF item below — gNMI is gRPC-based and telemetry-first, and no open-source BGP daemon (GoBGP / BIRD / FRR) ships it cleanly, so it is a potential differentiator for the RR / SDN use case (e.g. dropping rustbgpd into the same gNMI pipeline as SONiC / OpenConfig leaves; note SONiC's own gNMI exposes its Redis DB schema, not OpenConfig, so the target is a standard OpenConfig collector, not SONiC-schema parity). Constraints if pursued: `Capabilities` advertises only implemented paths (no overclaiming); `ON_CHANGE` deferred until backed by loss-free change events (today's broadcast streams can lag); `Set` deferred and gated on a real config-transaction model (see "Runtime-vs-file diff and config UX") and reusing ADR-0064 tier authz. Cheap de-risking step is a throwaway spike mapping one subtree (`bgp/neighbors/neighbor[...]/state`) to a snapshot reader and pointing `gnmic` at it to validate the path→reader contract. Revisit when a concrete OpenConfig/gNMI NMS user appears.

### Deprioritized

Features that market research indicates are lower value than originally planned.

- **YANG model / NETCONF** — FRR can't finish their BGP YANG model; gRPC is the modern interface; low ROI. (gNMI — the gRPC-based, telemetry-first successor — is tracked separately as exploratory P4 future work, not deprioritized.)
...
protected self-hosted `kernel-dataplane` workflow because privileged
...
`fib_runtime` netns selectors now run in the protected self-hosted
...
runner drift skips loudly instead of failing unrelated dataplane gates.
