...
|----------|------|---------|-----------------------------|
| P0 | **Privileged runner expansion for remaining dataplane gates** | The self-hosted `kernel-dataplane` workflow now covers M39, M40, M42, and the Docker `fib_runtime` / `fdb_nhg` selectors behind protected-environment approval. The remaining high-value local-only dataplane gates are the earlier VTEP / DF-election smokes and soak variants. | M36, M37, M37+IP, M38, and the Gate 8b MAC-churn soak either run on the protected self-hosted runner or are explicitly kept manual with a documented reason. |
| P0 | **ADR-0061 FIB hardening** | GoBGP and FRR both expose kernel route programming. rustbgpd now has the minimal configured-table path; per-peer / peer-group guardrails and route-count caps are closing the immediate adoption gap, but restart/crash ownership semantics still need a deliberate answer. | Per-peer / peer-group FIB allow-lists and route-count caps shipped; next exit is a safe crash-restart story, either persisted owned-state or a rustbgpd-specific ownership marker. Follow-on if operators need it: a paginated/detail API for over-cap rejected routes beyond the current sampled `route_limit_exceeded` status rows. |
...
- [ ] **EVPN VTEP alpha-soak slate.** With Gates 7a / 7b / 7b+1 /
  ...
  PR B import + M39 manual smoke against FRR 10.3.1 + sub-second
  ...
  a privileged CI runner, 24 h soak with synthetic MAC churn, and
...
- [x] **ADR-0061 opt-in unicast Linux FIB integration** (v0.21.0) —
  ...
  netns harness selector plus the M42 FRR containerlab smoke. The next
  hardening slice adds per-peer / peer-group allow-lists and route-count
  caps; crash-restart adoption remains deferred until rustbgpd has a
  distinct ownership marker or persisted owned-state.
...
- [x] **EVPN Phase 3 partial — ADR-0059 aliasing dataplane ECMP via FDB nexthop groups** (v0.19.0, ADR-0059, PRs #84 / #86 / #87 / #88 / #89) — multi-homed Type 2 routes on the receive path now program FDB nexthop groups (`NDA_NH_ID` / `NHA_FDB`). Slices 1-4 plus M40 manual containerlab smoke against FRR EVPN-MH 10.3.1 all shipped on `main`. M40 16/16 PASS first-shot validates kernel programming under the expected `NHG_TAG | n` / `VTEP_NH_TAG | n` tag scheme. Slice 3.5 follow-ups (periodic `RTM_GETNEXTHOP` drift recovery, `apply_aliasing_ecmp` operator off-switch, IPv6 alias members) deferred at v0.19.0 and shipped post-release in PRs #91 / #92 / #93.
