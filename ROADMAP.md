...
- [ ] **EVPN VTEP alpha-soak slate.** With Gates 7a / 7b / 7b+1 /
  ...
  linked IP-VRF delete / tenant teardown, ES-aware L2VNI delete, IP-VRF
  redefine (and ES-member L2VNI redefine / `ip_vrf` relink), mixed
  L2VNI + IP-VRF edits, and multi-element edits.
  The daemon actor converger already commits eight live shapes through
  `ApplyEvpnRuntime`: single L2VNI add/delete, single non-ES-member L2VNI
  redefine, single IP-VRF add/standalone delete, and single Ethernet Segment
  add/delete/redefine, including ES add/redefine over a member VNI added by an
  earlier live L2VNI add when the segment actor exists.
