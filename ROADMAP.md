...
- [x] **ADR-0072 follow-up sprint (post-v0.29.0, 2026-05-27)** — closes the three deferrals the durable-event-history v1 unblocked: PR #291 wired the dataplane FIB / blackhole producers through EHM (closes the ADR-0072 v1 dataplane deferral); PR #292 added the structured `OtcRouteBlockedEvent` payload under `EVENT_CATEGORY_POLICY` with the next-free `BGP_EVENT_TYPE_OTC_ROUTE_BLOCKED` enum, sourced from a new `TransportEventSink` trait that mirrors `RibEventSink` from PR5 (closes the ADR-0071 deferral); PR #293 wired gNMI `STREAM ON_CHANGE` for `…/neighbor[neighbor-address=*]/state/session-state`, sourcing live FSM transitions from `EventHistoryManager::subscribe_live()` with fresh-snapshot-on-reconnect semantics (closes the ADR-0070 deferral). Each PR includes its own interop coverage (M56 for the gNMI ON_CHANGE work) and preserves the original ADR deferral text with a "Resolved by PR #N" annotation rather than striking it.
...
|----------|------|---------|-----------------------------|
| P0 | **Durable event history / replay** | The in-memory rings and live streams are useful, but operators debugging fabric incidents need "what happened while I was not connected?" across route, session, policy, EVPN, FIB, and BFD events. This also gives future gNMI `ON_CHANGE` subscriptions a stronger replay story. | Bounded persisted event store or WAL; unified `ListEvents` / cursor API; `WatchEvents` resume-from-cursor semantics; CLI history command; backpressure and retention documented. |
...
- [x] **gNMI / OpenConfig telemetry adapter — shipped under ADR-0070.** Read-only gNMI (`Capabilities` / `Get` / `Subscribe` ONCE+POLL+SAMPLE) is a thin adapter over rustbgpd's existing typed snapshots, exposing a *small, exactly-implemented* OpenConfig BGP subset (global + neighbor `state` — no per-AFI-SAFI counters or capabilities in v1) rather than claiming full OpenConfig coverage. M54 verifies the mTLS TCP path with `gnmic` against a real daemon. Constraints: `Capabilities` advertises the OpenConfig modules backing the supported paths (`ModelData` is module-level, not per-path), with the path whitelist enforced at `Get` / `Subscribe`; `ON_CHANGE` deferred until backed by loss-free change events (today's broadcast streams can lag); `Set` deferred and gated on a real config-transaction model (see "Runtime-vs-file diff and config UX") and reusing ADR-0064 tier authz. Distinct from the deprioritized YANG/NETCONF item below — gNMI is gRPC-based and telemetry-first, and the target is a standard OpenConfig collector, not SONiC-schema parity.
