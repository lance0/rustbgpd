...
| P1 | **Real-time event/history surface** | Operators debugging policy and convergence need a timeline, not only snapshots. This is where an API-first daemon can beat CLI-first stacks. The first slice has landed as bounded unicast route-event history via `RibService.ListRouteEvents` and `rustbgpctl events`. | Remaining exit: unified `EventService.WatchEvents`, broader event categories (policy/session/dataplane), and per-prefix route-history query. |
...
- [ ] **Real-time BGP observability** — first slice shipped: unicast best-path events now land in a bounded 4096-event RIB ring with `RibService.ListRouteEvents` and `rustbgpctl events`. Remaining work: unified event bus (`broadcast::Sender<BgpEvent>`) streaming policy_filtered, session_state_change, dataplane, and other operator events; gRPC `EventService` with `WatchEvents` streaming RPC and peer/prefix/type filtering; foundation for TUI live event view
- [ ] **Route history** — first slice shipped: recent add / withdraw / best-path changes are queryable from the bounded RIB event ring. Remaining work: per-prefix timeline query via gRPC and `rustbgpctl history <prefix>`, plus configurable depth if operators need retention beyond the fixed in-memory window
