...
| Graceful Restart (4724) + LLGR (9494) + Notification GR (8538) | Partial | GR helper + minimal restarting speaker shipped; optional bounded shutdown checkpoint publication shipped without boot restore/adoption; real-peer receipts cover IPv4 unicast GR/LLGR (M11/M16), IPv4/IPv6 unicast GR (M85), VPNv4/RTC GR+LLGR and BGP-LS GR (M77), and IPv4/IPv6 labeled-unicast GR (M79); IPv6-unicast LLGR, VPNv6, EVPN, FlowSpec, and BGP-LS VPN restart behavior remains internally tested rather than real-peer receipted |
...
- **Wire / API strictness items.** Continue typed error variants where
  ...
  very large dumps; BMP client connect-loop shutdown. FlowSpec unknown component
