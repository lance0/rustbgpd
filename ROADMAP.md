...
- [ ] **EVPN Gate 9 follow-ups — overlay-index IRB** — Gate 9 slice 6 ships the Interface-less symmetric IRB model, and auto-derived Route Targets (RFC 8365 §5.1.2.1) are now available as an explicit config opt-in for `[[evpn_instances]]` and `[[evpn_ip_vrfs]]`. Remaining standards-completeness item: overlay-index IRB (RFC 9135), needed for broader tenant-routing topologies after the first-class runtime/API work is settled.
