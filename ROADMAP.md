...
|----------|------|---------|-----------------------------|
| P0 | **BGP unnumbered / IPv6 link-local peering** | Whitebox data-center fabrics commonly use interface-scoped BGP sessions with IPv6 link-local next hops and extended nexthop to carry IPv4 NLRI without numbered IPv4 point-to-point links. This is the highest-signal gap for cloud / AI-scale fabric usefulness now that BFD and ECMP have landed. | Static interface-bound neighbor support; correct link-local scope handling in active and passive paths; IPv4 NLRI over IPv6 link-local next hop; CLI/API/status surfaces include interface binding; FRR / Cumulus-style interop smoke proves establish, exchange, failover, and recovery. |
| P0 | **Read-only gNMI / OpenConfig telemetry adapter** | Cloud and whitebox operators already aggregate device state through gNMI/OpenConfig tooling. rustbgpd has typed gRPC snapshots and events; a narrow read-only adapter can plug into that ecosystem without claiming full router-model coverage or adding config mutation risk. | ADR accepted; `Capabilities`, `Get`, and `Subscribe` for a deliberately small OpenConfig BGP subset; expose global / neighbor / AFI-SAFI state first, then BFD, FIB, and EVPN runtime status; verified with `gnmic`; no `Set` until a daemon-wide transaction model exists. |
| P1 | **BGP Roles + OTC (RFC 9234)** | Still the strongest route-server / route-leak-prevention gap. It complements ASPA/RPKI and makes rustbgpd credible for IX, MANRS, and route-server deployments even as the broader target shifts toward data-center fabrics. | OPEN role capability, role-mismatch NOTIFICATION, OTC path attribute set/check behavior, policy/explain visibility, and FRR interop covering provider/customer/peer plus RS/RS-client roles. |
| P1 | **ASPA completion + external validation** | ASPA is moving from research into operational RPKI tooling. rustbgpd already has upstream ASPA verification; the remaining value is complete-scope behavior, cache-update revalidation, and external test vectors rather than a rewrite. | Implement remaining draft scope that applies to supported roles/topologies; add NIST / independent vectors where practical; trigger import-policy revalidation on ASPA/RPKI cache updates for peers whose policy depends on validation state. |
| P1 | **Durable event history / replay** | The in-memory rings and live streams are useful, but operators debugging fabric incidents need "what happened while I was not connected?" across route, session, policy, EVPN, FIB, and BFD events. This also gives future gNMI subscriptions a stronger replay story. | Bounded persisted event store or WAL; unified `ListEvents` / cursor API; `WatchEvents` resume-from-cursor semantics; CLI history command; backpressure and retention documented. |
| P2 | **Config transaction model and runtime/file diff UX** | gRPC owns truth after startup, and gNMI `Set` would require transaction semantics. A validate/diff/commit model across the daemon is the right substrate before adding more remote mutation surfaces. | Candidate config object, validate-only, diff against live effective runtime, atomic commit where supported, explicit restart-required surfaces, rollback/receipt model; no partial silent drift. |
| P2 | **FIB operational hardening** | ADR-0061/0066/0068 now cover configured-table install, ECMP, per-class caps, `multipath_relax`, and Link Bandwidth weighting. The next pain points are lifecycle and scale rather than base capability. | Hot-swap `[[fib_tables]]`; decide whether over-cap rejected routes need paginated/detail APIs beyond sampled `route_limit_exceeded` rows; optional platform-diversity interop for weighted multipath; incremental equal-cost sibling index for wide full-table multipath. |
| P2 | **EVPN / VXLAN polish** | EVPN is alpha-complete for the core VXLAN DC story, with the Linux softswitch local-bias limitation honestly documented. Remaining work should follow demand and close standards gaps that matter to real fabrics. | Native overlay-index Type 5 origination + protected recursion-path interop; optional single-active backup-path preinstall; runtime mixed-edit composer if operators need fewer split requests; keep VXLAN local-bias split-horizon documented as ASIC/offload-dependent on pure Linux. |
| P2 | **TCP-AO dynamic / rotation polish** | Static TCP-AO and BIRD interop are shipped, but route-server/security operators will eventually need wildcard MKT behavior for dynamic neighbors and key rollover. | Dynamic-neighbor wildcard-MKT design; runtime key rotation / multi-key rollover; accepted-socket inspection and observability. |
...
BFD single-hop async with RFC 5882 coupling (ADR-0067, M51), unicast ECMP /
weighted multipath (ADR-0066/0068, M50/M52), EVPN production-default BUM
enforcement and aliasing ECMP, ADR-0064 gRPC tier authorization, and the
major mega-module splits.
