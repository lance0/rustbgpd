...
| P2 | **EVPN runtime instance mutation implementation** | `[[evpn_instances]]`, `[[evpn_ip_vrfs]]`, and `[[ethernet_segments]]` are still startup-pinned. ADR-0063 now defines the safe mutation contract: a single command-driven EVPN coordinator, validation-first model updates, and explicit drain/replay across IMET, Type 2, Type 5, DF/ES, and Linux owned state. The foundation has a generationed runtime model, `GetEvpnRuntime` / `rustbgpctl evpn runtime` status, the coordinator core / commit gate, the public `EvpnService.ApplyEvpnRuntime` API contract wired through daemon-owned parsing, full candidate validation, validate-only planning, no-op apply, credential-redacted audit summaries, fail-closed non-noop rejection without generation advance, the IMET controller primitive with per-VNI originate/withdraw, watch-backed effective L2VNI / IP-VRF tables in the dataplane supervisor (re-projecting on table change), and a Type 5 originator IP-VRF model watch. | Implement the remaining daemon actor convergence so non-noop EVPN runtime edits can commit without direct shared-table swaps; keep SIGHUP restart-required until the coordinator can safely converge Type 2, Type 5, DF/ES, and Linux-owned edits ([#210](https://github.com/lance0/rustbgpd/issues/210)). |
