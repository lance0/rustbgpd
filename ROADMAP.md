...
| P0 | **ADR-0061 FIB hardening** | GoBGP and FRR both expose kernel route programming. rustbgpd now has the minimal configured-table path, peer / peer-group guardrails, route-count caps, and exact-match crash-restart recovery via persisted owned-state. | Persisted owned-state slice lands with focused tests; then decide whether operators need a paginated/detail API for over-cap rejected routes beyond the current sampled `route_limit_exceeded` status rows. |
