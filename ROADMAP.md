...
| P0 | **Privileged runner expansion for remaining dataplane gates** | The self-hosted `kernel-dataplane` workflow now covers M36, M37, M37+IP, M38, M39, M40, M42, conditional M43, and the Docker `fib_runtime` / `fdb_nhg` selectors behind protected-environment approval. M43 is wired into the workflow but runs only on kernels that advertise `CONFIG_TCP_AO=y`; runner rollout for TCP-AO-capable kernels remains tracked separately. The remaining high-value local-only dataplane gates are long soak variants and platform-diversity checks that intentionally do not run on every PR. | Protected CI covers the repeatable EVPN dataplane / DF-election smokes and conditionally covers TCP-AO; any future long-soak gate either gets a protected-runner job or a documented reason for staying manual. |
...
| P0 | **gRPC security audit + authorization split** | v1.0 is blocked on an external security review. mTLS, bearer auth, UDS defaults, and listener-level `access_mode` exist; ADR-0064 now adds a checked 66-RPC risk-tier matrix, runtime decision logs/metrics, a threat-model packet for reviewers, staged `[security.grpc.roles]`, explicit non-mTLS listener principal labels for audit identity, and enforced per-listener `max_tier` caps. Per-principal role enforcement remains audit/deferred. Operator trust still needs mTLS certificate principal extraction, deny-by-role enforcement, the tier-enforcement default flip, and hardened result/request audit records. | Security review complete; mutating/operator-only RPCs can be disabled or isolated from sensitive-read observability endpoints, and the method-tier matrix is enforced by the gRPC runtime. |
...
| P1 | **TCP-AO (RFC 5925)** | BIRD 3 has TCP-AO on Linux, and it is the modern replacement for TCP MD5. This is a visible route-server/security parity gap. ADR-0062 now covers the Linux socket primitive, read-only host capability status, static-neighbor TOML, startup key installation on active-open and passive-listener sockets, and M43 BIRD 3.2.1 interop with a mismatched-key fail-closed leg. The protected workflow includes M43 but skips it on runner kernels without `CONFIG_TCP_AO=y`. | Remaining exit: dynamic-neighbor wildcard-MKT design, runtime key rotation / multi-key rollover, richer accepted-socket inspection/observability, and a TCP-AO-capable protected runner for always-on M43 gating. |
...
`kernel-dataplane` workflow. M43 is wired into that workflow and runs when
the selected runner kernel advertises `CONFIG_TCP_AO=y`.
