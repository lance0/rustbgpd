...
- [ ] **EVPN Phase 3 — Controller injection** — `AddEvpnRoute` / `DeleteEvpnRoute` gRPC RPCs for external controllers (SDN controllers, orchestration systems) to inject EVPN routes into a running RR.
- [ ] **EVPN Phase 4 — Multi-homing + IRB** — DF election (RFC 7432 §8 + RFC 8584), symmetric IRB semantics (RFC 9135), auto-derived Route Targets (RFC 8365 §5.1.2.1), aliasing / backup-path via Type 1 EAD. Needed for active-active ToR deployments.
...
26 automated interop scripts cover M1, M3, M4, M10–M32 against FRR 10.3.1,
