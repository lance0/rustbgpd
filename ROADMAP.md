...
- [ ] **`OtcRouteBlocked` structured event payload** — unblocked by ADR-0072 PR5 (the durable event outbox), but not yet implemented. The existing `bgp_otc_routes_blocked_total{peer, reason}` counter + the `otc_routes_blocked` scalar on `NeighborState` remain the steady-state aggregate; the follow-up should add the per-decision event payload and feed it through `SubscribeFromEvent` with the durable envelope `event_id`.
...
- [ ] **`Subscribe ON_CHANGE`** — unblocked by ADR-0072 PR5 (the durable event outbox shipped in v0.30.0). The next step is wiring the gNMI `Subscribe` handler to consume `SubscribeFromEvent` and translate `BgpEvent` envelopes into per-path leaf updates.
