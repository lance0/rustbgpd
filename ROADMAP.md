...
- [x] **gNMI / OpenConfig telemetry adapter — design committed (ADR-0070).** Read-only gNMI (`Capabilities` / `Get` / `Subscribe` ONCE+POLL+SAMPLE) as a thin adapter over rustbgpd's existing typed snapshots and event streams, exposing a *small, exactly-implemented* OpenConfig BGP subset (global / neighbor / afi-safi `state`) rather than claiming full OpenConfig coverage. Distinct from the deprioritized YANG/NETCONF item below — gNMI is gRPC-based and telemetry-first, and no open-source BGP daemon (GoBGP / BIRD / FRR) ships it cleanly, so it is a potential differentiator for the RR / SDN use case (e.g. dropping rustbgpd into the same gNMI pipeline as SONiC / OpenConfig leaves; note SONiC's own gNMI exposes its Redis DB schema, not OpenConfig, so the target is a standard OpenConfig collector, not SONiC-schema parity). Constraints: `Capabilities` advertises the OpenConfig modules backing the supported paths (`ModelData` is module-level, not per-path), with the path whitelist enforced at `Get` / `Subscribe`; `ON_CHANGE` deferred until backed by loss-free change events (today's broadcast streams can lag); `Set` deferred and gated on a real config-transaction model (see "Runtime-vs-file diff and config UX") and reusing ADR-0064 tier authz. See ADR-0070 for the accepted design: the supported-path subset, the canonical OpenConfig paths, the mTLS / `SensitiveRead` security model, and the PR1–PR4 slicing.
