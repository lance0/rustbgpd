...
- [ ] **EVPN Phase 3 — Multi-homing enforcement (Gate 8b residual) + Gate 9 overlay-index IRB** — Gate 8b residuals: aliasing dataplane ECMP (design landed in [ADR-0059](docs/adr/0059-evpn-aliasing-fdb-nexthop-groups.md): FDB nexthop groups via `NDA_NH_ID` + `NHA_FDB`, raw-netlink construction because `rtnetlink 0.21` has no nexthop API, four implementation slices ~3 days total; first slice extends the portable intent shape with `RemoteMacEntry::alias_group_key: Option<(EthernetSegmentIdentifier, EthernetTagId)>`), production-default multi-homing enforcement after the 24 h soak. Gate 9 follow-ups: overlay-index IRB (RFC 9135 — Gate 9 slice 6 ships the Interface-less variant only), auto-derived Route Targets (RFC 8365 §5.1.2.1), privileged-runner CI gate for the M39 smoke. Needed for active-active ToR deployments and broader tenant-routing topologies.
