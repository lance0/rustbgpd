...
- [ ] **EVPN VTEP alpha-soak slate.** With Gates 7a / 7b / 7b+1 /
  ...
  L2VNI/IP-VRF redefine, mixed L2VNI + IP-VRF edits, and multi-element edits.
  The daemon actor converger already commits seven live shapes through
  `ApplyEvpnRuntime`: single L2VNI add/delete, single IP-VRF add/standalone
  delete, and single Ethernet Segment add/delete/redefine, including
  ES add/redefine over a member VNI added by an earlier live L2VNI add when
  the segment actor exists.
