...
- [ ] **EVPN Gate 9 follow-ups — overlay-index IRB + auto-derived RTs** — Gate 9 slice 6 ships the Interface-less symmetric IRB model. Remaining standards-completeness items are overlay-index IRB (RFC 9135) and auto-derived Route Targets (RFC 8365 §5.1.2.1). Needed for broader tenant-routing topologies after the first-class runtime/API work is settled.
