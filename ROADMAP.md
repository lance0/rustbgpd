...
| P1 | **Real-time event/history surface** | Operators debugging policy and convergence need a timeline, not only snapshots. This is where an API-first daemon can beat CLI-first stacks. Bounded unicast route-event history has landed via `RibService.ListRouteEvents` and `rustbgpctl events`, including exact-prefix drilldown; `EventService.WatchEvents` now exposes route events, structured session lifecycle events, FIB / BLACKHOLE dataplane status-row summary changes, and Prometheus visibility for lagged consumers / active subscribers through a unified typed stream. | Remaining exit: policy, EVPN, BGP NOTIFICATION, and richer dataplane event categories; retention/backfill decisions if operators need them; precomputed dataplane summary counters or watch channels if snapshot polling cost shows up at large table scale. |
...
- [ ] **Real-time BGP observability** — first slices shipped: unicast best-path events now land in a bounded 4096-event RIB ring with `RibService.ListRouteEvents` and `rustbgpctl events`, including exact-prefix drilldown; `EventService.WatchEvents` now exposes route events, structured session lifecycle events, and FIB / BLACKHOLE dataplane status-row summary changes through a unified typed live stream with peer/family/prefix/type filters where those filters apply. Prometheus metrics now expose lagged live-stream consumers, active stream subscribers, and route-history depth/capacity. Remaining work: policy_filtered, notification sent/received, EVPN, richer dataplane event categories, retention/backfill decisions, API-visible missed-event events if operators need them beyond metrics, precomputed dataplane summary counters or watch channels if full status-snapshot polling becomes expensive, optional subscription-side indexing or a dedicated event bus if subscriber count or event rate makes post-broadcast filtering expensive, and foundation for TUI live event view
