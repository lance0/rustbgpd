...
| P0 | **gRPC security audit + authorization split** | v1.0 is blocked on an external security review. mTLS, bearer auth, UDS defaults, and listener-level `access_mode` exist; ADR-0064 now adds a checked 66-RPC risk-tier matrix as the foundation for finer-grained authz. Operator trust still needs runtime tier enforcement and audit logs. | Security review complete; mutating/operator-only RPCs can be disabled or isolated from sensitive-read observability endpoints, and the method-tier matrix is enforced by the gRPC runtime. |
