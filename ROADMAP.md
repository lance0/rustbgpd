...
- [ ] **EVPN VTEP alpha-soak slate.** With Gates 7a / 7b / 7b+1 /
  ...
  confidence is completing the remaining ADR-0063 runtime convergence shapes:
  linked IP-VRF delete / tenant teardown, ES-aware L2VNI delete, `ip_vrf`
  relink, L3VNI/device/table IP-VRF redefine, mixed
  L2VNI + IP-VRF edits, and multi-element edits.
  The daemon actor converger already commits nine live shapes through
  `ApplyEvpnRuntime`: single L2VNI add/delete/redefine, single IP-VRF
  add/standalone delete/redefine with unchanged
