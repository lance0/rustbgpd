...
- [ ] **Investigate +49-min monitor saturation under M33 1k rps churn** — even with the writer-split, the soak still hits the saturation path at ~+49 min: a peer's TCP receive buffer can't sustain the reflected route firehose, our writer parks on `write_all`, the bulk channel fills, we Cease/8 + flap. The flap is clean now, but the underlying "monitor can't keep up" is worth understanding. Likely paths: monitor-side parsing speed (`evpn-monitor` HashSet update at ~3k routes/sec), kernel TCP receive buffer tuning, or RIB-side distribution batching that emits more, smaller UPDATEs than necessary. Not a 1.0 blocker — failure mode is now bounded and observable — but the investigation feeds future scale guidance.
