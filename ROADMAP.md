...
- [x] **ASPA verification (RFC 9582)** — upstream path verification with RTR v2 support. AspaTable + verify_upstream() algorithm, RTR v2 codec (ASPA PDU type 11) with automatic v1 fallback, best-path step 0.7 (Valid > Unknown > Invalid), export policy `match_aspa_validation`, gRPC `aspa_state` exposure. Import policy limitation documented (validation runs post-ingress). Downstream verification deferred (needs per-peer relationship config). (ADR-0049)
...
- [x] ASPA verification (RFC 9582) — upstream verification with RTR v2 (ADR-0049)
