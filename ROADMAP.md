...
- [ ] **EVPN Phase 2 — VTEP mode** — local EVI/VRF/VNI state, MAC learning from kernel FDB, local route origination. Required for general-purpose routing; not required for RR-only deployments. Kernel integration design landed in ADR-0054 with the Gate 7b foundation (v0.14.0): the remote-MAC FDB programmer ships, the dataplane boundary is locked, and the supervisor / projection / dump-merge plumbing is in place. Local-MAC learning + Type 2/3 origination are the next slices on top of that foundation (tracked separately in `docs/evpn-enablement.md` Gate 7b "Still ahead").
