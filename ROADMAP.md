...
| P1 | **Real-time event/history surface** | Operators debugging policy and convergence need a timeline, not only snapshots. This is where an API-first daemon can beat CLI-first stacks. Bounded unicast route-event history has landed via `RibService.ListRouteEvents` and `rustbgpctl events`, including exact-prefix drilldown; the first `EventService.WatchEvents` slice exposes those route events through a unified typed stream. | Remaining exit: broader event categories (policy/session/dataplane) and retention/backfill decisions if operators need them. |
...
- [ ] **Real-time BGP observability** — first slices shipped: unicast best-path events now land in a bounded 4096-event RIB ring with `RibService.ListRouteEvents` and `rustbgpctl events`, including exact-prefix drilldown; `EventService.WatchEvents` now exposes those route events through a unified typed live stream with peer/family/prefix/type filters. Remaining work: policy_filtered, session_state_change, dataplane, EVPN, and other operator event categories; retention/backfill decisions; explicit missed-event/slow-consumer signaling for lagged watchers; optional subscription-side indexing or a dedicated event bus if subscriber count or event rate makes post-broadcast filtering expensive; foundation for TUI live event view
