...
|----------|------|---------|-----------------------------|
| P0 | **Performance / polish phase** *(ASPA PR1 correctness slice as kickoff)* | ASPA momentum is real but mostly ecosystem readiness (Cloudflare Radar deployment visibility, NIST BRIO tooling, RIPE signing maturity) rather than obvious operator pull — upstream-only verification covers the route-leak prevention story for the common case today. Adoption-moving work sits in perf/polish: bulk initial load, churn benchmarks, CI benchmark tracking, shared route storage / compact indexing research. A short ASPA correctness slice closes algorithm-fidelity uncertainty cheaply before the pivot; downstream verification + cache-update revalidation stay planned but deferred until operator signal materializes or the MANRS / route-leak-prevention bundle becomes the explicit next headline. | Kickoff: ASPA PR1 (2-5 days) — bounds-equivalence investigation against draft-ietf-sidrops-aspa-verification-25 §5.4 resolves to documented proof or replacement bounds-checker; per-AFI/SAFI filter applied at the validation entry point; `match_aspa_validation` policy-match unit coverage; NIST-BRIO vectors imported where format and license cooperate (hand-crafted equivalents otherwise). Main phase: define and track convergence + steady-state memory benchmarks in CI; bulk initial-load and churn improvements shipped as discrete PRs; shared route storage / compact indexing research informs the next bigger refactor. |
