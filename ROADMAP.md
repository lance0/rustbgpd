...
| P0 | **gRPC security audit + authorization split** | v1.0 is blocked on an external security review. mTLS, bearer auth, UDS defaults, and listener-level `access_mode` exist; ADR-0064 now adds a checked 66-RPC risk-tier matrix, runtime decision logs/metrics, a threat-model packet for reviewers, staged `[security.grpc.roles]`, explicit non-mTLS listener principal labels, enforced per-listener `max_tier` caps, mTLS certificate principal extraction, opt-in deny-by-role enforcement, result-aware audit outcomes, and credential-masked request summaries. Operator trust still needs the tier-enforcement default flip after the documented migration window, plus durable audit sink / retention guidance. | Security review complete; mutating/operator-only RPCs can be disabled or isolated from sensitive-read observability endpoints, the method-tier matrix is enforced by the gRPC runtime, and operators have completed the tier-mode migration checklist before the production default changes. |
