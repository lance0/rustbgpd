...
- [ ] **EVPN polish + observability gaps** (low-priority, Phase 1 known limitations):
  ...
  - **CI wiring for M29-M33** — interop scripts running cleanly on a developer box but tied into the GitHub Actions matrix. **M29 done** — `.github/workflows/interop.yml` runs the EVPN cap-negotiation gate against FRR 10.3.1 on every push and PR (~10 s test, ~3 min total with image build + topology spin-up). **M30b blocked on hosted runners** — the Azure-tuned kernel on ubuntu-latest (6.17.0-1010-azure) ships without the `vrf` module, so `ip link add ... type vrf` fails inside the FRR container and FRR cannot bind the L3VNI to originate Type 5; verified empirically via a throwaway probe workflow. M30 (Type 2 with VXLAN + bridge, no VRF) is kernel-viable on hosted runners but the same probe surfaced an orthogonal `start_rustbgpd` quirk under the sudo'd containerlab path; not chased further. M32 (multi-homing on a bond ES) and M33 (50k-route scale + churn) stay manual until we have a self-hosted runner with the wall-time budget. Net effect: M29 stays the only EVPN gate on hosted runners; M30/M30b/M31/M32/M33 stay manual unless we add a self-hosted runner.
