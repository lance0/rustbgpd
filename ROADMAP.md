...
| P1 | **Mega-module splits — engineering velocity** | Several source files grew past comfortable rebase boundaries and serialize every contributor. The EVPN runtime converger has been extracted to `src/evpn_runtime_converger.rs`, materially shrinking `src/main.rs` (the converger was its largest single block). `src/peer_manager.rs` has been split into a concern-based module directory (`peer_manager/{mod,lifecycle,inbound,policy,reconcile,notifications,tests,…}.rs`), dropping its root well under the target. Still oversized: `src/evpn_originator.rs` and `crates/api/src/event_service.rs`. Every multi-PR batch in the last two release cycles has hit at least one conflict here, and the `clippy::too_many_lines` suppression count is up to ~94 workspace-wide. Splitting them lets parallel PRs land cleanly and unblocks the cleanups in the sustainability subsection below. | Each oversized module either drops below ~2k LoC or gains a clear top-level module structure (sub-modules per concern) so future PRs can land in narrower files; `clippy::too_many_lines` suppression count trends down rather than up. |
