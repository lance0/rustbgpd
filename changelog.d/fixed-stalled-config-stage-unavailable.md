...
- A config store that never acknowledges a staged runtime-config candidate
  ...
  now end at a pre-effect deadline, 30 seconds before the budget. The
  mutation fails as `UNAVAILABLE`, the late stage is discarded, and the
  daemon keeps running. This covers the stage acknowledgement in FIB-table,
  neighbor, peer-group and policy CRUD and config transactions, the
  FIB-table read and peer-manager handoff, and the peer-group `Set` read.
