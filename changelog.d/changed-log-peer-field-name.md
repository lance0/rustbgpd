...
- Log events name a BGP peer's address in one event field, `peer`. Events
  ...
  **Operator-visible:** a log pipeline or alert that selects on the old
  ...
  `client`, which now holds the client's `address:port` and is omitted when
  the socket address is unavailable instead of logging `Some(…)` or `None`,
  and the metrics endpoint's `metrics connection error` event
  ...
  the bare address instead of `address:port`, and the rejected unconfigured
  link-local inbound event logs the bare address as `peer` with the source
  port and scope as `port` and `scope_id`.
