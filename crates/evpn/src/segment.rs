...
pub struct EthernetSegment {
    ...
    /// the DF Election extended community (non-revertive) so a returning
    /// higher-preference PE does not preempt the incumbent DF on a tie.
    /// Only meaningful for the preference algorithms; the daemon config
    /// layer rejects `true` for default-modulo / HRW.
