...
//! (`evaluate_with_attribution_legacy`, deleted after the freeze).
//! The corpus test asserts the frozen outcomes against the IR path
//! (`PolicyChain::evaluate_with_attribution`, which compiles through
//! `crate::compile` and evaluates through the `CompiledChain`
//! evaluator) and asserts the regenerated inputs against their frozen
//! reprs, so neither an evaluator nor a generator edit can drift
//! silently. The corpus is never regenerated by tooling — an
//! auto-blessed golden agrees with whatever the code does. A corpus
//! change is a hand-edited, reviewed commit.
//!
        ...
        for action in [PolicyAction::Permit, PolicyAction::Deny] {
            for default_action in [PolicyAction::Permit, PolicyAction::Deny] {
                for with_mods in [false, true] {
                    ...
                    s.action = action;
                    if with_mods {
                        // Also attached to Deny statements: the frozen
                        // goldens drop their modifications, the
                        // compiler must too.
...
/// Every corpus case against the IR path: the frozen outcome must
/// equal its `(PolicyResult, PolicyEvaluation)`. The regenerated
/// inputs are asserted against their frozen reprs first, so an
/// outcome comparison never runs against a silently different chain
/// or route.
#[test]
fn golden_corpus_pins_ir_evaluator() {
