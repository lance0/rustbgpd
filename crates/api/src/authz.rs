...
pub const METHODS: &[GrpcMethodAuthz] = &[
    ...
    method(
        ...
        "/rustbgpd.v1.ConfigService/StreamPlanConfigTransaction",
        AuthTier::SensitiveRead,
    ),
    method(
        "rustbgpd.v1.ConfigService",
    ...
    method(
        "rustbgpd.v1.NeighborService",
        "ReplayOutbound",
        "/rustbgpd.v1.NeighborService/ReplayOutbound",
        AuthTier::Mutating,
    ),
    method(
        "rustbgpd.v1.NeighborService",
    ...
    method(
        ...
        "/rustbgpd.v1.PolicyService/GetValidationPolicyPosture",
        AuthTier::SensitiveRead,
    ),
    method(
        "rustbgpd.v1.PolicyService",
    ...
    method(
        "rustbgpd.v1.RibService",
        "LookupBestPath",
        "/rustbgpd.v1.RibService/LookupBestPath",
        AuthTier::SensitiveRead,
    ),
    method(
        "rustbgpd.v1.RibService",
    ...
    method(
        ...
        "ValidateRouteOrigin",
        "/rustbgpd.v1.RpkiService/ValidateRouteOrigin",
        AuthTier::SensitiveRead,
    ),
    method(
        "rustbgpd.v1.RpkiService",
        "LookupAspa",
        "/rustbgpd.v1.RpkiService/LookupAspa",
        AuthTier::SensitiveRead,
    ),
    method(
        "rustbgpd.v1.RpkiService",
        "VerifyAsPath",
        "/rustbgpd.v1.RpkiService/VerifyAsPath",
        AuthTier::SensitiveRead,
    ),
    method(
        "rustbgpd.v1.RpkiService",
    ...
    method(
        ...
        "/rustbgpd.v1.EvpnService/ListDuplicateMacQuarantines",
        AuthTier::SensitiveRead,
    ),
    method(
        "rustbgpd.v1.EvpnService",
...
mod tests {
    ...
    #[test]
    fn evpn_explain_is_sensitive_read() {
        assert_eq!(
            method_authz("/rustbgpd.v1.RibService/ExplainEvpnRoute").map(|method| method.tier),
            Some(AuthTier::SensitiveRead),
        );
    }

    #[test]
    fn validation_policy_posture_is_sensitive_read() {
        assert_eq!(
            method_authz("/rustbgpd.v1.PolicyService/GetValidationPolicyPosture").map(|m| m.tier),
            Some(AuthTier::SensitiveRead)
        );
    }

    #[test]
    fn aspa_diagnostics_are_sensitive_reads() {
        for method in ["LookupAspa", "VerifyAsPath"] {
            assert_eq!(
                method_authz(&format!("/rustbgpd.v1.RpkiService/{method}"))
                    .map(|method| method.tier),
                Some(AuthTier::SensitiveRead)
            );
        }
    ...
    #[test]
    fn replay_outbound_is_mutating() {
        assert_eq!(
            method_authz("/rustbgpd.v1.NeighborService/ReplayOutbound").map(|method| method.tier),
    ...
    #[test]
    fn duplicate_mac_quarantine_listing_is_sensitive_read() {
        assert_eq!(
            method_authz("/rustbgpd.v1.EvpnService/ListDuplicateMacQuarantines")
                .map(|method| method.tier),
            Some(AuthTier::SensitiveRead)
        );
    }
