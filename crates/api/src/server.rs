...
pub struct ServeConfig {
    ...
    /// The RIB manager's published export roster (`GetPolicyStats`).
    pub export_roster: rustbgpd_rib::export_roster::ExportRosterReader,
...
async fn run_listener(
    ...
    let export_roster = config.export_roster;
...
async fn run_tcp_listener(
    ...
    export_roster: rustbgpd_rib::export_roster::ExportRosterReader,
    asn: u32,
    router_id: String,
    listen_port: u32,
    metrics: BgpMetrics,
    ...
    routes.add_service(PolicyServiceServer::with_interceptor(
        ...
        .with_export_roster(export_roster.clone()),
        interceptor.clone(),
    ));
    ...
    if tls_enabled {
        routes.add_service(GNmiServer::with_interceptor(
            GnmiService::new_with_operator_queries(
                asn,
                router_id.clone(),
                access_mode,
                peer_mgr_tx.clone(),
