...
impl EhmHarness {
    fn new(subscribe_to_commits: bool) -> Self {
        ...
        assert_bound_zero_queue_depth_children(&metrics);
...
fn assert_bound_zero_queue_depth_children(metrics: &BgpMetrics) {
    let text = metrics_text(metrics);
    let children: Vec<&str> = text
        .lines()
        .filter(|line| line.starts_with("bgp_event_outbox_queue_depth{"))
        .collect();
    assert_eq!(
        children.len(),
        Category::ALL.len(),
        "EHM benchmark requires the bound collector to expose six queue-depth children"
    );
    for category in Category::ALL {
        let expected = format!(
            "bgp_event_outbox_queue_depth{{category=\"{}\"}} 0",
            category.as_str()
        );
        assert!(
            children.contains(&expected.as_str()),
            "EHM benchmark ledger-backed queue-depth collector is not zero for {category:?}: {children:?}"
        );
    }
}
