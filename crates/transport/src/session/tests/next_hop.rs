...
async fn process_update_rejects_ipv4_mp_link_local_without_extended_nexthop() {
    ...
    // link-local MP_REACH (no route reaches the RIB), but the UPDATE is
    // treat-as-withdraw (Invalid NEXT_HOP), not a session reset. Pinning both
    // ends guards against a regression in either direction — silently
    // accepting the route, or escalating to a session reset. The positive
    // case is covered by
    // `process_update_accepts_ipv4_mp_link_local_for_scoped_unnumbered_peer`.
    while let Ok(message) = rib_rx.try_recv() {
        assert!(
            !matches!(&message, RibUpdate::RoutesReceived { announced, .. } if !announced.is_empty()),
            "no route may reach the RIB"
        );
    }
    assert_eq!(
        session.notifications_sent, 0,
        "treat-as-withdraw must not send a NOTIFICATION"
    );
}
