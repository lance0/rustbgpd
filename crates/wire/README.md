...
| 9234 | BGP Roles (OPEN capability code 9, `BgpRole`) + Only-to-Customer path attribute (type 35, `PathAttribute::OnlyToCustomer { asn, partial }`). Valid OTC stays typed and preserves Partial; Extended Length input canonicalizes on emission. The legacy decoder reports malformed flags/length with the RFC 4271 subcode and offending attribute data, while revised decoding omits the attribute and records RFC 7606 treat-as-withdraw. Negotiation + ingress/egress rules live in the daemon (ADR-0071) |
