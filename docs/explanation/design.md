...
**Later:** the EVPN runtime convergence remainder is down to L3VNI/device/table IP-VRF identity redefine (restart-required by design); decomposable mixed edits already commit live with fail-stop semantics — see [evpn-enablement.md](../project/evpn-enablement.md) for the full live-vs-fail-closed breakdown. VPNv4/v6 reflection has shipped; MPLS-EVPN encap remains later. (Shipped since: duplicate-MAC remote-route suppression + bounded atomic quarantine listing + manual clear, production-default `apply_bum_enforcement` / `apply_aliasing_ecmp` enforcement, auto-derived Route Targets per RFC 8365 §5.1.2.1, receive-side RFC 9135 overlay-index Type 5 recursion with fail-closed unresolved / ambiguous gateways, and controller Type 5 Gateway Address injection.)
...
For crate dependency graph, runtime model, ownership model, data flow, lifecycle flows, backpressure model, and the "where to change X" guide, see [docs/explanation/architecture.md](architecture.md).
...
group-accessible access; see `docs/reference/configuration.md` under `[security.grpc]`.
...
Dispositions follow RFC 7606, not RFC 4271's blanket session reset: `wire::validate::ErrorDisposition` classifies each error as attribute-discard, treat-as-withdraw, or session-reset, and the session applies the strongest disposition in the message (§3 (h)). Treat-as-withdraw and attribute-discard keep the session Established. See [RFC_NOTES.md — RFC 7606](../reference/rfc-notes.md#rfc-7606--revised-bgp-update-error-handling) for the authoritative per-attribute table.
...
**Best-path: type-specific head + shared BGP body.** `evpn_tiebreak_simple` runs the stale rank and a Type-2-specific MAC Mobility head (sticky flag + sequence per RFC 7432 §15.1), then falls through to the same chain as unicast (LocalPref → AS_PATH → ORIGIN → MED → eBGP>iBGP → effective BGP Identifier → CLUSTER_LIST length → peer address, with a locally originated route ranked ahead of every received route at the identifier step). Type 1/4 DF-election tiebreaks are not implemented — the RR reflects, downstream VTEPs elect. Types 3/5 have no type-specific head.
...
Phase 1 hardening (the RR enablement ladder in [evpn-enablement.md](../project/evpn-enablement.md))
...
**Dual-family BGP listener:** By default the daemon listens for inbound BGP on both address families — `0.0.0.0:{listen_port}` and `[::]:{listen_port}` — behind one accept loop and one accept channel (ADR-0019). `IPV6_V6ONLY` is set explicitly on the IPv6 socket so v4-mapped connections always arrive through the IPv4 socket. Every listener-side authentication entry below (MD5 key, TCP-AO MKT, GTSM selector) is installed on the socket matching the peer's address family, so IPv6 peers get the same inbound enforcement as IPv4 peers. Under that default, if one family cannot be bound (for example IPv6 disabled on the host), a warning names the family and the other keeps serving; startup fails only when neither family binds. Optional `[global].listen_addresses` replaces the wildcard pair with exact endpoints — at most one address per family, restart-required — enabling only the listed families, selecting the matching active-open source address, and binding every listed endpoint atomically. ADR-0019 records the dual-wildcard default and predates that knob; [CONFIGURATION.md](../reference/configuration.md) carries the validation rules.
...
- See [CONFIGURATION.md](../reference/configuration.md#inbound_admission).
...
Bounded channels, prefix limits, and backpressure behavior are detailed in [docs/explanation/architecture.md — Failure and Backpressure Model](architecture.md#failure-and-backpressure-model). Additional guards:
...
- Native gRPC mTLS is supported on TCP listeners via `tls_cert_file` /
  ...
  TLS-without-mTLS half-mode) — see docs/reference/configuration.md "Native gRPC mTLS".
...
- Per-method tier authorization (ADR-0064) is the enforcement model: every
  RPC carries a tier assignment (docs/reference/grpc-method-inventory.md), listeners
...
See [docs/explanation/architecture.md — Where to Change X](architecture.md#where-to-change-x) for a task-oriented guide. The crate dependency graph and runtime model are also in docs/explanation/architecture.md.
...
The 8 non-negotiable constraints are defined in [docs/explanation/architecture.md — Design Invariants](architecture.md#design-invariants). They cover: pure FSM, independent wire crate, bounded channels, no silent drops, no panics on malformed input, structured protocol violation events, enforced resource limits, and interop-tested features.
