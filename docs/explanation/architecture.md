...
| `AttrSet` | `rib::attr_set` | Shared route attributes with cached best-path inputs. Construction and copy-on-write edits keep the inputs consistent; interning shares both across routes. |
...
| Local EVPN/VTEP domain | `crates/evpn/src/` — `instance.rs`, `route_target.rs`, `mac.rs` (LocalMacObservation, RemoteMacTable), `dataplane.rs` (DataplaneIntent / DataplaneReport), `origination.rs` / `origination_macip.rs` / `origination_es.rs` (per-route-type state machines), `projection.rs` (RIB → RemoteMacTable), `segment.rs` / `df_election.rs` / `aliasing.rs` / `label_allocator.rs` (Gate 8/8b multi-homing), `ip_vrf/` (IpVrf / IpVrfTable, readiness probe, Type 5 origination + projection helpers — Gate 9), `duplicate_mac.rs` / `duplicate_ip.rs` (duplicate MAC / IP detection windows), `managed_netdev.rs` (ADR-0091 managed-netdev desired rows and status types), `runtime.rs` (ADR-0063 generationed runtime model), `runtime_plan_shape.rs` (supported runtime plan-shape classifier), `colon_hex.rs` (operator colon-hex MAC/ESI parsing); daemon-owned receive-side projection and its route-event/periodic reconcile triggers live in `src/evpn_dataplane.rs` |
| EVPN Linux kernel dataplane | `crates/evpn-linux/src/` — `reconcile.rs` (reconcile actor), `dataplane.rs` (`Dataplane` trait), `in_memory.rs` (in-memory fake), `snapshot.rs` (kernel snapshot + owned set), `backoff.rs` (failed-op retry schedule), `linux/fdb.rs` (program/withdraw), `linux/links.rs` (bridge + VXLAN inventory), `linux/notify.rs` (RTNLGRP_NEIGH classifier + RTNLGRP_IPV4_ROUTE / RTNLGRP_IPV6_ROUTE route observer), `linux/probe.rs`, `bum_filter.rs` + `linux/bum_filter.rs` (Gate 8b split-horizon), `enforcement.rs` (Gate 8b BUM-enforcement plan, no kernel mutation), `ac_gate.rs` + `linux/ac_gate.rs` (single-active AC gate), `linux/link_carrier.rs` (ADR-0085 carrier monitor), `linux/managed_netdev.rs` (ADR-0091 managed-netdev lifecycle), `linux/ip_vrf.rs` (Gate 9 VRF / L3VXLAN dumps + `probe_ip_vrfs`), `l3_diff.rs` + `linux/l3.rs` + `linux/routes.rs` (Gate 9 slice 6 import-side L3 FIB programming), `l3_adoption.rs` + `linux/l3_adoption.rs` (ADR-0079 crash-restart adoption sweep), `linux/nexthop_raw/` + `linux/fdb_nhg.rs` + `group_state.rs` + `nh_id_alloc.rs` + `diff.rs` Pass 1b (ADR-0059 FDB nexthop group aliasing-ECMP) |
