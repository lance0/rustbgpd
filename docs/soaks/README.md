...
| [Route-server flagship 24-hour soak, 2026-09-21 (PASS)](soak-rs-flagship-24h-2026-09-21.md) | Qualifying run on the v0.71.0 tag; every gate passes on the on-host verdict; slowest `policy stats` read 1,969 ms, 31 ms under its 2 s deadline |
| [Route-server flagship 24-hour soak, 2026-09-24 (FAIL)](soak-rs-flagship-24h-2026-09-24.md) | Run on the v0.72.0 tag; fails management correctness only: one of 17,556 `policy stats` reads expired at its 2 s deadline inside a reload commit (daemon `DEADLINE_EXCEEDED`, import stage); every other gate passes |
