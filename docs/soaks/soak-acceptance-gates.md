...
| readyz availability | Fewer than 3 consecutive samples miss HTTP 200 within 250 ms (the route-server-1000 receipt enforces the 250 ms bound during reloads at this exact shape). Three consecutive is the readiness-probe `failureThreshold` default: the point at which an ordinary consumer withdraws the workload. The endpoint reports instantaneous truth, so a single sample is below the resolution any prober acts on; the gate models the consumer. The count is borrowed, not the period: at this scenario's 30 s sampling cadence three consecutive samples span at least 60 s, so the gate stays stricter in elapsed time than the prober default it is calibrated against. Passing is not a statement that an isolated breach is acceptable — the verdict reports the total, the longest consecutive run, and each offending sample split into a non-200 status and a late 200, so isolated breaches stay visible as latency findings | CSV `readyz_code`, `readyz_ms` | Probed every sample, including mid-reload and mid-trip. |
...
| Counter advancement | `bgp_messages_sent_total` strictly increases across every adjacent sample; equal or decreasing intervals fail | CSV `msgs_sent_total`, with the first equal interval reported by timestamp and value | Advances with every keepalive/UPDATE across 1000 sessions; a decrease remains the daemon-restart diagnostic, while equality identifies a non-advancing sample interval. |
...
The accepted 24 h archive has 2,882 strictly advancing adjacent message-counter
intervals, so this stricter future-analysis rule does not change its verdict;
its exact terminal reflected-delivery receipt remains independently valid. This
sampled aggregate counter proves neither that every churn update propagated in
real time nor that partial or sub-sampling-interval stalls were absent, and the
churn-floor slack is unchanged.
