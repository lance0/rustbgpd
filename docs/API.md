...
| `GetPolicyStats` | Read the live per-term hit counters of the installed policy chains (since chain install; direction `import`, `export`, or `both` — import chains also report their install generation). An explicit unknown peer returns `NOT_FOUND`; an import-session timeout returns `DEADLINE_EXCEEDED`; a selected session exit returns `UNAVAILABLE`; a session that answers with no import chain contributes no row. CLI: `rbgp policy stats`. `SensitiveRead` tier. |
