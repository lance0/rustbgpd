...
Seven non-noop shapes converge live and commit the next generation: a
...
**single Ethernet Segment add, delete, or redefine** (exactly one added,
...
delete / tenant teardown, IP-VRF redefine, ES-member L2VNI redefine / `ip_vrf`
relink, mixed L2VNI + IP-VRF edits, multi-element edits, ES-aware L2VNI delete,
or an apply on an RR-only / no-actor daemon — returns
...
IP-VRF delete with no L2VNI links, or a single Ethernet Segment add/delete/redefine
against the committed model; the response
carries the committed generation and outcome. Every other non-noop shape
(linked IP-VRF delete, IP-VRF redefine, ES-member L2VNI redefine / `ip_vrf` relink,
a mixed L2VNI + IP-VRF request,
