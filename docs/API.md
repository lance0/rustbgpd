...
`[managed_netdevs]` bridge, fixed-VNI VXLAN, VLAN upper, VRF, and L3VXLAN rows joined with
the latest Linux link snapshot, plus rustbgpd-stamped orphan links observed by
the dataplane actor. This is read-only status: a row can be `desired-absent`,
`foreign-present`, `owned-unsafe`, `owned-safe`, `orphaned`, or `unknown`.
Bridge, fixed-VNI VXLAN, VLAN upper, VRF, and L3VXLAN rows are active lifecycle intent
inside the dataplane reconciler. The RPC itself never mutates links.
