...
Six non-noop shapes converge live and commit the next generation: a
...
**single Ethernet Segment add or delete** (exactly one added or one removed
`[[ethernet_segments]]` entry and no other changes). A supported add/delete
...
L2VNI delete, ES redefine, or an apply on an RR-only / no-actor daemon — returns
...
or a single Ethernet Segment add/delete against the committed model; the response
