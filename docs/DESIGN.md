...
**Later:** Duplicate-MAC quarantine action (ADR-0055 §9), aliasing dataplane ECMP and production-default multi-homing enforcement (Gate 8b follow-ups), Type 5 origination + Type 5 FIB programming (Gate 9 remaining slices — the IP-VRF config schema and readiness probe are already in via `[[evpn_ip_vrfs]]` per RFC 9136 §4.4.2 / ADR-0058), VPNv4/v6, MPLS-EVPN encap.
