...
**Later:** remaining EVPN runtime convergence shapes (`ip_vrf` relink, L3VNI/device/table IP-VRF redefine, and non-teardown mixed edits; single L2VNI/IP-VRF/Ethernet-Segment add/delete/redefine and atomic tenant teardown already commit live via `ApplyEvpnRuntime`, including single L2VNI redefine with unchanged `ip_vrf` link metadata), VPNv4/v6, MPLS-EVPN encap. (Shipped since: duplicate-MAC remote-route suppression + manual clear, production-default `apply_bum_enforcement` / `apply_aliasing_ecmp` enforcement, auto-derived Route Targets per RFC 8365 §5.1.2.1, receive-side RFC 9135 overlay-index Type 5 recursion with fail-closed unresolved / ambiguous gateways, and controller Type 5 Gateway Address injection.)
