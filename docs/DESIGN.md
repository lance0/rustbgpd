...
**Later:** remaining EVPN runtime convergence shapes (IP-VRF redefine, ES-member L2VNI redefine / `ip_vrf` relink, linked IP-VRF delete / tenant teardown, ES-aware L2VNI delete, mixed, and multi-element edits; eight `ApplyEvpnRuntime` shapes already commit live, including single non-ES-member L2VNI redefine), VPNv4/v6, MPLS-EVPN encap. (Shipped since: duplicate-MAC remote-route suppression + manual clear, production-default `apply_bum_enforcement` / `apply_aliasing_ecmp` enforcement, auto-derived Route Targets per RFC 8365 §5.1.2.1, receive-side RFC 9135 overlay-index Type 5 recursion with fail-closed unresolved / ambiguous gateways, and controller Type 5 Gateway Address injection.)
