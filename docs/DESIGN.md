...
**Later:** the EVPN runtime convergence remainder (L3VNI/device/table IP-VRF identity redefine — restart-required by design — and generic mixed add/delete/redefine edits; the rest of the `ApplyEvpnRuntime` and SIGHUP EVPN shape set already commits live — see [evpn-enablement.md](evpn-enablement.md) for the full live-vs-fail-closed breakdown), VPNv4/v6, MPLS-EVPN encap. (Shipped since: duplicate-MAC remote-route suppression + manual clear, production-default `apply_bum_enforcement` / `apply_aliasing_ecmp` enforcement, auto-derived Route Targets per RFC 8365 §5.1.2.1, receive-side RFC 9135 overlay-index Type 5 recursion with fail-closed unresolved / ambiguous gateways, and controller Type 5 Gateway Address injection.)
