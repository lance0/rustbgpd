...
|---------|--------|-------|
| Foundation M-series interop | CI-gated on every PR | `interop.yml` covers the core protocol and management surfaces listed in [`INTEROP.md`](INTEROP.md#ci-coverage), including M73's GoBGP 4.6.0 BGP-LS source -> rustbgpd RR -> GoBGP sink reflection / withdrawal receipt and M74's GoBGP 3.37.0 VPNv4 (SAFI 128) reflection receipt (RD / MPLS label / RT / next-hop preserved verbatim, RD-scoped identity, withdrawal propagation, no dataplane install). |
...
Compact M36-M74 index (details and assertions stay in
...
| Other M41-M74 rows in this span | Foundation interop receipts such as BLACKHOLE FIB discard, gRPC/gNMI, EVPN Type 5 injection, BGP Roles/ORF/ASPA, inbound backpressure, IPv6-only peering, BGP-LS reflection, and VPNv4 route-reflection remain catalogued in `INTEROP.md`. |
