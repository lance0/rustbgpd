...
|---------|--------|-------|
| Foundation M-series interop | CI-gated on every PR | `interop.yml` covers the core protocol and management surfaces listed in [`INTEROP.md`](INTEROP.md#ci-coverage), including M73's GoBGP 4.6.0 BGP-LS source -> rustbgpd RR -> GoBGP sink reflection / withdrawal receipt. |
...
Compact M36-M73 index (details and assertions stay in
...
| Other M41-M73 rows in this span | Foundation interop receipts such as BLACKHOLE FIB discard, gRPC/gNMI, EVPN Type 5 injection, BGP Roles/ORF/ASPA, inbound backpressure, IPv6-only peering, and BGP-LS reflection remain catalogued in `INTEROP.md`. |
