...
| Hosted kernel dataplane | CI-gated on PRs, pushes to `main`, nightly, and manual dispatch | Uses GitHub-hosted `ubuntu-latest` runners with privileged containerlab/netns setup. EVPN receipts include M68 for FRR consumption of rustbgpd's native GW-IP overlay-index Type 5, and M70 for ADR-0089 VLAN-aware bridge FDB attribution: FRR originates the same MAC in two traditional VNIs while rustbgpd programs and withdraws only the matching `NDA_VLAN`-scoped row on its `vlan_filtering=1` bridge. TCP-AO topologies are probed and skipped only if the runner kernel lacks support. |
