...
| **EVPN GR / LLGR stale** | Wired end-to-end (RFC 4724 + RFC 9494). `mark_stale_evpn` / `promote_to_llgr_stale_evpn` / EoR clear paths validated by 13 unit + integration tests covering stale marking, LLGR promotion with `LLGR_STALE` community injection via `Arc::make_mut`, peer-originated community preservation, `NO_LLGR` drop on promotion, GR / LLGR timer sweep, and ERR `refresh_stale_evpn` tracking. | Real FRR VTEP flap against a rustbgpd RR with tcpdump validation that reflected routes carry `LLGR_STALE` during the restart window and are withdrawn cleanly on EoR. Requires the Gate 1 harness. |
