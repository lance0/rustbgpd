...
15 of the 40 automated interop milestone scripts are gated on every PR. Per-tier breakdown
...
The other 25 interop tests are gated locally — either because they
...
| FRR (bgpd) | 10.3.1 | `tests/interop/m41-blackhole-frr.clab.yml` | Tested (M41) | RFC 7999 BLACKHOLE receiver scoping | Single-peer EBGP topology. FRR's outbound route-map tags `203.0.113.66/32` with the `BLACKHOLE` community (`65535:666`) and leaves `203.0.113.67/32` untagged. rustbgpd runs with `[global] honor_blackhole = true`; the driver asserts the tagged route in `ListReceivedRoutes` carries both `BLACKHOLE` and `NO_ADVERTISE`, while the untagged route carries neither. This is a control-plane-only smoke per ADR-0060: it does not assert local kernel discard/FIB enforcement. **In CI** on every push and PR via `.github/workflows/interop.yml` (control-plane only, no special kernel features required). |
