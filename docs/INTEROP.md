...
| **EVPN DF election (Gate 8)** | Done (M38 default-modulo, M46 HRW). Two-PE rustbgpd Ethernet Segment exercises RFC 7432 §8.5 default-modulo carving (M38) and RFC 8584 §3.2 Highest Random Weight (M46, over a VNI where the HRW winner differs from the modulo winner so the test proves HRW is in effect), with unanimous-or-default algorithm negotiation; `evpn_df_role` Prometheus gauge + `evpn_df_role_changes_total` counter; reflected Type 4 ES carries ES-Import RT extcomm and Type 1 EAD-per-ES carries ESI Label extcomm (Gate 8b prep). Byte-exact HRW conformance is locked by a known-answer unit test. | Cross-vendor HRW interop is not available (FRR implements RFC 9785 preference-DF, not HRW); the known-answer test is the conformance guarantee against an RFC-conformant peer. `evpn_bum_filter_kernel` continues to cover the BUM-suppression kernel primitive on every PR. |
