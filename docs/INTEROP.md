...
entire lab roster. The classifier is fail-closed: an empty diff, a malformed
path, or any mixed diff runs the labs, and only `pull_request` events can take
the skip path at all — pushes to `main`, the nightly schedule, and manual
dispatch always run.
...
set). Those jobs run on lab-relevant PRs, pushes to `main`, nightly schedule,
and manual dispatch on GitHub-hosted `ubuntu-latest` runners; that workflow
