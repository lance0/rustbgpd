...
- **Protocol resilience + validation** — max-prefix Cease, RTR v2 ASPA
  validation (roleless and role-aware), dynamic-neighbor admission, and RFC 7606
  malformed-UPDATE dispositions: **M26**, **M27**, **M28**, **M59**, **M91**.
...
| In-tree synthetic load | in-tree HEAD | `tests/interop/m32b-evpn-ead-synthetic.clab.yml` | Tested (M32b, manual) | EVPN Type 1 EAD-per-EVI reflection with synthetic ESI (no kernel bond) | Manual-only sibling to M32. An `evpn-tester` injects a 500-route synthetic EAD-per-EVI load and `evpn-monitor` plus gRPC verify reflection; no third-party peer participates. | — |
...
| FRR (bgpd) ×2 | 10.3.1 | `tests/interop/m53-bgp-unnumbered-spike.clab.yml` | Tested (M53 spike, manual) | ADR-0069 BGP unnumbered target-behavior observation | Manual FRR-to-FRR observation only; no rustbgpd participant. The peers establish over IPv6 link-local only, exchange IPv4 routes with `fe80::/10` next hops, and expose RFC 8950 Extended Next Hop. The capture pins a 32-byte IPv6 MP_REACH next hop and no Link-Local Next Hop capability 77; the production rustbgpd↔FRR gate is M53 above. |
