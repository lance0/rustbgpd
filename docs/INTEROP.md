...
| **VTEP Type 5 origination (Gate 9)** | Pure-logic helpers landed in `[Unreleased]` (`ip_vrf::origination` / `ip_vrf::projection`); `[[evpn_ip_vrfs]]` config, readiness probe, and the `probe_ip_vrfs` plumbing through `DataplaneIntent` are in place; M30b validates Type 5 *receive* against FRR (manual-only, hosted Azure runner lacks `vrf` module). | Daemon-side origination wiring + FIB programming + the planned M39 manual containerlab smoke against FRR. `DataplaneReport.ip_vrf_status` rows and `rustbgpctl evpn vrfs` CLI / gRPC RPC for operator visibility. |
