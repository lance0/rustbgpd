...
| GoBGP ×2 + pmacct + gobmp + tshark | GoBGP 3.37.0, pmbmpd 1.8.1-git 20260513 (bleeding-edge, digest-pinned), gobmp master 2026-06-24 (digest-pinned), tshark 4.4.7 | `tests/interop/m81-bmp-trio-gobgp.clab.yml` | Tested (M81) | The BMP trio receipt: RFC 8671 Adj-RIB-Out + RFC 9069 Loc-RIB + BMPv4 TLV framing (draft-ietf-grow-bmp-tlv-21) | rustbgpd streams all three RM views to pmacct and gobmp as independent semantic v3 oracles plus raw v3/v4 sinks under tshark capture. The refreshed byte oracle pins draft-21 BGP Message TLV type 4, index 0, Stats TLV code 1, byte-equal embedded PDUs, reconnect dump/EoR behavior, and absence of ambiguous type-5 Path Marking. Path Marking remains unavailable until its draft receives a non-colliding assignment. **Hosted CI** in `.github/workflows/interop.yml`. | Dedicated images: `docker build -t bmpsink:m81 -f tests/interop/Dockerfile.bmpsink tests/interop`; pmacct/gobmp digest-pinned in the topology |
