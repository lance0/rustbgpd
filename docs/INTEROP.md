...
`run_labs` output. A pull request whose complete three-dot diff is
documentation-only, standalone-fuzz-only, or release/packaging-only skips the
entire lab roster. The classifier defaults to every lane on an empty diff, a
malformed path, or any mixed diff, and only `pull_request` events can take the
skip path at all. Pushes to `main` always run; Kernel Dataplane also supports
manual dispatch. Neither workflow has a scheduled trigger.
...
set). Those jobs run on lab-relevant PRs, pushes to `main`, and manual dispatch
