...
| FRR (bgpd, 3×) | 10.3.1 | `tests/interop/m32-evpn-multihome-frr.clab.yml` | Tested (M32) | EVPN multi-homing Type 4 ES reflection (RFC 7432 §8) | VTEP-A and VTEP-C share an ESI (`evpn mh es-id` + `es-sys-mac` match); VTEP-B observer sees both VTEPs' Type 4 ES routes through the RR with correct RFC 4456 attributes (ORIGINATOR_ID + CLUSTER_LIST). RR does not execute DF election — just reflects inputs. Type 1 EAD-per-EVI presence is advisory: FRR only originates it when the ES is bound to a specific EVI via VLAN-aware bridge + SVI, which the Phase 1 harness does not configure (deferred to Phase 3 / VTEP execution). | — |
...
| **EVPN Type 4 multi-homing reflection** | Done (M32). 4-node containerlab harness: two FRR VTEPs share an ESI (same `evpn mh es-id` + `es-sys-mac`); rustbgpd RR reflects both VTEPs' Type 4 ES routes to a third observing VTEP unchanged with correct RFC 4456 attributes (ORIGINATOR_ID + CLUSTER_LIST). The RR does not run DF election — VTEPs do — and the harness asserts the observer receives the Type 4 ES inputs DF election needs. Type 1 EAD-per-EVI reflection is logged as advisory: FRR only originates EAD-per-EVI when the ES is bound to a specific EVI via VLAN-aware bridge + SVI, which is out of Phase 1 RR scope. | Phase 3 (rustbgpd-as-VTEP) will exercise EAD-per-EVI origination. |
