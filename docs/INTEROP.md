...
- **gRPC/gNMI + EVPN injection** — ADR-0064 mTLS tier enforcement, ADR-0070 gNMI / OpenConfig telemetry, and EVPN Type 5 control-plane injection: **M44**, **M54**, **M45**.
...
**M50**, **M51**, **M52**, **M53**, the conditional **M43** TCP-AO smoke, and the Docker
...
| gnmic (mTLS client) | 0.46.0 | `tests/interop/m54-gnmi-openconfig.clab.yml` | Tested (M54) | ADR-0070 read-only gNMI / OpenConfig telemetry over native mTLS | Single rustbgpd node, no BGP peer — exercises the collector-facing gNMI surface through the real `gnmic` client. The daemon serves `gnmi.gNMI` only on an mTLS-configured TCP listener; the driver presents the operator client certificate generated by `tests/interop/scripts/gen-m54-certs.sh`, verifies `Capabilities` reports gNMI `0.10.0`, the OpenConfig BGP modules, and `JSON` / `JSON_IETF`, then reads the supported OpenConfig BGP global and neighbor `state` paths with `Get` and proves `Subscribe STREAM/SAMPLE` emits repeated snapshots for `global/state/router-id`. **Hosted CI** in `.github/workflows/interop.yml` (no kernel features required). |
| gnmic (mTLS client) + FRR (bgpd) | 0.46.0 / 10.3.1 | `tests/interop/m56-gnmi-onchange-frr.clab.yml` | Tested (M56) | ADR-0070 deferral resolved / ADR-0072 follow-up — gNMI `Subscribe ON_CHANGE` (session-state only) | Two-node topology (rustbgpd ↔ FRR) with `[event_history].enabled = true`. After the peering reaches Established, the driver first asserts that `STREAM ON_CHANGE` on an unsupported leaf (`state/peer-as`) is rejected with `Unimplemented`. It then opens an `ON_CHANGE` subscription on the wildcard `neighbor[neighbor-address=*]/state/session-state` path, observes the initial-snapshot Update carrying `ESTABLISHED` for the configured peer, and `clear bgp` on the FRR side to flap the session — asserting that a fresh transition Update arrives without waiting for any SAMPLE interval and that the post-flap re-`ESTABLISHED` transition also streams. Finally, the driver opens a second subscription and asserts that reconnect yields a fresh initial snapshot (no replay of disconnected-window transitions — the v1 contract is that historical replay belongs to `SubscribeFromEvent`, not gNMI). Reuses the M54 mTLS PKI for the gNMI listener. **Hosted CI** in `.github/workflows/interop.yml` (no kernel features required). |
...
| **VTEP local origination (Gate 7b+1)** | Done (M37). rustbgpd-as-originator + FRR-as-consumer; `bridge fdb add` triggers Type 2 origination, `bridge fdb del` triggers withdrawal, daemon shutdown drains Type 3 IMET before peer down. | Hosted `kernel-dataplane` CI now covers the M37 smoke; the 24 h MAC-churn soak harness at `tests/soak/run-m37-local-origination-churn-soak.sh` is ready but still needs a full run + postmortem to confirm the originator retention model doesn't compound under heavy churn. |
| **VTEP MAC-with-IP origination (Gate 7b+2)** | Done (M37+IP). FRR-style replace model: at most one of MAC-only / MAC+IP advertising for a MAC at any time. Operator prerequisite: `neigh_suppress on` per VXLAN port. | Hosted `kernel-dataplane` CI now covers the M37+IP smoke. |
