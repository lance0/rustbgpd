...
preferred key's secret mismatched and requires fail-closed withdrawal. Live
deprecated/unselected-MKT deletion is implemented, but M43 does not yet drive
that full SIGHUP coordinator path against BIRD; retain it as the next proof
extension before claiming deletion interop.
