...
16 of the 42 automated interop milestone scripts are gated on every PR. Per-tier breakdown
...
The remaining interop scripts are local / manual gates — either
because they still need privileged kernel state not yet assigned to
the self-hosted `kernel-dataplane` runner (M36 / M37 / M37+IP / M38),
substantial wall-clock (M11/M16 GR/LLGR, M33 scale soak), additional
fixtures (StayRTR / mock RTR v2 server), or platform-diversity
validation (BIRD, GoBGP — exercising alternate implementations
without gating every PR). The self-hosted `kernel-dataplane` workflow
now covers the highest-risk kernel dataplane paths: M39, M40, M42,
and the Docker `fdb_nhg` / `fib_runtime` netns selectors.
