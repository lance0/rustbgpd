...
| FRR (bgpd, 3×) | 10.3.1 | `tests/interop/m32-evpn-multihome-frr.clab.yml` | Tested (M32) | EVPN multi-homing Type 1 EAD + Type 4 ES reflection (RFC 7432 §8) | VTEP-A and VTEP-C share an ESI (`evpn mh es-id` + `es-sys-mac` match); VTEP-B observer sees both VTEPs' Type 4 ES routes and Type 1 EAD routes through the RR with correct RFC 4456 attributes. RR does not execute DF election — just reflects inputs. | — |
| In-tree evpn-load (2 testers + 1 monitor) | v0.8.0 | `tests/interop/m33-evpn-scale.clab.yml` | Tested (M33) | EVPN RR scale validation — 50k Type 2 routes + 60 s of 1000/sec churn | 2 synthetic iBGP testers originate 25k Type 2 each via the in-tree `bench/evpn-load` generator; monitor counts reflected UPDATEs and asserts convergence < 60 s + post-churn count = 50,000 (no loss) + gRPC health survives. No third-party daemon in the measurement path. | — |
...
| **EVPN Type 1 + Type 4 multi-homing** | Done (M32). 4-node containerlab harness: two FRR VTEPs share an ESI (same `evpn mh es-id` + `es-sys-mac`); rustbgpd RR reflects both VTEPs' Type 1 EAD-per-ES and Type 4 ES routes to a third observing VTEP unchanged with correct RFC 4456 attributes. The RR does not run DF election — VTEPs do — and the harness asserts the observer receives the inputs DF election needs. | — |
