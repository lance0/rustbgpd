...
| FRR (bgpd) | 10.3.1 | `tests/interop/m41-blackhole-frr.clab.yml` | Prep (M41) | RFC 7999 BLACKHOLE receiver scoping | Single-peer EBGP topology. FRR's outbound route-map tags `203.0.113.66/32` with the `BLACKHOLE` community (`65535:666`) and leaves `203.0.113.67/32` untagged. rustbgpd runs with `[global] honor_blackhole = true`; the driver asserts the tagged route in `ListReceivedRoutes` carries both `BLACKHOLE` and `NO_ADVERTISE`, while the untagged route carries neither. This is a control-plane-only smoke per ADR-0060: it does not assert local kernel discard/FIB enforcement. Manual skeleton only, not in CI. |
