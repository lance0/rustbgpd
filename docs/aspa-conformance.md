...
| 5 | §5.9/§5.10 canonical Prefix PDU network addresses | Implemented | Under negotiated v2, IPv4 and IPv6 Prefix PDUs with nonzero host bits are fatal corrupt data: the client sends Error Report code 0 with the offending frame, closes, flushes that cache's previously learned data, and publishes none of the incomplete transaction. RTR v1 still accepts nonzero host bits; invalid prefix-length and max-length PDUs on either version share the fatal code-0 flush disposition. | Codec boundary tables and end-to-end client test in `crates/rpki/src/rtr_codec.rs` and `crates/rpki/src/rtr_client.rs` |
