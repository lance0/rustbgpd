...
Follow current development priorities and the remaining release gates.
...
[docs/explanation/comparison.md](../explanation/comparison.md) and
[docs/explanation/gobgp-parity.md](../explanation/gobgp-parity.md) — this table does not duplicate
those.
...
| Graceful Restart (4724) + LLGR (9494) + Notification GR (8538) | Partial | GR helper + minimal restarting speaker shipped; optional bounded shutdown checkpoint publication shipped without boot restore/adoption; real-peer receipts cover IPv4 unicast GR/LLGR (M11/M16), IPv4/IPv6 unicast GR (M85), VPNv4/VPNv6/RTC GR+LLGR and BGP-LS GR (M77), and IPv4/IPv6 labeled-unicast GR (M79); IPv6-unicast LLGR, EVPN, FlowSpec, and BGP-LS VPN restart behavior remains internally tested rather than real-peer receipted |
...
build-order history see [docs/project/milestones.md](milestones.md).
...
Completed roadmap records are preserved in
[docs/project/roadmap-history.md](roadmap-history.md).
...
- **Maintain the narrow v1 role contract.** The shipped machine-checked
  ...
  [ADR-0125](../adr/0125-v1-stability-contract.md). Its E2 pair of ≥24 h
  soak receipts is archived — route server
  ([docs/soaks/soak-rs-flagship-24h.md](../soaks/soak-rs-flagship-24h.md))
  and route reflector
  ([docs/soaks/soak-rr-flagship-24h.md](../soaks/soak-rr-flagship-24h.md)) —
...
- **Make changed-policy reload the primary performance program.** The corrected
  ...
  to ~360 ms
  ([the reload-flush envelope receipt](../perf/reload-flush-envelope-2026-07.md)).
  ...
  ([reload-stall-2026-07.md](../perf/reload-stall-2026-07.md)); the
...
- **Keep authoritative replacement readiness design-gated.** A retained
  ...
  remainder after its grouped cohort committed. [ADR-0111](../adr/0111-authoritative-policy-replacement-continuation.md)
  is Rejected after Gate 1 NO-GO, so the accepted
  [ADR-0105](../adr/0105-grouped-export-policy-transition.md) single cohort
  ...
  ([feasibility receipt](../perf/authoritative-policy-replacement-cursor-feasibility-2026-07.md));
...
- **Tighten lifecycle security without reopening scope.** Preserve the shipped
  ...
  dataplane capability profile, stated in `docs/reference/security.md` — not an
...
- **Route-server next-hop ownership strict-peer pilot shipped
  ...
  [ADR-0107](../adr/0107-route-server-next-hop-ownership.md) is Accepted:
  ...
  [ADR-0128](../adr/0128-route-server-next-hop-translation.md) records the
...
- **Single-owner RIB actor ceiling: measured at 1M; sharding stays unjustified.**
  ...
  ([the 1M actor-ceiling receipt](../perf/actor-ceiling-1m-2026-07.md),
...
[ADR-0126](../adr/0126-shared-group-per-client-best.md) (Accepted) landed
...
fleets group instead of running fully ungrouped — byte-identical per-member
views at grouped-class cost, proven by the
[grouped per-client-best receipt](../perf/irr-reload-grouped-per-client-best-2026-08.md)
against the sealed
[realistic-mix baseline](../perf/irr-reload-realistic-mix-2026-08.md).
...
- **Policy / explain follow-ups** *(operator polish, not feature).* Stable
  ...
  `docs/reference/operations.md` "Ingress rejection / route-leak detection").
...
- **Performance — remaining items.** The scale & memory sprint shipped
  ...
  - Adj-RIB-In attribute-intern hashing: **measured NO-GO**
    ([August 2026 receipt](../perf/attr-intern-hashing-2026-08.md)). The
  ...
  - Neighbor RIB snapshot coalescing — thin the memory-for-speed trade
    ...
    [August 2026 receipt](../perf/memory-attribution-2026-08.md).
...
- **TCP-AO remaining rotation polish.** Static TCP-AO + BIRD interop (M43), direct
  ...
  observed peer use. A later SIGHUP can remove only deprecated, unselected MKTs
  while preserving the exact owner set, survivor order, and selected key.
  Edits/reordering, protected-owner CRUD, and per-socket metrics remain
  demand-shaped rather than core-feature blockers. M43 couples the real-kernel
  listener/queued-child deletion receipt with a full SIGHUP deletion against
  BIRD, preserving the established session and the route at every sample from
  a 100 ms polling oracle while authenticated traffic continues on the sole
  surviving MKT. Its separate crash mode restarts from each durable phase
  config (add-only, selection/deprecation `awaiting_peer`, and delete) and
  requires a fresh exact authenticated inventory plus route/session recovery;
  it does not claim persistence across a container or host reboot.
...
- **RFC 9234 (BGP Roles + OTC) follow-ups** *(ADR-0071 counterpart).* None
  ...
  `docs/reference/reload-matrix.md`; in-place re-evaluation needs Route-Refresh +
...
- **[CHANGELOG.md](../../CHANGELOG.md)** — per-release shipped detail and feature
  deltas.
- **[docs/project/roadmap-history.md](roadmap-history.md)** — completed roadmap sections
  retained in their original wording and order.
- **[docs/project/milestones.md](milestones.md)** — build-order history (the
...
- **[docs/explanation/comparison.md](../explanation/comparison.md)** + **[docs/explanation/gobgp-parity.md](../explanation/gobgp-parity.md)**
  — the detailed competitive / parity tables vs FRR, BIRD, GoBGP, OpenBGPD.
- **[docs/project/evpn-enablement.md](evpn-enablement.md)** — the EVPN enablement
  guide. **[docs/how-to/evpn-alpha-soak.md](../how-to/evpn-alpha-soak.md)** —
  EVPN alpha-soak status and remaining follow-ups.
- **[docs/benchmarks.md](../benchmarks.md)** — convergence, CPU, and memory results
