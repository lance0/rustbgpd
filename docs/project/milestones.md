...
- **Still deferred (alpha-soak follow-up):** duplicate-MAC remote-route
  ...
  Tracked in [docs/evpn-alpha-soak.md](../how-to/evpn-alpha-soak.md).
...
| **M36** | Gate 7b downward path — rustbgpd-as-VTEP, FRR-as-originator. Validates kernel FDB program/withdraw via rtnetlink. | `kernel-dataplane` CI (GitHub-hosted); needs `CAP_NET_ADMIN`. Script: `test-m36-evpn-vtep-smoke.sh`. |
| **M37** | Gate 7b+1 upward path — rustbgpd-as-originator, FRR-as-consumer. Validates Type 2 + IMET origination per ADR-0055. | `kernel-dataplane` CI (GitHub-hosted); needs `CAP_NET_ADMIN`. Script: `test-m37-evpn-local-origination.sh`. |
| **M37+IP** | Gate 7b+2 — MAC+IP Type 2 origination via ARP/ND suppression under the FRR-style replace model. | `kernel-dataplane` CI (GitHub-hosted); needs `CAP_NET_ADMIN`. Script: `test-m37-evpn-mac-ip-origination.sh`. |
| **M38** | Gate 8 — observable DF election with two rustbgpd-as-VTEPs sharing an ESI. Validates Type 1/4 origination + the Prometheus `evpn_df_role` surface. | `kernel-dataplane` CI (GitHub-hosted); needs `CAP_NET_ADMIN`. Script: `test-m38-evpn-df-election.sh`. |
...
| **M62** | Durable BLACKHOLE ownership kill-and-restart. A+B are installed and recorded exactly in the private mode-0600 receipt before SIGKILL; foreign `proto static` and unreceipted `proto bgp` BLACKHOLE rows are planted. After B is withdrawn while down, restart adopts only receipt∩marker: A is reclaimed continuously, B is pending then reaped after the deferral, both foreign rows survive without status, counters equal two adoptions/one reap, and the final receipt is exactly A. | `kernel-dataplane` CI (GitHub-hosted). Script: `test-m62-blackhole-adoption-sweep.sh`. |
...
| **M74** | RFC 4364/RFC 4659 VPNv4/VPNv6 (SAFI 128) route-reflection receipt, GoBGP ↔ rustbgpd. Two routes per family reuse the same two RDs across AFIs while carrying distinct prefixes, labels, RTs, and next-hops. Family+peer-scoped rustbgpd API rows and family-scoped sink adj-in prove exact per-key single-path preservation, including RFC 4456 ORIGINATOR_ID/CLUSTER_LIST on that same path. VPNv6 RD1 withdrawal precedes VPNv4 RD1; each leaves exact cross-family RD2 state, and neither inner prefix nor MPLS state reaches the kernel. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m74-vpnv4-reflection-gobgp.sh`. |
| **M75** | RFC 4684 RT-Constrain (SAFI 132) VPNv4 reflection-filtering interop receipt, GoBGP ↔ rustbgpd. A GoBGP source with two VRFs feeds the RR; an RTC sink must receive only its member RT while a non-RTC sink receives everything; the RR's self-originated default RTC NLRI is accepted into GoBGP's adj-in (the wire pin for the #626 empty-AS_PATH fix), membership widen/narrow churns with zero session flaps and no UPDATE storm, and the RR touches no dataplane. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m75-rtc-vpnv4-filter-gobgp.sh`. |
...
| **M77** | GR/LLGR stale preservation for the RR families, GoBGP ↔ rustbgpd. A GoBGP PE carrying VPNv4 + RT-Constrain (per-family GR + LLGR) is daemon-restarted (`gobgpd -r`): the RR retains its VPN and RTC routes as stale through the GR window with zero UPDATEs toward the client, re-establish + EoR converges without churn while a non-readvertised route is swept (RFC 4724 §4.1), the preserved RTC membership serves VPN immediately at re-establish, the LLGR two-phase runs live (GR-expiry promotion, LLGR_STALE 65535:6 observable on the LLGR-capable client wire, LLGR-expiry sweep), a never-returns variant sweeps at GR expiry with client withdraws, and ORR vantages survive a linkstate source restart window. No dataplane writes. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m77-gr-llgr-rr-gobgp.sh`. |
| **M78** | RFC 9107 multi-cluster ORR with inter-RR Add-Path, GoBGP ↔ rustbgpd. Two rustbgpd RRs in different clusters hold a non-client iBGP session negotiating Add-Path both directions; a linkstate GoBGP source feeds both LSDBs, one PE and one vantage-pinned client per cluster. Proves each RR learns the other cluster's path over the inter-RR session with a non-zero Add-Path path_id on the wire, the clients receive divergent topologically-correct bests, a metric flip moves one client onto a path its RR holds only via the other RR (CLUSTER_LIST naming both clusters, zero churn to the unaffected client), withdrawal propagates across the inter-RR session, and neither RR touches any dataplane. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m78-multicluster-orr-gobgp.sh`. |
| **M79** | RFC 8277 labeled-unicast (SAFI 4) route reflection + GR, GoBGP ↔ rustbgpd. A GoBGP PE injects IPv4 + IPv6 labeled-unicast routes — single label, a multi-label stack, and a v6 route with a v6 next-hop over the same v4 session — and the RR reflects them with label stack / next-hop preserved verbatim plus ORIGINATOR_ID / CLUSTER_LIST. Relabel is an implicit replace end to end (one path, new label, no flap); withdrawal uses the RFC 8277 §2.4 compatibility field on the wire; a kill -9 GR window (RFC 4724) holds the labeled routes stale with zero UPDATEs toward the client and the relaunched (`gobgpd -r`) PE's End-of-RIB sweeps exactly the not-re-advertised route. No dataplane writes. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m79-labeled-reflection-gobgp.sh`. |
| **M80** | ADR-0096 `.rpol` policy parity vs FRR route-maps — the policy-language arc closer. rustbgpd runs its whole policy surface from an `.rpol` file (parameterized `customer-in(peer_lp)`, a modify-then-continue scrub term, `edge-out` export policy) while an FRR parity node expresses the same intent as route-maps / prefix-lists / community-lists; both receive one route matrix — IPv4 and IPv6 unicast over the same MP-BGP sessions — from a source FRR and export to a downstream FRR. Proves route-for-route import parity (prefix-set ge/le boundary, community add + remove, LOCAL_PREF set, as-path regex reject, the same parameterized policy instantiated twice with different LP for two peers, `route.origin-as in <asn-set>` vs FRR's anchored as-path regex, and family-branched terms whose same-origin v4/v6 twins diverge purely by `route.family`) and export parity (MED / communities / deny compared per prefix at the downstream, with a family-branched v6 MED), `rbgp policy check` exit 0 on the file's in-language tests (incl. family fixtures), a `rbgp policy test` live-RIB dry run over both families (counts + per-term hits + diffs), nonzero `rbgp policy stats` live term hits in both directions (`--direction import` incl. install generation), import explain naming the deciding source term and export explain proving chain-default Permit attribution with exact modifications, and an `.rpol` edit under traffic: SIGHUP hot-apply with zero session flap and a wire-observed Route Refresh at exactly the one peer whose resolved chain changed. No dataplane writes (v4 or v6). | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m80-rpol-policy-parity-frr.sh`. |
| **M81** | The BMP trio + BMPv4 receipt — the ADR-0097 arc closer. Two GoBGP clients and independent pmacct/gobmp v3 oracles prove rib-in-pre, rib-out-post, and loc-rib behavior, while raw v3/v4 sinks pin framing and byte equality. Dedicated PE1 IPv4/IPv6 policy rejects pin exact RFC 9972 type-22 family rows. A digest-pinned StayRTR fixture installs Valid and Invalid VRPs beside a NotFound route and proves exact `(Invalid, Valid, NotFound) = (1, 1, 1)` type-35/36/37 rows for each negotiated IPv4/IPv6-unicast family with byte-identical v3/v4 Stats bodies. Unit tests separately pin per-path Add-Path counting, omission before VRP authority, and authoritative negotiated zero rows. The draft-21 refresh asserts BGP Message TLV type 4 index 0, Stats TLV code 1, reconnect dump plus four End-of-RIB markers, and no ambiguous type-5 Path Marking pending a non-colliding assignment. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m81-bmp-trio-gobgp.sh`. |
| **M82** | ADR-0092 EVPN VLAN-Aware Bundle (non-zero Ethernet Tag) route reflection — the Decision 6 proof ladder, both legs. **Synthetic leg (CI):** GoBGP originates the RFC 7432 §6.3 bundle shape — one EVI (RD 65000:100, RT 65000:100), Ethernet Tags 10/20 with per-tag VNIs 10010/10020, the same MAC under both tags — and rustbgpd keys the tags as route identity (two distinct Type 2 entries for one (RD, MAC), per-tag IMET entries), surfaces them via `rbgp evpn`, reflects tag-verbatim NLRIs (RD/etag/MAC/IP/label field-equal on the sink's re-decoded NLRI) with ORIGINATOR_ID + CLUSTER_LIST, and a tag-10 withdraw removes exactly that entry. 26/26. **Vendor leg (local lab):** Nokia SR Linux 25.10.1 in VLAN-aware-bundle interoperability mode (`vlan-aware-bundle-eth-tag 10/20` on two mac-vrfs, shared RT, the same static MAC in both bridge domains) originates real vendor non-zero-tag Type 2 + IMET routes; same assertion classes, 20/20 — **rustbgpd's first vendor-NOS interop receipt**. SR Linux quirks recorded: one EVI per mac-vrf (bundle identity = shared RT + tag, per-BD RDs) and mandatory explicit `transport local-address`. | Synthetic leg: hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`, script `test-m82-evpn-bundle-synthetic-gobgp.sh`. Vendor leg: local containerlab (`m82-evpn-bundle-srlinux.clab.yml`), script `test-m82-evpn-bundle-srlinux.sh`. |
| **M83** | RFC 7947 route-server profile multi-stack receipt — the ADR-0101 proof-ladder closer. rustbgpd is the route server (AS 65500) for BIRD 2.19.2 + GoBGP 4.8.0 (Add-Path receive) + FRR 10.7.0 members with a dedicated reduced IPv4 lab hygiene/ROV projection and a StayRTR RTR fixture. RFC 6598 and TEST-NET-3 remain usable only as deterministic fixture prefixes; M83 does not exercise the public example's dated dual-stack special-purpose snapshot. 50 assertions: RFC 7947 §2.2 transparency proven on all three client views and at byte level (tshark on the RS↔BIRD link — no 65500 in any AS_PATH segment, NEXT_HOP and unfiltered MED/standard/Large Communities verbatim); a BIRD-only `discard_path_attributes = [4]` boundary proven by a pre-SIGHUP raw UPDATE with MED 120 + community 65001:111, normalized RIB and GoBGP/FRR downstream views without MED but with standard and Large Communities preserved, and a positive `bgp_path_attribute_discarded_total{peer="10.83.1.2",type_code="4"}` delta; RFC 9234 OTC toward members (wire attribute 35); per-member export policy views with `rbgp rib advertised` agreement; ROV reject-at-import with `rbgp policy explain` naming the deciding term; the RFC 7947 §2.3 path-hiding contrast — single-best member receives NOTHING (pinned), the same member receives the runner-up after a live `per_client_best` flip with the explain ladder naming candidate 1 of 2 denied, and the Add-Path member holds both candidate paths; runner-up withdraw convergence; EoR-after-flood ordering; withdraw propagation; session stability through a policy SIGHUP. Caught a real product bug pre-CI: `per_client_best` was dropped at the `build_transport_config` seam — every configured peer silently stayed single-best; fixed in the same change with a unit pin. First-AS relaxation per stack documented (FRR per-neighbor `no enforce-first-as`; BIRD `enforce first as off`; GoBGP none), and `bird -p` gates BIRD startup. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m83-routeserver-multistack.sh`. |
| **M84** | Multi-cache RTR/ASPA epoch conformance — the interop receipt for the 8210bis per-cache `CacheEpoch {version, session_id, serial}` model. rustbgpd runs three RTR caches at once: Routinator 0.15.2 (real RTR v2, offline and deterministic via `--no-rir-tals` + SLURM local assertions), StayRTR 0.6.4 (real RTR v1-only), and the in-repo phased Python RTR v2 server (the only deterministic ASPA source — StayRTR removed ASPA support and Routinator 0.15 silently ignores SLURM `aspaAssertions`). An FRR 10.3.1 peer advertises probe prefixes so each cache's data is observable as a route validation state. 37/37: per-cache full-table load at validated End of Data with exact merged `bgp_rpki_vrp_count` / `bgp_aspa_records`; the shipped v2→v1 fallback against StayRTR's version-0 Error Report; a `docker restart` of Routinator rotating the server session — the client's stale Serial Query draws a Cache Reset and it resynchronizes onto the new session's EoD while a 0.5 s poller proves zero validation flap; incremental epoch advance plus ASPA provider-set replacement (valid → invalid) and empty-provider withdrawal (`aspaState = unknown`) with zero full-table resyncs; and a same-session serial regression rejected at EoD and resolved by exactly one Reset Query resync that purges the stale VRP and leaves the other caches' data intact. Restarting the v1-fallback cache is covered too: the client treats the Error Report code 0 as a Cache Reset, resynchronizes at v1 without re-probing v2, and a 0.5 s poller proves the stayrtr-backed route never leaves `valid`. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m84-rtr-multicache.sh`; both cache images are digest-pinned and the retry harness deploys a fresh lab per attempt. |
| **M87** | Exact-export rejection at the classic BGP message ceiling, GoBGP 4.6 -> rustbgpd route server -> BIRD 2.0.12. A legal 4,095-byte source UPDATE carries exactly 337 Large Communities; sink-only policy adds one more, producing a 4,107-byte exact candidate for BIRD's 4,096-byte session. Proves one `message_too_long` rejection, withdrawal of the independently observed prior route, rejected Adj-RIB-Out/explain state, recovery on a small replacement, source withdrawal, and BIRD-session continuity. Mutation proofs remove the owed-withdrawal insertion and the exact size guard; each turns the receipt red. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m87-exact-export-rejection.sh`. |
| **M89** | Experimental Paths-Limit real-FRR receipt (IANA-assigned capability 76 from expired `draft-abraitis-idr-addpath-paths-limit-04`). Three digest-pinned FRR 10.3.1 source ASes each originate the same IPv4 and IPv6 prefix through a rustbgpd route server to a real FRR sink. Proves exactly three source candidates per family before limit claims; typed FRR capability state 2/7 and 3/7; exact rustbgpd configured/advertised/received/effective values; exact Adj-RIB-Out and independent sink counts 2/3; nonzero unique Add-Path receive IDs; exact source-AS membership; and four stable sessions. Mutations that bypass the family lookup or force IPv4 lookup make the v4/v6 exact-count assertions red. The experimental capability remains outside the v1 contract. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`, sharing M17's build/job. Script: `test-m89-paths-limit-frr.sh`. |
| **M90** | ADR-0110 route-server filtering differential. One pinned arouteserver site input renders the BIRD 2.0.12 side through arouteserver 1.23.2 proper and the rustbgpd 0.60.0 side through `rs-config-render`; three GoBGP 3.37.0 members inject 11 announcements spanning IRR authorization, the peering-LAN blacklist, bogons, prefix-length windows, and never-via-route-server ASNs. The lab proves 11/11 identical accept/reject verdicts, exact rustbgpd reject-reason retention and generated policy/term attribution, exact generated per-client max-prefix ceilings, generated-config and `.rpol` gates, and all six sessions Established before and after the verdict sweep. A rust-only context mutation authorizing AS64501's otherwise-rejected `198.51.100.0/24` makes the differential exit 1 while a separate query proves BIRD still rejects that member path; suppressing max-prefix emission fails before daemon start. The final restored run is 65/65. | Local containerlab (`m90-differential.clab.yml`), script `test-m90-differential.sh`; immutable run identities and fixture hashes are in the lab README. |
...
| **M93** | Required-family OPEN enforcement against BIRD 2.19.2: exact missing-IPv6 OPEN 2/7 bytes, explicit dual-stack restart success, then cleared-requirement IPv4-only compatibility. | Hosted CI beside M85 (`m93-required-families-bird.clab.yml`, `test-m93-required-families-bird.sh`). |
...
| **M99** | RFC 9072 extended OPEN against pinned FRR 10.3.1. Two links isolate FRR's forced-small extended form from a classic control; an independent raw-TCP oracle pins rustbgpd's maximum production 348-byte OPEN with 313 capability octets and its exact 49-byte classic OPEN, exact parameter consumption, one OPEN per direction, common capabilities, and zero NOTIFICATIONs. | Hosted CI (`m99-rfc9072-extended-open-frr.clab.yml`, `test-m99-rfc9072-extended-open.sh`); host tshark captures bytes but does not decode BGP. |
| **M100** | Released-daemon Partial-flag receiver differential. Exact `0xa0` MED, ORIGINATOR_ID, CLUSTER_LIST, MP_REACH, and MP_UNREACH attributes cross four isolated receivers and observer sessions, freezing 20 outcomes plus candidate/survivor state. Accepted MED stays 100; ORIGINATOR_ID/CLUSTER_LIST do not cross the observer boundary; every reset carries exact UPDATE `3/4` offending bytes and an ordered close/next-epoch re-establishment. No production, configuration, or default behavior changes. | Hosted CI (`m100-partial-receiver.clab.yml`, `test-m100-partial-receiver.sh`) against rustbgpd 0.67.0, BIRD 2.19.2, OpenBGPD 9.2, and FRR 10.3.1; exact images/source and successful-run evidence are required. |
| **M101** | Real BIRD 3.3.2 malformed-attribute proof through a three-node IPv4-unicast route server. Both checksum-built BIRD and digest-pinned FRR 10.3.1 are sleeping, image-ID/runtime/config preflighted before capture and daemon start. The exact 27/0 receipt proves BIRD emits optional-transitive-partial type 40 as `e0 28 01 00`; rustbgpd attribute-discards it while retaining the route and unrelated communities through post-policy Adj-RIB-In to FRR; exact malformed disposition deltas are +1/+0/+0; import and member-scoped export denies have positive controls and explain/advertised proof; withdrawal converges with both sessions Established and no flap delta. It claims no Prefix-SID, labeled-unicast, SR, or AS_SET breadth. | Hosted CI (`m101-routeserver-bird332.clab.yml`, `test-m101-routeserver-bird332.sh`); BIRD source SHA-256 and the current FRR digest are fail-closed. |
| **M102** | OpenBGPD 9.2 dual-stack route-server member proof. Sleeping digest-pinned OpenBGPD and FRR peers are identity/runtime/config preflighted before sidecar capture and daemon start. Exact 32/0 proves enforced role/AS4/both families, bidirectional transparent AS_PATH and communities, independently reassembled AS_TRANS/capability-65 and exact decoded IPv4 UPDATE fields (AS_PATH, NEXT_HOP, standard and Large Communities, and NLRI), explicit import/export policy, four directional-family withdrawals, and no-flap continuity. Partial and AS_SET behavior are excluded. | Hosted CI (`m102-routeserver-openbgpd92.clab.yml`, `test-m102-routeserver-openbgpd92.sh`); index, amd64 leaf, config, and both peer identities fail closed. |
| **M103** | GoBGP 4.8.0 incumbent vs rustbgpd dual-stack route-server differential, as a sibling that leaves M92 immutable. The exact 56/0 normal proof repeats fresh baseline, one-line export mutant, and byte-identical restore; a separate 17/0 GR-disabled negative retains all routes but withholds both incumbent EoRs and refuses capture/diff. All three GoBGP identities and the five current M92-derived config hashes are preflighted before daemon start. Live 4.8 raw output must equal archived 4.7 after deleting only recursive `age`; the separately versioned golden retains byte-identical route/trailer records. | Hosted CI (`m103-gobgp-v48-rs-differential.clab.yml`, `test-m103-gobgp-v48-rs-differential.sh`), normal plus `M103_COMPLETENESS_NEGATIVE`, both single-attempt. |
| **M104** | Current-daemon sibling of the immutable M90 ARouteServer filtering differential. It reuses the exact M90 site, contexts, three member configs, and 11-route corpus read-only; requires the unchanged context-ingestion proof at 23/23; then requires 74/0 live checks across four accepts and seven rejects, exact current rustbgpd policy/term attribution (including `reject-irrdb-prefix-filtered`), exact BIRD cause communities, and all six sessions surviving. Runtime boundaries fail closed on the ARouteServer 1.23.2 manifest/config/package, staged BIRD 2.19.2 source plus runtime and `bird -p`, GoBGP 4.8 archive/binaries/image, and the checked-out rustbgpd SHA/image. | Hosted CI (`m104-arouteserver-current-rs-differential.clab.yml`, `test-m104-arouteserver-current-rs-differential.sh`), single-attempt; no M90 artifact is rewritten. |
| **M108** | RFC 5883 multihop BFD over routed /32 loopbacks against FRR `bfdd`. Validates BGP and BFD Up from both sides, forwarding-loss Down with Cease/BFD Down coupling, and recovery. | `kernel-dataplane` CI (GitHub-hosted). Script: `test-m108-bfd-multihop-frr.sh`. |
| **M111** | SRv6 VPNv4/VPNv6 service reflection from FRR 10.7.1 through rustbgpd to GoBGP 3.37.0. Independent packet decoding proves receive capabilities, exact service/NLRI/IPv6-next-hop preservation, expected RR attributes, SID transposition against FRR allocation, withdrawals and session continuity. A fresh typed-view run also ties CLI service fields and labels to the packet and allocation evidence. | Local/manual [procedure and receipts](../../tests/interop/m111-srv6-l3vpn-reflection/README.md), with offline replay in CI. Script: `test-m111-srv6-l3vpn-reflection.sh`; no forwarding or EVPN L2 service claim. A [rerun with semantic eligibility](../artifacts/interop/m111-srv6-eligibility-20260906T200426Z/README.md) preserves both valid transposed services. |
| **M112** | Controlled-source SRv6 L2 reflection: MAC-only Type 2, TLV6 / End.DT2U, exact announced service bytes, typed RR visibility, malformed-input withdrawal and recovery with a survivor. Withdrawals retain route-key identity and sessions remain established. | Local/manual [procedure and receipt](../../tests/interop/m112-srv6-l2-reflection/README.md); offline negative controls and receipt replays in CI. No forwarding or complete EVPN-over-SRv6 claim. The [semantic replacement receipt](../artifacts/interop/m112-srv6-semantic-20260906T200333Z/README.md) proves retained invalid bytes and typed fields, selection/export exclusion, observer withdrawal, and recovery without survivor churn. |
...
- **RFC 7999 BLACKHOLE receiver + opt-in FIB discard** (v0.21.0) — natural
  ...
  counters. M41 is CI-gated against FRR 10.3.1; receipt-backed startup adoption
  is shipped. V1 declines a second FIB allow-list in favor of accepted import
  and peer-group policy. Remaining work is active/rate limits, richer audit,
  and outbound advertise (gRPC `SetBlackhole { peer, prefix, enabled }` or
