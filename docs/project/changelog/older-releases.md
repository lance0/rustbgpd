...
- Effective-config rendering now materializes defaults and redaction on a
  sentinel-only skeleton and splices source statements in bounded borrowed chunks.
...
- The pre-IR legacy policy evaluator (`evaluate_with_attribution_legacy`) and
  ...
  `crates/policy/src/engine/tests/golden_corpus.txt` — and the corpus test now
  pins the IR evaluator against the frozen goldens. The corpus is never
  regenerated by tooling; any change is a hand-edited, reviewed commit.
  (LAN-965)
- `rbgp evpn list` and `rbgp bfd list` removed; the bare `rbgp evpn` /
  `rbgp bfd` forms are unchanged and take the same flags. (LAN-967)
...
- SIGINT, SIGTERM, and SIGHUP handlers are now registered before gRPC, BGP,
  ...
  of taking the default process action in that window (LAN-949).
...
- **The v0.61.0 release tip now has an exact, durable performance baseline.**
  ...
  [`docs/perf/v0.61.0-final-performance-2026-07.md`](../../perf/v0.61.0-final-performance-2026-07.md).
...
- **Wire-codec temporary allocation churn is lower on the measured attribute
  ...
  [`docs/perf/wire-codec-allocation-2026-07.md`](../../perf/wire-codec-allocation-2026-07.md).
...
- **Revised UPDATE decoding no longer allocates a duplicate-type set per
  ...
  production UPDATE-body timing band is separated from a biased same-revision
  control,
  establishing a fixture-scoped speedup without publishing the raw -21.73% as
  a causal percentage. See
  [`docs/perf/revised-update-duplicate-table-2026-07.md`](../../perf/revised-update-duplicate-table-2026-07.md).
...
- **Grouped route-server fanout avoids two peer-multiplied bookkeeping
  ...
  [`docs/perf/fanout-metrics-handle-2026-07.md`](../../perf/fanout-metrics-handle-2026-07.md)
  and
  [`docs/perf/otc-pristine-reconcile-2026-07.md`](../../perf/otc-pristine-reconcile-2026-07.md).
...
- **RFC 8654 receive buffers now grow on demand.** Negotiating the 65,535-byte
  ...
  [`docs/perf/per-peer-rss-attribution-2026-07.md`](../../perf/per-peer-rss-attribution-2026-07.md).
...
- **A peer with no rejected routes no longer allocates its rejected-route
  ...
  [DHAT capture](../../perf/explain-cache-opt-in-2026-07.md#dhat-attribution)
...
- Grouped outbound prefix-limit admission sets now store family-typed IPv4
  ...
  [`docs/perf/outbound-prefix-limit-admission-compaction-2026-07.md`](../../perf/outbound-prefix-limit-admission-compaction-2026-07.md).
...
- **Update groups: shared outbound staging for the RR fanout
  ...
  [1000-peer RR scale receipt](../../perf/scale-receipt-2026-07.md)
...
- **Update groups v2: VPNv4/VPNv6 join the shared staging, with the
  ...
  3-run medians; [scale receipt](../../perf/scale-receipt-2026-07.md)
...
- **Gate 9 slice 6 24h Type 5 churn soak harness in-tree**
  ...
  [`docs/soaks/soak-gate9-slice6-24h-symmetric-irb.md`](../../soaks/soak-gate9-slice6-24h-symmetric-irb.md)
...
- **[ADR-0059](../../adr/0059-evpn-aliasing-fdb-nexthop-groups.md)
...
- **BMP drop / replay Prometheus counters.** Four new `bmp_*_total`
  ...
  Closes the [BMP drop/replay counters](../../reference/known-issues.md) gap.
...
- **Per-peer outbound writer task; eliminates `GetHealth` wedge under
  ...
  the M33 1h soak. See [ADR-0051](../../adr/0051-per-peer-outbound-writer-task.md).
...
- **EVPN Route Reflector (Phase 1, RFC 7432).** L2VPN/EVPN address family
  ...
  FRR 10.3.1. See [ADR-0050](../../adr/0050-evpn-route-reflector.md).
