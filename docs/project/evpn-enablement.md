...
record, [gobgp-parity.md](../explanation/gobgp-parity.md) for the cross-daemon comparison.
...
3. **Single-active whole-port AC gate** — the RFC 7432 §14.1.1
   ...
   `docs/how-to/evpn-vtep-troubleshooting.md` for the
...
- Auto-derived Route Targets for both `[[evpn_instances]]` and
  ...
  See `docs/reference/configuration.md` for the full interop matrix.
...
| RFC 9721 extended IRB mobility | Partial: a local bridge-port move of a MAC advertised as MAC+IP re-advertises every (MAC, IP) Type 2 for that MAC at the incremented MAC Mobility sequence (§5.1 parent/child, §6.2 inheritance), counted as one duplicate-MAC move per MAC event; the MAC-only and per-(MAC, IP) mobility ratchets otherwise stay independent. A Type 2 from a PE on the VNI's own non-zero Ethernet Segment is a peer-sync route, excluded from mobility contention and both duplicate detectors. For a locally learned MAC, an eligible peer sequence is adopted exactly without adding one, and local MAC/IP children synchronize to the highest peer or retained local sequence (§6.4/§6.5, partial). Adoption requires a configured import RT match, VNI, tag zero, and a nonlocal next hop, works in either arrival order, and preserves sticky state and lifecycle suppression | Remaining: the §6.1 third rule (a local MAC-IP learn must exceed a different remote MAC holding the same IP), Peer-Sync-Local origination of a MAC or MAC/IP learned only from the ES peer (§6.4/§6.5), full §8.2 duplicate-address procedures, and §6.8 probing. Optional per-IP M/N accounting now detects local/local and local/remote ownership conflicts through counters and warnings only; no IP quarantine or probing §7/§8.3 RT-5 mobility is not applicable: Type 5 routes carry no MAC Mobility extended community (RFC 9136 defines none; the Type 5 originator emits ORIGIN, AS_PATH, and extended communities only, and the receive-side mobility comparison is gated to route type 2); mobility reaches Type 5 only through GW-IP overlay-index resolution, which selects the highest-sequence Type 2 and fails closed when distinct MACs tie | Pure originator unit tests plus daemon originator and RIB event tests; no interop leg yet |
...
- Auto-derivation of Route Targets (landed later as
  ...
  `[[evpn_ip_vrfs]]`; see [CONFIGURATION.md](../reference/configuration.md))
- PBB-EVPN (RFC 7623)
- Multicast EVPN / MVPN (including RFC 9251 IGMP/MLD proxy routes)
- MPLS encapsulation
- `match_evpn_route_type` policy clause (shipped in v0.11.0 — see
  [CONFIGURATION.md — Match conditions](../reference/configuration.md#match-conditions))
...
- **docs/project/roadmap.md** — EVPN rows in the status-at-a-glance table and the "EVPN standards tail" item
- **docs/interop.md** — P1.5 "EVPN validation depth" gap list
- **docs/reference/rfc-notes.md** — RFC 7432 / 9012 / 9135 implementation notes
- **docs/explanation/use-cases.md §7** — "VXLAN-EVPN DC Fabric Route Reflector"
- **docs/explanation/gobgp-parity.md** — DC fabric RR section + feature-parity matrix
