...
- [`CONFIGURATION.md`](../reference/configuration.md) — every config field, with examples.
- [`reload-matrix.md`](../reference/reload-matrix.md) — which fields hot-apply vs. need a restart.
- [`OPERATIONS.md`](../reference/operations.md) — debugging, log filtering, failure modes.
- [`SECURITY.md`](../reference/security.md) — firewalling, gRPC mTLS, authorization tiers.
...
- `ExecReload=kill -HUP` is the supported reload path. See the
  [reload matrix](../reference/reload-matrix.md) for which fields hot-apply vs.
  need a restart.
...
5. **Edit + reload cycle.** Edit a copy of the config, then dry-run
   ...
   against the [reload matrix](../reference/reload-matrix.md) to confirm which
...
events. See [`CONFIGURATION.md`](../reference/configuration.md#bmp) for the schema.
...
local-only extension. RFC 7951 JSON encoding. See [`GNMI.md`](../reference/gnmi.md)
...
2. Extract the package into a temporary directory and run the **new** binary's
   ...
   [`CONFIGURATION.md`](../reference/configuration.md) → `config_epoch`); stop or quiesce
...
[`CONFIGURATION.md`](../reference/configuration.md) → "Config Persistence").
...
See [`SECURITY.md`](../reference/security.md) for the full posture document. The
...
- **gRPC.** Use mTLS in production. Set `[security.grpc] enforcement = "tier"`
  ...
  [`SECURITY.md`](../reference/security.md); the per-method tier matrix itself is
...
without a restart (see the [reload matrix](../reference/reload-matrix.md)).

## Related

- [`OPERATIONS.md`](../reference/operations.md) — start/stop, reload, upgrade, debugging.
- [`CONFIGURATION.md`](../reference/configuration.md) — config reference.
- [`reload-matrix.md`](../reference/reload-matrix.md) — per-field reload classification.
- [`SECURITY.md`](../reference/security.md) — security posture + firewall guidance.
