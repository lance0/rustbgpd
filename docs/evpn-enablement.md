...
| Runtime mutation surface for the startup-pinned EVPN model — ADR-0063 rejects a direct `ArcSwap` / `RwLock` table swap as the first implementation. The foundation exposes the committed generation through `GetEvpnRuntime` / `rustbgpctl evpn runtime`, includes the coordinator core / commit gate, and now wires `EvpnService.ApplyEvpnRuntime` to daemon-owned candidate parsing, full EVPN table validation, plan summaries, validate-only mode, no-op apply, and fail-closed non-noop rejection without generation advance. The Type 3 IMET path now has a daemon-owned controller with per-VNI originate/withdraw methods while preserving startup/shutdown behavior. Remaining daemon actor convergence still needs to wire the coordinator through Type 2, Type 5, DF/ES, and Linux owned state before live edits can commit. | `crates/evpn/src/runtime.rs`, `crates/api/src/evpn_service.rs`, `src/main.rs`, `src/evpn_imet.rs` | public API contract landed; first IMET convergence primitive landed; remaining actor convergence tracked in [#210](https://github.com/lance0/rustbgpd/issues/210) |
