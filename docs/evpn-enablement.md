...
| ADR-0090 all-active ESI overlay-index Type 5 receive | Projection + L3 ownership substrate landed; writer pending | Extend the M71 shape to all-active ESI recursion with deterministic remote-VTEP target sets, route-level ECMP, per-VTEP L3 neighbors, and L3VXLAN FDB-NHG for the shared Router MAC. The model now distinguishes single-active, all-active, and conflicting EAD redundancy signals, and the Linux dataplane has partitioned L3 NHID / Router-MAC FDB-NHG ownership state, but production import still fails closed until the writer lands | M72 real-peer proof: unresolved before EAD, then VRF ECMP route + `nhid` Router-MAC FDB row, then deterministic withdraw/collapse cleanup |
