...
- **Gate 8/8b** adds active-active multi-homing alpha execution:
  ...
  **Gate 9** ships symmetric
  ...
  VXLAN creation (managed bridge, fixed-VNI VXLAN, VRF, and L3VXLAN lifecycle
  has shipped).
  ...
  fixed-VNI VXLAN, VRF, and L3VXLAN creation now ship under ADR-0091's
  altname-stamp ownership model; SVD / collect-metadata VXLAN lifecycle stays
  fail-closed until its own ownership/lifecycle proof lands.
...
| rustbgpd-managed bridge / VXLAN / VRF netdev creation | Demand-shaped operator ergonomics; boundary accepted in ADR-0088 and ADR-0091; bridge, fixed-VNI VXLAN, VRF, and L3VXLAN lifecycle landed (`[managed_netdevs]`, derived altname stamps, `EvpnService.ListManagedNetdevs`, `rbgp evpn managed-netdevs`, create -> stamp -> fresh-dump confirm, crash-restart adoption, safe same-owner reap, foreign/unsafe preservation). Fixed-VNI VXLAN rows create traditional one-VNI VXLAN devices on the desired bridge and fail closed on SVD/collect-metadata, `vnifilter`, learning, bridge-attachment, or protected-attribute drift. VRF/L3VXLAN rows create the IP-VRF kernel topology, fail closed on protected-attribute drift, refuse VRF deletion while slave links remain, and cross-validate matching `[[evpn_ip_vrfs]]` table/L3VNI/local/Router-MAC identity | SVD/collect-metadata VXLAN creation and VLAN-upper creation remain separate class-specific gates | Unit + actor tests and hosted/gated local kernel netns tests `managed_bridge`, `managed_vxlan`, `managed_ready`, and `managed_ip_vrf_ready`; the readiness proofs show rustbgpd-created L2 and IP-VRF topologies drive the real probes from NotReady to Ready on a real kernel |
