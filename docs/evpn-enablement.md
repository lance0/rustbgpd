...
| rustbgpd-managed bridge / VXLAN / VRF netdev creation | Demand-shaped operator ergonomics; boundary accepted in ADR-0088 and ADR-0091; bridge-class config/status substrate landed (`[managed_netdevs]`, derived altname stamp, `EvpnService.ListManagedNetdevs`, `rbgp evpn managed-netdevs`) | Add bridge lifecycle execution first: create → stamp → fresh-dump confirm, crash-restart adoption, safe reap, foreign-state preservation. VXLAN and VRF classes remain deferred gates | Kernel unit tests plus deployment doc receipt |
