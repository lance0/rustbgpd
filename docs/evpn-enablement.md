...
| Runtime mutation surface for the startup-pinned EVPN model — ADR-0063 rejects a direct `ArcSwap` / `RwLock` table swap as the first implementation. The foundation now exposes the committed generation through `GetEvpnRuntime` / `rustbgpctl evpn runtime` and includes the coordinator core / commit gate that accepts fully validated candidates, plans add/delete/redefine work, and only publishes after an injected convergence backend accepts the plan. Public gRPC mutation and daemon actor convergence still need to wire that coordinator through IMET, Type 2, Type 5, DF/ES, and Linux owned state. | `crates/evpn/src/runtime.rs`, `crates/api/src/evpn_service.rs`, daemon wiring | coordinator core landed; public live mutation remains tracked in [#210](https://github.com/lance0/rustbgpd/issues/210) |
