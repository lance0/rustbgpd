...
| rustbgpd-managed bridge / VXLAN / VRF netdev creation | Demand-shaped operator ergonomics; boundary accepted in ADR-0088 and ADR-0091; bridge-class lifecycle landed (`[managed_netdevs]`, derived altname stamp, `EvpnService.ListManagedNetdevs`, `rbgp evpn managed-netdevs`, create → stamp → fresh-dump confirm, crash-restart adoption, safe same-owner reap, foreign/unsafe preservation). Fixed-VNI VXLAN schema/status substrate also landed: desired rows, derived `rustbgpd:vxlan:<owner>:<name>` stamps, named VXLAN link parsing, protected-attribute status, and orphan/foreign/unsafe reporting with no kernel mutation | Add fixed-VNI VXLAN create/adopt/reap lifecycle, then VRF/L3VXLAN classes. SVD/collect-metadata VXLAN creation and VLAN-upper creation remain separate class-specific gates | Unit + actor tests and hosted/gated local kernel netns test `managed_bridge`; fixed-VNI VXLAN status is covered by config/API/CLI/link-parser/reconcile unit tests |
