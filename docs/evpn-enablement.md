...
| Runtime mutation surface for the startup-pinned EVPN model — ADR-0063 rejects a direct `ArcSwap` / `RwLock` table swap as the first implementation. The foundation exposes the committed generation through `GetEvpnRuntime` / `rustbgpctl evpn runtime`, includes the coordinator core / commit gate, and wires `EvpnService.ApplyEvpnRuntime` to daemon-owned candidate parsing, full EVPN table validation, plan summaries, validate-only mode, and no-op apply. A daemon actor converger now commits five live shapes: a **single L2VNI add** (IMET originate + effective-table republish to the dataplane supervisor, Type 2 MAC/MAC+IP originator, and SVI task), a **single standalone L2VNI delete** in L2-only deployments (effective-table republish to the dataplane supervisor, Type 2/SVI drain, and IMET withdraw), a **single IP-VRF add** (effective-table republish to the dataplane supervisor and Type 5 originator), a **single standalone IP-VRF delete** when no L2VNI references it (effective-table republish to the dataplane supervisor and Type 5 originator), and a **single Ethernet Segment add** (full desired-ES snapshot republished to the segment actor, which drains/rebuilds Type 4 / EAD-per-ES / EAD-per-EVI / BUM state). Convergence is ordered with rollback, and the originator / SVI / Type 5 actors drain removed-or-redefined VNIs/IP-VRFs (including stale duplicate-MAC move-window state where applicable) before accepting a new model. The segment actor's instance view is startup-pinned, so an ES whose member VNI was added at runtime is rejected (restart-required), not silently dropped. Other non-noop shapes — linked L2VNI/IP-VRF delete, ES delete, redefine, mixed L2VNI + IP-VRF, multi-add/delete, or an add/delete on an RR-only / no-actor daemon — still fail closed with `FAILED_PRECONDITION` without advancing or degrading the committed generation. | `crates/evpn/src/runtime.rs`, `crates/api/src/evpn_service.rs`, `src/main.rs`, `src/evpn_imet.rs`, `src/evpn_originator.rs`, `src/evpn_svi.rs`, `src/evpn_l3_originator.rs`, `src/evpn_dataplane.rs`, `src/evpn_segment.rs` | single L2VNI add/delete + IP-VRF add/delete + ES add convergence landed; redefine / linked delete / mixed / multi-add + runtime-added-VNI ES convergence tracked in [#210](https://github.com/lance0/rustbgpd/issues/210) |
...
- Runtime instance mutation completion (ADR-0063 / #210): single L2VNI add,
  single standalone L2VNI delete, single IP-VRF add/delete, and single
  Ethernet Segment add commit live via `ApplyEvpnRuntime`; linked delete,
  redefine, mixed / multi-element edits, and an ES referencing a runtime-added
  member VNI (segment actor is startup-pinned) remain restart-required.
