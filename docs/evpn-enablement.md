...
| RFC 9721 extended IRB mobility | Partial: a local bridge-port move of a MAC advertised as MAC+IP re-advertises every (MAC, IP) Type 2 for that MAC at the incremented MAC Mobility sequence (§5.1 parent/child, §6.2 inheritance), counted as one duplicate-MAC move per MAC event; the MAC-only and per-(MAC, IP) ratchets stay independent | Remaining: the §6.1 third rule (a local MAC-IP learn must exceed a different remote MAC holding the same IP), §6.4/§6.5 Peer-Sync-Local, §8.2 duplicate-IP detection, §7/§8.3 RT-5 mobility, and §6.8 probing | Pure originator unit tests plus daemon originator and RIB event tests; no interop leg yet |
