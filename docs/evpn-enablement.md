...
| VLAN-aware bridges | Demand-shaped Linux/VXLAN operability; ADR-0088 boundary accepted; ADR-0089 v1 scope accepted; read-only substrate landed | Next: replace today's `NotReady` guard for VNI-per-broadcast-domain service only after explicit `bridge_vlan` / VNI binding and per-VLAN FDB/neighbor attribution exist; Ethernet Tag ID stays `0` | Local kernel dataplane test plus one M-series smoke |
