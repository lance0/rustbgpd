...
- **Gate 8/8b** adds active-active multi-homing alpha execution:
  ...
  **Gate 9** ships symmetric
  ...
  gaps such as bridge-ifindex MAC+IP VLAN correlation and true shared-VNI /
  non-zero Ethernet Tag service — for which ADR-0092 is Accepted and the
  RR-side receive/reflect half is already CI-gated by M82, leaving
  origination, import selection, and dataplane bundle mode.
  ADR-0091 managed bridge, fixed-VNI VXLAN,
  SVD / collect-metadata VXLAN, VLAN upper, VRF, and L3VXLAN lifecycle have
  shipped.
  ...
  because Linux does not report bridge VLAN identity there. Managed bridge,
  fixed-VNI VXLAN, SVD / collect-metadata VXLAN, VLAN upper, VRF, and L3VXLAN
  creation now ship under ADR-0091's altname-stamp ownership model.
...
| Mutation surface — whole-model `EvpnService.ApplyEvpnRuntime` plus SIGHUP reload (ADR-0063); single L2VNI/IP-VRF/Ethernet-Segment add/delete/redefine, additive build-up, atomic tenant teardown, `ip_vrf` relink, and decomposable mixed edits ordered as deletes -> redefines -> `ip_vrf` relinks -> adds commit live; L3VNI/device/table identity changes remain restart-required by design | `crates/api/src/evpn_service.rs`, `src/main.rs`, `src/reload.rs`, `src/evpn_runtime_converger.rs`, `src/evpn_segment.rs` |
...
on the Linux softswitch (ADR-0065; re-confirmed as a negative feasibility
result by ADR-0094's 2-PE `l2_miss` spike, which closed A1-adjacent
non-DF filtering but not A2 local-bias).
...
- Runtime instance mutation bounds (ADR-0063 / #268): single L2VNI add/delete/
  ...
  decomposable mixed edits ordered as deletes -> redefines -> `ip_vrf` relinks ->
  adds commit live via `ApplyEvpnRuntime` and SIGHUP reload. L3VNI/device/table
  IP-VRF identity changes remain restart-required by design; unsupported
  dependency cycles fail closed before commit and later convergence failures
  fail-stop on the last committed generation.
...
| VLAN-aware bridges | Demand-shaped Linux/VXLAN operability; ADR-0088 boundary accepted; ADR-0089 v1 VNI-per-broadcast-domain slice landed for traditional multi-VXLAN bridges and SVD / collect-metadata VXLAN: `bridge_vlan` schema/status, observed VLAN topology validation, `NDA_VLAN` remote-MAC FDB attribution for fixed-VNI devices, `NDA_SRC_VNI` attribution for SVD devices, AF_BRIDGE local-MAC VLAN attribution, VLAN-upper AF_INET / AF_INET6 MAC+IP attribution, M70 FRR interop proving same-MAC two-VNI isolation on a traditional rustbgpd-owned `vlan_filtering=1` bridge, `dataplane_vlan_fdb` + `macip_vlan_attribution` proving real-kernel VLAN-scoped FDB and MAC+IP isolation, and `svd_fdb_vni` proving SVD Ready + add + same-MAC two-VNI isolation + scoped delete on a real kernel. Ethernet Tag ID stays `0`; unattributable VLAN observations fail closed as normal "not ours" classifier outcomes, downstream observation backpressure is metered, and startup link-cache/probe priming bounds the boot window | Raw bridge-ifindex ARP/ND on `vlan_filtering=1` bridges remains fail-closed unless the ADR-0093 FDB-correlation design (Proposed) proves freshness and ambiguity handling; true VLAN-aware bundle / non-zero Ethernet Tag is covered by ADR-0092 (Accepted) — its RR reflect/receive slice is CI-gated by the M82 GoBGP-synthetic leg, with the Nokia SR Linux 25.10.1 vendor leg local-only, while origination, import selection, and dataplane bundle mode remain unimplemented tranches; managed netdev creation stays a separate ergonomics track | Unit tests, hosted/gated local kernel netns tests including `dataplane_vlan_fdb`, `macip_vlan_attribution`, and `svd_fdb_vni`, and hosted M70 FRR containerlab receipt |
| rustbgpd-managed bridge / VXLAN / SVD / VLAN upper / VRF netdev creation | Demand-shaped operator ergonomics; boundary accepted in ADR-0088 and ADR-0091; bridge, fixed-VNI VXLAN, SVD / collect-metadata VXLAN, VLAN upper, VRF, and L3VXLAN lifecycle landed (`[managed_netdevs]`, derived altname stamps, `EvpnService.ListManagedNetdevs`, `rbgp evpn managed-netdevs`, create -> stamp -> fresh-dump confirm, crash-restart adoption, safe same-owner reap, foreign/unsafe preservation). Fixed-VNI VXLAN rows create traditional one-VNI VXLAN devices on the desired bridge and fail closed on SVD/collect-metadata, `vnifilter`, learning, bridge-attachment, or protected-attribute drift. SVD rows create one external / `vnifilter` / `nolearning` VXLAN per bridge, derive bridge-VLAN -> VNI tunnel mappings from configured `[[evpn_instances]] bridge` / `bridge_vlan` pairs, reject ambiguous bindings (a `bridge_vlan` mapping to more than one VNI) at config validation, report protected-attribute drift on an owned SVD as owned-unsafe, and reap exact same-owner SVD orphans. VLAN upper rows create `ip link add link BR name BR.VLAN type vlan id VID` helpers bound to configured `[[evpn_instances]] bridge` / `bridge_vlan` pairs, and reap before their parent bridge. VRF/L3VXLAN rows create the IP-VRF kernel topology, fail closed on protected-attribute drift, refuse VRF deletion while slave links remain, and cross-validate matching `[[evpn_ip_vrfs]]` table/L3VNI/local/Router-MAC identity | Managed-netdev creation is complete for the ADR-0091 classes; future work is class-specific only if a new link class is accepted | Unit + actor tests and hosted/gated local kernel netns tests `managed_bridge`, `managed_vxlan`, `managed_svd_vxlan`, `managed_vlan_upper`, `managed_ready`, and `managed_ip_vrf_ready`; the readiness proofs show rustbgpd-created L2, SVD, and IP-VRF topologies drive the real probes from NotReady to Ready on a real kernel |
| RFC 9721 extended IRB mobility | Partial: a local bridge-port move of a MAC advertised as MAC+IP re-advertises every (MAC, IP) Type 2 for that MAC at the incremented MAC Mobility sequence (§5.1 parent/child, §6.2 inheritance), counted as one duplicate-MAC move per MAC event; the MAC-only and per-(MAC, IP) ratchets stay independent. A Type 2 from a PE on the VNI's own non-zero Ethernet Segment is a peer-sync route, not a mobility contender: it neither raises the local sequence nor counts as a duplicate-MAC move (§6.4/§6.5, partial) | Remaining: the §6.1 third rule (a local MAC-IP learn must exceed a different remote MAC holding the same IP), Peer-Sync-Local origination of a MAC or MAC/IP learned only from the ES peer (§6.4/§6.5), §8.2 duplicate-IP detection, and §6.8 probing. §7/§8.3 RT-5 mobility is not applicable: Type 5 routes carry no MAC Mobility extended community (RFC 9136 defines none; the Type 5 originator emits ORIGIN, AS_PATH, and extended communities only, and the receive-side mobility comparison is gated to route type 2); mobility reaches Type 5 only through GW-IP overlay-index resolution, which selects the highest-sequence Type 2 and fails closed when distinct MACs tie | Pure originator unit tests plus daemon originator and RIB event tests; no interop leg yet |
