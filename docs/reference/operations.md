...
profiles), see [deployment.md](../how-to/deployment.md).
...
deadline; see [deployment.md](../how-to/deployment.md#systemd).
...
workflow table in [deployment.md](../how-to/deployment.md) for the full exit-code
contract.
...
[package upgrade and rollback procedure](../how-to/deployment.md#package-upgrade-and-rollback).
...
| `bgp_rib_actor_work_duration_seconds{work_unit}` | Wall-clock duration of one indivisible unit of RIB actor ingest work. The actor serves the readiness lane only between units, so a unit above a probe deadline is what a readiness timeout waits on. The closed `work_unit` set is `route_chunk` (one bounded route chunk, excluding the distribution its drain triggers), `distribute_flush` (the coalesced outbound pass a drained route batch performs across the whole peer set — bounded by changed prefixes times peers, not by a chunk count), and `exact_export_retire` (retiring exact-export rejections after that pass). Compare the `le="0.2"` bucket against the series count to read the share of units that outlast a 200 ms readiness deadline |
| `bgp_rib_readiness_query_wait_seconds{seam}` | Wall-clock delay between a readiness query being enqueued on the dedicated RIB readiness lane and the actor serving it. The closed `seam` set is `actor_loop` (the ordinary ingest-loop drain) and `policy_transition_fence` (the checkpoint drain that keeps the lane live while an export-policy transition fences every other lane). This is the readiness latency a `/readyz` deadline actually observes; `bgp_rib_actor_work_duration_seconds` attributes it to a unit |
...
[explain.md](../how-to/explain.md); the sections here carry the full semantics.
...
> validation. See
> [`evpn-enablement.md`](../project/evpn-enablement.md) for the gate ladder,
> [`evpn-alpha-soak.md`](../how-to/evpn-alpha-soak.md) for the residual
> alpha-confidence checklist, and
> [`evpn-vtep-troubleshooting.md`](../how-to/evpn-vtep-troubleshooting.md) for
...
[docs/how-to/evpn-vtep-troubleshooting.md](../how-to/evpn-vtep-troubleshooting.md).
