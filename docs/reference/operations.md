...
| `bgp_peer_manager_operator_query_wait_seconds{seam}` | Wall-clock delay from send on the peer manager's operator-read lane (neighbor snapshots, policy stats, dataset status) until actor service. The closed `seam` set names the policy-transaction wait that held the read: `unfenced` (served by the normal loop with no transaction ahead of it), `prestage`, `forward_transition`, `commit_batches`, and `rollback`. Records service even after a caller times out, so a `rbgp policy stats` deadline during a reload shows up under the seam that caused it; reads never drained contribute no sample. Excludes prior peer-manager work and reply delivery. The 0.1 s, 0.5 s, and 2 s bucket edges are the per-peer, explain, and aggregate caller budgets, so the over-budget share is `count - bucket{le="2"}` |
