...
| 9 | ORIGINATOR_ID | optional non-transitive; flags `0x80`; RFC 7606 §7.9 | typed round-trip; from an external neighbor attribute-discard when well formed, on a length error, or with only Partial set, before the route is stored and selected, and the reflection-loop check does not act on it; a wrong Optional/Transitive flag class is treat-as-withdraw from any neighbor; malformed from iBGP is treat-as-withdraw | core matrix below |
| 10 | CLUSTER_LIST | optional non-transitive; flags `0x80`; RFC 7606 §7.10 | typed round-trip; from an external neighbor attribute-discard when well formed, on a length error, or with only Partial set, before the route is stored and selected, and the reflection-loop check does not act on it; a wrong Optional/Transitive flag class is treat-as-withdraw from any neighbor; malformed from iBGP is treat-as-withdraw | core matrix below |
