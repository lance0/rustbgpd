...
| 9 | ORIGINATOR_ID | optional non-transitive; flags `0x80`; RFC 7606 §7.9 | typed round-trip; from an external neighbor always attribute-discard, well formed or malformed, before the reflection-loop check, import policy, and the RIB; malformed from iBGP is treat-as-withdraw | core matrix below |
| 10 | CLUSTER_LIST | optional non-transitive; flags `0x80`; RFC 7606 §7.10 | typed round-trip; from an external neighbor always attribute-discard, well formed or malformed, before the reflection-loop check, import policy, and the RIB; malformed from iBGP is treat-as-withdraw | core matrix below |
