...
[docs/reference/limitations.md](limitations.md) the boundaries and non-goals.
...
- Full knob semantics, what counts as explicit policy, and the observability
  ...
  [docs/reference/configuration.md](configuration.md) under
...
- **Partially shipped (tracked in ADR-0055 §9 +
  `docs/how-to/evpn-alpha-soak.md`):** RFC 7432 §15.1 duplicate-MAC M/N
...
| RFC 9721 (Apr 2025; extends the RFC 7432 and RFC 9135 IRB procedures) | Partial | Extended IRB mobility. Implemented: a local bridge-port move of a MAC advertised as MAC+IP re-advertises every (MAC, IP) Type 2 for that MAC with the MAC Mobility sequence incremented (§5.1 parent/child, §6.2 inheritance; `LocalMacIpOriginator::on_local_mac_moved` in `crates/evpn/src/origination_macip.rs`). The MAC-only and per-(MAC, IP) ratchets stay independent per RFC 7432 §15.1, so this is a bump-all rather than a shared parent attribute. §6.4/§6.5 peer-sync, partial: a Type 2 received from a PE on the VNI's own non-zero Ethernet Segment is a peer-sync route, not a mobility contender — it neither raises the local MAC Mobility sequence nor counts as a duplicate-MAC move (`is_same_segment_peer` in `crates/evpn/src/origination.rs`, applied where the daemon builds the remote contender views in `src/evpn_originator/rib_polling.rs`); originating a MAC or MAC/IP learned only from the ES peer (Peer-Sync-Local) is not implemented. Not applicable: §7/§8.3 RT-5 mobility — Type 5 routes carry no MAC Mobility extended community (RFC 9136 defines none for the route type; `crates/evpn/src/ip_vrf/origination.rs` builds ORIGIN, AS_PATH, and extended communities only, and the receive-side mobility comparison in `crates/rib/src/loc_rib.rs` is gated to route type 2). Mobility reaches Type 5 only through GW-IP overlay-index resolution, which already selects the highest-sequence Type 2 and fails closed when distinct MACs tie at the top sequence. Absent: the §6.1 third rule (a local MAC-IP learn must exceed the sequence of a different remote MAC holding the same IP), full §8.2 duplicate-address procedures, and §6.8 probing. Optional §8.2 detect-only IP accounting is implemented: conflicting local/local or local/remote MAC ownership uses a per-(VNI, IP) M/N window, excludes sticky and duplicate-MAC-quarantined contenders, and reports counters/warnings without suppression or sequence changes. |
