...
> Docker, containerlab quick-start), see [`deployment.md`](../how-to/deployment.md).
...
4. **FDB nexthop group programming** (ADR-0059, slices 1-4 on
   ...
   raw-netlink primitive (`rtnetlink 0.23` /
   `netlink-packet-route 0.33` still expose no typed nexthop
...
[deployment.md](../how-to/deployment.md#kernel-dataplane-opt-in-privilege-drop-in)):
...
- A separate in-daemon durable audit sink for `grpc_authz` records (file /
  ...
  [`docs/reference/operations.md`](operations.md#grpc-authorization-audit-and-resource-guardrails);
