...
Check known issues and their current workarounds.
...
- **Single task per peer (resolved).** The peer session task no longer
  ...
  back-pressure can no longer park the session's `select!`. Pending
  outbound envelopes now pause at writer capacity; sustained admission
  stalls expire with `Cease/8` (Out of Resources) and a clean BGP session
  restart instead of silent drops. The original writer split was validated by the
...
- **Family scope is still limited.** MP-BGP supports AFI/SAFI negotiation,
  ...
  RT-Constrain (AFI 1, SAFI 132) per RFC 4684 (strict per-peer VPN and
  EVPN reflection filtering with self-originated default membership; §3.2(ii)
