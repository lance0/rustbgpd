...
Sessions service import-counter and neighbor-state queries during grouped
unicast output, including waits for shared encoded chunks. Earlier queued
session commands retain their ordering, so a pending policy change can still
delay a read. Eligible reads need not wait for the complete shared-output
envelope; other backend work still consumes the same two-second statistics
deadline.
