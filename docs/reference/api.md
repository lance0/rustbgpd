...
| `SetGlobalImportChain` / `SetGlobalExportChain` | Replace global chain assignment. Sessions that are positively down (Idle/Connect/Active) accept the new chain in memory and install it (plus RIB outbound registration) at `PeerUp`; ambiguous sessions (`SessionGone` / state-query timeout) fail closed with `INTERNAL` and full compensation. IPv4/IPv6-unicast Route Refresh still requires an Established session with Route Refresh negotiated          |
