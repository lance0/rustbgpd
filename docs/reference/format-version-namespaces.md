...
| Config history | v2 payload rows (`VERSION = 2`, [`src/config_history/v2.rs`](../../src/config_history/v2.rs)) and v3 metadata-only rows (`version: 3`, [`src/config_history/v3.rs`](../../src/config_history/v3.rs)) | Current JSON history codecs, sharing one newest-first twenty-row sequence. v2 carries normalized TOML (up to 10 MiB) and is rollback-capable; v3 records hashes, byte count, reason, and a redacted summary for larger configs and is permanently rollback-ineligible. Retired TOML history is ignored and retained. A `v4-` or higher row from a newer release lists as unreadable, keeps its sequence and roster slot, and stops this build from recording history until it is moved aside. | [ADR-0122](../adr/0122-compatibility-debt-inventory.md) row D1, [ADR-0124](../adr/0124-bounded-config-history-retention.md) |
...
| Event history store | `metadata.schema_version = 1` in `events.db` ([`crates/event-history/src/migrations.rs`](../../crates/event-history/src/migrations.rs)) | SQLite schema of the durable event store. No migrations exist yet, so only schema 1 opens. A store written with a newer schema is left in place: it is not migrated, quarantined or written to, although the version check runs on a connection that has already applied the store's pragmas (WAL journal mode), so SQLite may create its `-wal`/`-shm` files. With event history enabled, the daemon then exits when `[event_history].required = true` and otherwise runs live-only without durable history; with `enabled = false` the store is never opened, so `required` has no effect. A store whose content is corrupt (a non-SQLite file or malformed metadata) is quarantined as `events.db.stale`; one the host will not let the daemon open or write (permissions, a full or read-only filesystem, I/O errors, locks) stays in place and follows `required` the same way. | [ADR-0072](../adr/0072-durable-event-history.md) |
| BLACKHOLE ownership receipt | `schema = "rustbgpd.blackhole-owned/v1"` in `blackhole-owned.json` ([`src/blackhole/owned_state.rs`](../../src/blackhole/owned_state.rs)) | Exact-prefix authority for BLACKHOLE discard routes. The reader accepts only this schema string and rejects unknown fields; any load error leaves the file untouched and disables BLACKHOLE kernel mutations for the run. | [ADR-0060](../adr/0060-rfc-7999-blackhole.md), [ADR-0079](../adr/0079-kernel-state-crash-restart-reconciliation.md) |
| Warm checkpoint bundle | `format_version = 2` in `warm-bundle-v1/manifest.json` ([`crates/mrt/src/warm_bundle.rs`](../../crates/mrt/src/warm_bundle.rs)) | MRT encoding contract of the optional shutdown checkpoint. The reader requires the exact version. Startup never restores the bundle; with checkpointing enabled, a manifest of another version makes startup skip stale-entry cleanup with a warning, and shutdown publication stays available. | [ADR-0104](../adr/0104-shutdown-warm-checkpoint-publication.md), [ADR-0106](../adr/0106-warm-checkpoint-restore-decision.md) |
...
- **Refuse to write past a newer format** (config history): an older build
  lists a newer release's history row as unreadable and skips recording
  (with a warning) while it exists, so it never reuses that row's sequence or
  evicts it.
...
- **Exact version** (streamed config frame): ingress frames must carry exactly
  ...
  inventory.
- **Refuse newer, leave intact** (event history store, BLACKHOLE receipt):
  a store or receipt this build cannot read stays on disk, unmigrated and
  unquarantined, and its contents are not rewritten. The daemon runs
  without the feature it backs, or, with event history enabled and
  `[event_history].required = true`, refuses to start.
