...
| `send_hold_time` | live (effective next session) | RFC 9687 send hold timer. On SIGHUP it is classified as a runtime-neighbor change, so the reconciler rebuilds the session immediately; the replacement per-peer writer captures the new value (including 0 = disable) when its TCP connection is established. |
...
| `route_reflector_client` | live (effective next session) | RFC 4456 RR-client flag affects iBGP best-path + reflection behavior. Registered with the RIB manager at session establishment; the running session's Adj-RIB-Out is not re-evaluated in place. On SIGHUP the reconciler rebuilds the session: the session drops, the routes learned from the peer are withdrawn (unless Graceful Restart retention holds them stale) and re-learned, and the peer is re-advertised under the new flag once the replacement session establishes. Annotated "session reset: session re-establish" by `rustbgpd --diff`. |
...
| `dynamic_neighbor_limit` | reload-applied | Admission cap compared live against the dynamic-peer counter (inbound admission and LRU eviction bound); nothing is pre-allocated. SIGHUP updates the admission ceiling immediately; existing dynamic sessions are preserved even if the new limit is below current occupancy, and headroom metrics update on the snapshot swap. |
...
| `[policy.datasets]` | live | SIGHUP stages content for generation settlement through the stable live handles and refreshes referencing peers. Content-equal re-reads are no-ops; malformed input rejects before publication. Adding, removing, or re-mapping an entry (a member's datasets, say) rides in the same generation: a new dataset must load cleanly or the candidate is rejected at load, a removed one must no longer be declared, and a re-mapped one is a content change through its existing handle. Compensation restores prior contents at a new generation and the prior binding set with the prior config; see [dataset settlement](operations.md#configuration-reload-sighup). |
...
v1 pins every field as restart-required and surfaces the change as an
`ERROR`-level log line during reload.
