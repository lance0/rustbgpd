...
**Machine-readable export:** `docs/reference/grpc-method-inventory.json`.
...
2. **Principal role.** The call's authenticated principal is looked
   ...
   under the pre-1.0 alpha stability posture — see `docs/reference/api.md` for the
...
reads, and `docs/reference/grpc-method-inventory.json` is the machine-readable
export.
...
`docs/reference/grpc-method-inventory.json`; CI checks that JSON artifact against the
...
| `ReplayOutbound` | `mutating` | Schedules one Established peer's negotiated IPv4/IPv6 unicast replay with terminal UPDATE EoRs. The reply confirms scheduling only; terminal BMP EoRs mark completion. Outside the v1 contract. |
...
static Rust table. `docs/reference/grpc-method-inventory.json` is the
...
3. Add the corresponding row to `docs/reference/grpc-method-inventory.json` and bump
