...
> **Receipt status: corrected historical run, durable rerun pending.** The
> unique-prefix harness was run with alternating A/B generation markers after
> its occurrence-counting defect was fixed, and it produced the figures
> preserved below. The raw harness stream, daemon run-window log, generated
> inputs, and exact host/source provenance were
> not retained together, however. The result is therefore useful historical
> diagnosis, but the `< 1 s` gate is **not current acceptance evidence**. The
> revision-pinned runner and fail-closed validator in
> `bench/scale/reloadstall/` must produce a complete checksummed bundle before
> this status changes.

## Corrected historical headline (not acceptance evidence)
...
**Historical result: the corrected run indicated that the < 1 s stall gate
held, but marginally (0.82 s worst case), while full repropagation of the new
policy took minutes rather than seconds.** Its live observations reported
continuous churn delivery, no session flap, and successful control queries,
and the corrected in-memory gate required all 399,828 non-self prefixes at all
700 observers. Those claims now define what the retained rerun must prove from
durable artifacts; the unarchived console/log inputs do not by themselves
satisfy acceptance.
...
evidence for two independent correctness reasons. Both implementation defects
were resolved before the corrected historical run:
...
1. **Completion over-counting (harness).** The original harness treated
   ...
   after each 20-second quiesce. The retained harness emits completion and
   maximum-gap metrics only after that check, using the final authoritative
   completion timestamp; a late revoke followed by an active reassertion
   therefore extends both windows. The historical result predates this stronger
...
The corrected run landed where the withdrawn run did and used an alternating-
marker completion criterion whose per-cycle unique-prefix count cannot be
advanced by duplicates, plus a daemon that cannot silently drop the remainder
of an oversized IPv4-unicast group. Its missing durable evidence bundle is a
separate receipt-integrity gap, now enforced by
the retained runner and validator.
...
- 4 consecutive reloads (A→B, B→A, A→B, B→A) on a hot, converged daemon, 20 s
  quiesce between. The retained acceptance contract verifies the exact current
  community marker for every expected non-self prefix at every observer after
  quiesce (65500:2000 after odd reloads, 65500:1000 after even ones).
...
- The pending retained rerun additionally keeps current per-prefix state through
  the entire quiesce interval. An inactive or markerless replacement revokes
  completion, and any later active reassertion moves the completion timestamp
  used for both completion and maximum-gap statistics.
...
## Reproduction and durable acceptance
...
The only acceptance command is the revision-pinned wrapper below. Run it from
the exact clean commit under test and choose a path that does not exist yet:
