# Grouped per-client best-path IRR reload 2026-08 — compact evidence extract

Compact extract from the eight verified sealed artifact roots behind
...
all at one clean commit) — the ADR-0126 Phase 4 acceptance campaign.
The full roots (multi-MB daemon logs, RSS streams, received views,
per-cell manifests, quiet samples, seals), the four halted
defect-arc roots, the superseded green roots, and the pre-flip
confirmation roots are retained read-only off-repo in the campaign
artifact archive; this directory carries the independent verifier's
output, the received-view byte-identity digests, and the digests
needed to recognize the sealed originals (listed in the receipt's
provenance section).

| File | Source | Contents |
|---|---|---|
| `ov10-verification.json` / `ov50-verification.json` | `verify-receipt.py campaigns` `--output-dir` | verdict per overlap point: `status: "pass"`, 24 comparison rows + 8 grouped-control rows, commit + dataset binding, validated root order, post-flip topology gates, embedded received-view delta |
...
| `received-view-byte-identity.tsv` | `cmp` + `sha256sum` over the sealed roots | per-repeat digest of each comparison root's `rustbgpd-sighup/received-view.tsv`, identical to the sealed realistic-mix baseline capture at the same overlap point |
