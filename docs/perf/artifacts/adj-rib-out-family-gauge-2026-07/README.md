...
- `paired.csv`: exact means and confidence bounds from two additional retained
  short pairs at 256 and 1,000 peers. `delta_percent` is
...
Criterion estimates are nanoseconds. The CSV values are copied without
rounding from each retained `estimates.json` and `change/estimates.json`.
