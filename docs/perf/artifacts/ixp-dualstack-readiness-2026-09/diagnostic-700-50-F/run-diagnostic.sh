#!/usr/bin/env bash
# Usage: run-diagnostic.sh NEW_OUTPUT DIAGNOSTIC_BUILD_RECEIPT PASSED_NORMAL_200_OUTPUT
# Capacity characterization only; this is not production acceptance.
# This wrapper never builds. It uses the unchanged 700-peer fixture and gate.
set -euo pipefail
umask 022
ulimit -n 65536
repo=/tmp/readiness-source
prep=/tmp/readiness-acceptance-prep
[[ $# == 3 ]] || { echo 'see run-diagnostic.sh usage' >&2; exit 2; }
peers=700
total=400400
changed=600
ipv4=200200
...
[[ $(cat "$build/build-mode") == diagnostic ]]
cd "$repo"
...
[[ $(cat "$previous/build-receipt/build-mode") == normal ]]
for exit_file in driver.exit gate.exit provenance.exit; do
    [[ $(cat "$previous/$exit_file") == 0 ]]
done
# The diagnostic feature intentionally changes binary identity. Source and HEAD
# must still match the successful normal 200-peer receipt exactly.
...
cp "$prep/gate.py" "$prep/run-diagnostic.sh" "$prep/snapshot.py" "$out/"
cp -r "$build" "$out/build-receipt"
cp "$build/binaries.sha256" "$out/"
printf '%s\n' "$peers" >"$out/peers"
printf '%s\n' 'diagnostic-capacity-characterization' >"$out/mode"
printf '%s\n' "$previous" >"$out/previous-200"
