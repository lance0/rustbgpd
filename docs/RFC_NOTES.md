...
| RFC 9721 (Apr 2025; extends the RFC 7432 and RFC 9135 IRB procedures) | Partial | Extended IRB mobility. Implemented: a local bridge-port move of a MAC advertised as MAC+IP re-advertises every (MAC, IP) Type 2 for that MAC with the MAC Mobility sequence incremented (§5.1 parent/child, §6.2 inheritance; `LocalMacIpOriginator::on_local_mac_moved` in `crates/evpn/src/origination_macip.rs`). The MAC-only and per-(MAC, IP) ratchets stay independent per RFC 7432 §15.1, so this is a bump-all rather than a shared parent attribute. Absent: the §6.1 third rule (a local MAC-IP learn must exceed the sequence of a different remote MAC holding the same IP), the §6.4/§6.5 Peer-Sync-Local procedures, §8.2 duplicate-IP detection, §7/§8.3 RT-5 mobility, and §6.8 probing. |
