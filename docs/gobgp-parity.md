...
4. ~~**Multi-homing Type 4 ES reflection**~~ — shipped (Gate 4, M32, 2026-04-24). 4-node topology with two FRR VTEPs sharing an Ethernet Segment validates Type 4 ES reflection (with correct ORIGINATOR_ID + CLUSTER_LIST) through the rustbgpd RR. Type 1 EAD-per-EVI reflection is advisory in this harness because FRR origination requires a VLAN-aware bridge + SVI sub-interface that's out of Phase 1 scope; Phase 3 (rustbgpd-as-VTEP) covers EAD-per-EVI.
