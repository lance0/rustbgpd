...
| VPN FlowSpec | Yes | No | |
| BGP-LS (RFC 9552) | Yes | Partial | ADR-0077 receive/API slice negotiates BGP-LS / BGP-LS VPN, stores opaque RFC 9552 NLRI/TLV objects, and exposes them through `ListBgpLsRoutes` / `rbgp rib bgpls`; outbound reflection/export and local IGP topology production remain deferred |
| SR Policy | Yes | No | |
| SRv6 MUP | Yes | No | |
