...
- EVPN RR + bidirectional VTEP shipped (Phase 1 ADR-0050; Phase 2 ADR-0052/0054/0055/0056 — MAC-only Type 2 + Type 3 IMET in v0.15.0, MAC+IP Type 2 via ARP/ND suppression in [Unreleased] under the FRR replace model). Gate 8/8b adds alpha multi-homing execution: DF election, Type 1/4 origination, opt-in BUM suppression, ESI-aware Type 2 origination, aliasing projection, and receive-side mass-withdraw filtering. IRB / L3VNI / Type 5 dataplane, aliasing dataplane ECMP, production-default multi-homing enforcement after soak, duplicate-MAC quarantine action, and Route Types 6-9 still missing
...
2. ~~**EVPN RR + bidirectional VTEP (RFC 7432)**~~ — Phase 1 RR role shipped; **Phase 2 bidirectional VTEP shipped across Gates 7a/7b/7b+1/7b+2/7c, ADR-0052/0054/0055/0056** with kernel FDB program/learn loops, MAC-only + MAC+IP local origination under the FRR-style replace model, sticky-MAC config, and sub-second mobility convergence. Gate 8/8b adds alpha multi-homing execution; symmetric IRB (RFC 9135, Gate 9), aliasing dataplane ECMP, production-default multi-homing enforcement after soak, and duplicate-MAC quarantine action (ADR-0055 §9) are later phases
