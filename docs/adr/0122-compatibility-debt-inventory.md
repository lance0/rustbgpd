...
|---|------|-------------------|-----------------|----------------|-------------------|
| C1 | `GrpcEnforcementConfig::Legacy` enum variant (`src/config/schema.rs`). Its runtime path was removed in v0.63.0 (ADR-0064 status addendum). | The typed enum used to keep `enforcement = "legacy"` parseable solely for migration guidance. | Dead variant and validation branch. | removed (v0.65) | Typed config now accepts only `tier`. After typed failure, the loader matches only the exact semantic retired path/value and returns the paste-ready migration message; comments, unrelated strings/paths, malformed TOML, and other enum values keep ordinary diagnostics. The JSON schema and only the `GrpcEnforcementConfig` blessed digest were updated. |
...
| # | Item | What it preserves | Cost of keeping | Classification | Removal mechanics |
|---|------|-------------------|-----------------|----------------|-------------------|
| L1 | Hidden `--from-file` compatibility aliases on `config diff` / `config plan` / `config apply` (`crates/cli/src/main.rs`), kept when file arguments became positional. The conventions comment pledged they "must never be removed" in the same file. | Old scripts invoking the pre-positional flag spelling. | Three hidden args + `candidate_file()` merge helper; near zero. The real debt was the "never" pledge, which contradicted the alpha posture. | removed (v0.65) | The three hidden args and helper were deleted, first-party callers and checked-in shell completions were migrated, and the CHANGELOG names the positional `CANDIDATE` spelling. The unrelated JSON CRUD `--from-file` flags remain. |
| L2 | Visible aliases `--peer` (for `--neighbor`) and `--asn` (for `--remote-asn`) throughout `crates/cli/src/main.rs`. | Both product-language spellings, by design (CLI conventions block). | Zero; clap renders them in help. | keep | Canonical convention, not a shim. |
| L3 | `rbgp rib diff` older-daemon fallbacks from #1367: single-peer tolerance for daemons that omitted `page_version` and bare-MED fallback/caveat machinery. | Version-less legacy path for ≤v0.62 daemons. | A fallback state machine and caveat surface. | removed (v0.65) | Every live page must carry one unchanged `page_version`; missing values return daemon-upgrade guidance. `med_attr` alone is authoritative, so absent and explicit zero remain distinct. BIRD, GoBGP, and MRT producers preserve explicit MED 0; FRR omission remains documented because its source conflates zero with absence. |
| L4 | CLI rolling-upgrade fallback for an absent additive field: the `neighbor_source_label` "range unavailable" arm (`crates/cli/src/output.rs`). | `rbgp` from HEAD reading a previous-minor daemon that predates `accepted_dynamic_range`. | One small, individually tested branch. | keep (policy-bounded) | The max-prefix action absence derivation was removed for v0.65; HEAD now trusts the authoritative `NeighborState.max_prefix_action`. The remaining range-unavailable branch is governed by the N-1 rule and is prunable once its emitting release is two minors old; no new per-branch decision is needed. |

#### API surface (protobuf / metrics)

| # | Item | What it preserves | Cost of keeping | Classification | Removal mechanics |
|---|------|-------------------|-----------------|----------------|-------------------|
| A1 | `AddNeighborRequest.config` legacy carrier — field 1 outside the oneof-shaped pair, kept by ADR-0118 when `NeighborCreateIntent` became the presence-aware create path. | Clients and embedders that still populated the bare field through v0.64. | Dual-carrier validation on every create. | removed (v0.65) | Field 1/name `config` are reserved. Field-2 `intent`, inner config, and mask are required before mutation; v0.64 intent wire bytes remain accepted. The pinned NeighborService message graph was re-blessed without a signature change. |
...
| D2 | Unary `PlanConfigTransaction` / `ApplyConfigTransaction` (`proto/rustbgpd.proto`) alongside streaming Plan/Apply. | **Keep.** Unary Plan/Apply is the permanent, documented small-candidate path. Streaming remains additive and outside the initial v1 frozen inventory; it does not turn unary into a deprecation shim. | Preserve unary byte and behavior compatibility and the CLI's bounded `UNIMPLEMENTED`-only fallback. Any future reversal is a new blessed-surface decision subject to the v1 deprecation floor. |
...
- C1 and A1 completed their blessed-surface re-bless and CHANGELOG migration;
  retained entries continue under their recorded compatibility policy.
