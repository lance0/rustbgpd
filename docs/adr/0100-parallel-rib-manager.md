...
- **Critical unknown:** the manager task's own phase breakdown (ingest/insert vs recompute vs group-staging eval vs group-table commit vs emit vs event publish) was never measured post-arc. Every speedup number below that depends on it is labeled ASSUMED. Slice 0 exists to replace the assumptions.
...
- **Slice 0 — instrumented baseline (measure, no product code).** Re-run rrharness flood/churn/mixed at 256/1000; pprof-bucket the *manager task alone* into: ingest+insert / recompute / staging-eval / table-commit / shared-emit-build / member-emit / event-publish. Publish a phase table beside the scale receipt. **Kill:** parallelizable phases (staging-eval + recompute-classify + emit-build [+ commit if shardable]) < 35% of manager wall → skip (d) entirely, take the (b) decision with these numbers.
