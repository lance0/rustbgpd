...
| `tcp_mss` | restart-required | `TCP_MAXSEG` is installed when sockets are created. Active opens use the neighbor's effective value; every bound passive listener socket carries the smallest effective value across resolved static neighbors, and every accepted child inherits it. SIGHUP keeps the startup values. |
...
| `tcp_mss` | restart-required | Inherited by static group members and included in the passive-listener minimum only after resolution. Groups referenced by dynamic-neighbor ranges cannot set it. |
