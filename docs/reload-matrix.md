...
| `[managed_netdevs]` | restart-required | ADR-0091 bridge, fixed-VNI VXLAN, VRF, and L3VXLAN lifecycle is resolved at startup and reconciled by the dataplane actor (create, stamp, restart adoption, same-owner orphan reap). Managed netdevs are not live-mutable in this tranche; SVD / collect-metadata VXLAN creation remains deferred. |
