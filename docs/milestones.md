...
| **M71** | RFC 9136 §4.3 ESI overlay-index Type 5 single-active recursion, rustbgpd receive side. A GoBGP route source injects a non-zero-ESI RT-5 plus EAD-per-ES / EAD-per-EVI state into a full rustbgpd L3 datapath. Proves the RT-5 is held unresolved before EAD state exists, imports through exactly one single-active remote VTEP when EAD-per-ES carries the Single-Active flag, fails closed on the all-active EAD variant (`unsupported_all_active_esi_overlay_index`), and withdraws cleanly. ADR-0090 reserves the future all-active success path for M72. | `kernel-dataplane` CI (GitHub-hosted). Script: `test-m71-evpn-esi-overlay-type5-receive-gobgp.sh`. |
