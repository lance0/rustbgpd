...
| **M80** | ADR-0096 `.rpol` policy parity vs FRR route-maps — the policy-language arc closer. rustbgpd runs its whole policy surface from an `.rpol` file (parameterized `customer-in(peer_lp)`, a modify-then-continue scrub term, `edge-out` export policy) while an FRR parity node expresses the same intent as route-maps / prefix-lists / community-lists; both receive one route matrix from a source FRR and export to a downstream FRR. Proves route-for-route import parity (prefix-set ge/le boundary, community add + remove, LOCAL_PREF set, as-path regex reject, the same parameterized policy instantiated twice with different LP for two peers) and export parity (MED / communities / deny compared per prefix at the downstream), `rbgp policy check` exit 0 on the file's in-language tests, a `rbgp policy test` live-RIB dry run (counts + per-term hits + diffs), nonzero `rbgp policy stats` live term hits, and an `.rpol` edit under traffic: SIGHUP hot-apply with zero session flap and a wire-observed Route Refresh at exactly the one peer whose resolved chain changed. No dataplane writes. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m80-rpol-policy-parity-frr.sh`. |
