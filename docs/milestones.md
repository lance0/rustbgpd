...
| **M81** | The BMP trio + BMPv4 receipt — the ADR-0097 arc closer. rustbgpd RR reflects unicast v4/v6 + VPNv4 between two GoBGP clients while streaming all three Route Monitoring views (`rib_in_pre` / `rib_out_post` / `loc_rib`) to four collector slots at once: pmacct pmbmpd bleeding-edge + gobmp as independent semantic v3 oracles, plus a raw byte sink with one v3 and one v4 slot under tshark capture. Proves the trio on v3 (rib-in-pre vs rib-out-post O+L flags vs loc-rib for the same route; the RFC 9069 loc-rib instance peer — type 3, VRF table name `global`, PeerDown reason 6; best-path flip + withdraw on the loc-rib stream; collector-reconnect table sync with dump + exactly 4 End-of-RIBs; loc-rib stats types 8/10), BMPv4 at byte level (version byte on every message, single BGP Message TLV type 7 index 0, Stats TLV code 1 wrap byte-matching the v3 stats body, RM PDU sets byte-equal across v3/v4 framings), and Path Marking (every live loc-rib announce carries exactly one TLV with `Best`, a contested winner carries reason 0x0003, dump entries status-only, nothing outside loc-rib). Oracle scope set by a Phase-0 support matrix (script header): no shipped collector decodes tlv-20's final code points, so v4 code points are pinned at raw-byte offsets against the drafts' wire figures. No product bugs found; two pmacct upstream findings recorded. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m81-bmp-trio-gobgp.sh`. |
