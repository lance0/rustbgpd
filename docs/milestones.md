...
| **M84** | Multi-cache RTR/ASPA epoch conformance (LAN-243) — the interop receipt for the 8210bis per-cache `CacheEpoch {version, session_id, serial}` model. rustbgpd runs three RTR caches at once: Routinator 0.15.2 (real RTR v2, offline and deterministic via `--no-rir-tals` + SLURM local assertions), StayRTR v0.6.2 (real RTR v1-only), and the in-repo phased Python RTR v2 server (the only deterministic ASPA source — StayRTR removed ASPA support and Routinator 0.15 silently ignores SLURM `aspaAssertions`). An FRR 10.3.1 peer advertises probe prefixes so each cache's data is observable as a route validation state. 32/32: per-cache full-table load at validated End of Data with exact merged `bgp_rpki_vrp_count` / `bgp_aspa_records`; the shipped v2→v1 fallback against StayRTR's version-0 Error Report; a `docker restart` of Routinator rotating the server session — the client's stale Serial Query draws a Cache Reset and it resynchronizes onto the new session's EoD while a 0.5 s poller proves zero validation flap; incremental epoch advance plus ASPA provider-set replacement (valid → invalid) and empty-provider withdrawal (`aspaState = unknown`) with zero full-table resyncs; and a same-session serial regression rejected at EoD and resolved by exactly one Reset Query resync that purges the stale VRP and leaves the other caches' data intact. Also surfaced a v1-cache reconnect gap (client re-probes v2 after a dropped v1-fallback session and holds data until expiry before re-falling back), recorded in the job header. | Local containerlab (`m84-rtr-multicache.clab.yml`), script `test-m84-rtr-multicache.sh` — local-only like the other RTR-cache jobs (M21/M27/M59). |
