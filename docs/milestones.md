...
| **M74** | RFC 4364 VPNv4 (SAFI 128) route-reflection interop receipt, GoBGP ↔ rustbgpd. A GoBGP source injects two VPNv4 routes with distinct RDs but the same inner prefix (RD-scoped identity), each with an MPLS label and a Route Target; rustbgpd exposes both via `rbgp rib vpn` with RD/label/RT intact, reflects to the sink with label, RD, RT, and next-hop preserved verbatim plus RFC 4456 ORIGINATOR_ID/CLUSTER_LIST, propagates a single-RD withdrawal while the sibling RD survives, and installs nothing into any dataplane. | Hosted CI (GitHub-hosted) in `.github/workflows/interop.yml`. Script: `test-m74-vpnv4-reflection-gobgp.sh`. |
