...
| `StreamPlanConfigTransaction` (stream) | `sensitive_read` | Outside-v1 bounded client-streaming ingress for large plan-only candidates. Requires an authenticated listener and shares one active process-wide slot plus one nonpreemptive operator waiter under the original 30-minute deadline; lower roles and a second waiter fail `RESOURCE_EXHAUSTED`. This is not a general queue. Temporary bytes remain owner-private and descriptor-relative; audit output carries counts/outcome only. |
| `StreamApplyConfigTransaction` (stream) | `operator_only` | Outside-v1 bounded client-streaming commit ingress. Uses the same active slot and single operator wait path, then atomically consumes the exact candidate/runtime binding and hands the unchanged unary apply request to the existing cancellation-shielded executor. The 256-token store evicts lower-role bindings before protected operator bindings; exact consumption remains role-agnostic. |
...
| `DeleteNeighbor` | `mutating` | Single-peer scope. Deleting a neighbor that resolves `md5_password` or a `ttl_security` policy (including `ttl_security_hops`) is rejected `FAILED_PRECONDITION`, matching TCP-AO: the inbound listener key/TTL inventory is updated only by startup or SIGHUP reload. |
...
| `SetPeerGroup` | `operator_only` | Edits propagate to every neighbor inheriting the group — blast radius is N peers, not one. `md5_password` may be set when creating a group, but changing `md5_password`, `ttl_security`, or `ttl_security_hops` on an existing group is rejected `FAILED_PRECONDITION`: the inbound listener key/TTL inventory is updated only by startup or SIGHUP reload. |
