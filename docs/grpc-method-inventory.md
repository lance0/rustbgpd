...
| `RollbackConfigTransaction` | `operator_only` | Junos-style `rollback N`: eligible legacy TOML-only and provenance-verified v2 rows route through the same transaction executor as `ApplyConfigTransaction` (same plan/impact classification and receipts). Unreadable rows and v2 provenance mismatches fail closed before planning or mutation. Same tier as apply because it is an apply. Comment is audit-redacted (presence only). |
