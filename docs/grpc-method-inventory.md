...
| `ApplyEvpnRuntime` | `mutating` | ADR-0063 full-candidate EVPN runtime validation/apply entry point. `validate_only` and no-op applies are bounded; nine shapes converge live and commit a new generation (single L2VNI add/delete/redefine, single IP-VRF add/standalone-delete/redefine with unchanged L3VNI/device/table identity, single Ethernet Segment add/delete/redefine), while other non-noop shapes (ES-member L2VNI redefine / `ip_vrf` relink, linked IP-VRF delete, ES-aware L2VNI delete, L3VNI/device/table IP-VRF redefine, mixed or multi-element edits) fail closed (issue #210). Request TOML can contain credentials and must be audit-redacted. |
