...
| `ListConfigHistory` | `sensitive_read` | Lists the bounded on-disk recorded config history, newest recorded first: per-entry index, timestamp, SHA-256 of normalized TOML only, and a one-line count summary. Index 0 is not a claim about running or persisted state. Never returns config documents, but discloses change cadence and identity facts — full-config-adjacent read. |
