...
| IXP route-server receipt matrix | rustbgpd vs BIRD 3.3.1 vs OpenBGPD 9.1 at 700 clients × 400,400 routes through the same harness on the same host: reload stall + completion (two independent runs), flapstorm withdraw/re-announce, convergence, and process-tree RSS, each incumbent at its documented strongest configuration, losses published alongside wins (OpenBGPD's smaller stall, BIRD's faster flap re-announce and lower RSS) | [`perf/ixp-matrix-2026-07.md`](perf/ixp-matrix-2026-07.md) |
