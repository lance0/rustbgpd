...
| Fanout metrics-handle lifetime | Six alternating real-encoder pairs plus a six-pair same-SHA floor over one homogeneous route-server update group and 64 changed routes: cloning the 153-field metrics handle once per distribution pass instead of once per peer improves the measured actor/probe/commit/enqueue interval by 15.42% at 64 peers, 16.32% at 256, and 15.93% at 1,000; the one-peer negative control is noise | [`perf/fanout-metrics-handle-2026-07.md`](perf/fanout-metrics-handle-2026-07.md), [`artifacts`](perf/artifacts/fanout-metrics-handle-2026-07/README.md) |
| Pristine OTC reconciliation | Six alternating real-encoder pairs plus a six-pair same-SHA floor over one homogeneous route-server update group and 64 changed routes: skipping map construction and `64 * peers` prefix visits when no peer has OTC-blocked state improves the measured actor/probe/commit/enqueue interval by 41.74% at 64 peers, 44.12% at 256, and 42.95% at 1,000. Both binaries carry the same benchmark-local eligibility receipt; non-pristine add/remove semantics retain a separate load-bearing test | [`perf/otc-pristine-reconcile-2026-07.md`](perf/otc-pristine-reconcile-2026-07.md), [`artifacts`](perf/artifacts/otc-pristine-reconcile-2026-07/README.md) |
...
| Cross-stack bgperf2 comparison | Four-way rustbgpd / BIRD 2.18 / GoBGP 4.3.0 / FRR 10.7.0-dev across five fleet shapes, three runs per cell (six at 10p × 1k), one host and one harness. rustbgpd is fastest on total time at all five shapes and converges in 3 s at 100 peers against 5 / 7 / 20 s, the campaign's largest separation. It is **last of the four on memory at 100p × 1k — its own target shape** — 1.10× GoBGP, 1.58× FRR, 6.46× BIRD, with the BIRD ratio running 4.6×–9.6× at every shape. rustbgpd's RSS is the noisiest figure measured (42% run-to-run spread at 30p × 1k, 24% at 100p × 1k) and is published as a range, not a point. 100p × 1k and 2p × 100k carry 100k and 200k routes and both measure 212.0 MiB, so at route-server shapes memory is dominated by per-peer state rather than route count; the computed 1.93 MiB/peer marginal against BIRD's 0.27 is an upper bound and any projection past 100 peers is extrapolated. OpenBGPD is an uncollected cell from a bgperf2 harness defect, not a daemon result | [`perf/competitive-bgperf2-2026-07.md`](perf/competitive-bgperf2-2026-07.md), [`artifacts`](perf/artifacts/competitive-bgperf2-2026-07/README.md) |
