...
| Reload UPDATE-stall experiment (LAN-333; corrected rerun pending) | Historical 700-client × 400k-route run. Its occurrence-based completion gate could close before every unique new-generation prefix arrived, so the `< 1 s` result is invalidated as acceptance evidence. The corrected harness now requires unique prefixes carrying the expected generation marker | [`perf/reload-stall-2026-07.md`](perf/reload-stall-2026-07.md) |
...
| Exact-export fanout optimization (Criterion) | Two pinned campaigns: ordered prepared-attribute memo (18%..32% faster), then bounded wire-equivalent update-group probe reuse (58%..64% faster at 256 peers vs pre-cache; 78%..82% of excess over the permissive control recovered), with commands, confidence intervals, correctness fences, and checked-in CSV | [`perf/exact-export-fanout-2026-07.md`](perf/exact-export-fanout-2026-07.md) |
