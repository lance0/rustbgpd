...
| MRT snapshot allocation control (LAN-572) | Measurement-only two-shape protocol with a production-encoder timing path, separate opt-in allocation/growth diagnostics, and predeclared noise/path/byte acceptance rules plus four-block ABBA rules for any future candidate. The standalone control artifact is pending; no result or automatic acceptance enforcement is claimed yet | [`perf/mrt-snapshot-allocation-2026-07.md`](perf/mrt-snapshot-allocation-2026-07.md), [`artifact schema`](perf/artifacts/mrt-snapshot-allocation-2026-07/README.md) |
