...
| `config-history/*.toml` | Last 20 distinct normalized TOML snapshots for `rbgp config history` / `rollback N`; newest recorded is index 0; owner-private and secret-bearing. External `.rpol` and dataset contents are not included or hashed, and rollback re-reads their current filesystem contents. | Yes |
