...
| `max_prefix_action`    | string   | no       | `"shutdown"` | What a crossed max-prefix bound does. `"shutdown"`: Cease/1 teardown and latch (plus the optional `max_prefix_restart_seconds` timed restart). `"block"`: net-new prefixes beyond a full bound are withheld (never installed, counted in `bgp_max_prefix_blocked_total`) while the session stays Established; a prefix already accepted still takes attribute changes and new Add-Path identities; when usage falls back under the bound the episode ends and one route refresh asks the peer to replay what was withheld. `"warning"`: warn once per crossing at the bound and keep accepting. `block` applies to the per-family unicast bounds and requires the aggregate `max_prefixes` to be unset; `block` and `warning` exclude `max_prefix_restart_seconds`. Inherits from the peer group (ADR-0108 amendment) |
