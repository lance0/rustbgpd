...
| `max_prefixes_received_ipv4` | u32 | no      | --      | Maximum unique IPv4-unicast prefixes the peer may announce, counted **before** import policy: every prefix the peer currently announces counts once whether policy accepts or rejects it, Add-Path identities share one slot, and an explicit withdrawal or an enhanced-route-refresh sweep releases it. Same latched Cease/1 teardown, RFC 4486 data, Notification GR encapsulation, and `max_prefix_restart_seconds` contract as `max_prefixes_ipv4`, enforced independently of it. Rejected identities are tracked only while this bound is set; enabling it on an Established session requests a route refresh so the peer's existing rejections are recounted (ADR-0108 amendment) |
| `max_prefixes_received_ipv6` | u32 | no      | --      | IPv6-unicast sibling of `max_prefixes_received_ipv4` |
