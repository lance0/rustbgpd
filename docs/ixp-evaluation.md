...
| 6 | Looking glass incl. filtered routes | Yes | Alice-LG through the birdwatcher adapter retains status, peer, accepted, filtered, and noexport views. The sidecar also serves IXP Manager's status, live BGP inventory/detail, symbols, member received, and member export seam through direct startup aliases or an atomically reloadable file-backed resolver, with an enforced route maximum. Global snapshots/counts and other wildcard-community views remain unavailable, so this is not full Bird's Eye compatibility. | [birdwatcher adapter](../examples/birdwatcher-adapter/README.md) · [Alice-LG wiring](cookbook/ixp-filter-pipeline.md#6-looking-glass-alice-lg-via-the-birdwatcher-adapter) |
