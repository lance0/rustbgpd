...
| FRR (bgpd) | 10.7.1 | `tests/interop/m18-extnexthop-frr.clab.yml` | Tested (M18) | Extended Next-Hop (RFC 8950) | Dual-stack, IPv6 NH for IPv4, and RFC 8950 route suppression to non-ENHE peers | — |
...
| Export non-transitive EC strip | PASS | Non-transitive EC stripped on plain eBGP export; transitive preserved |
...
| RFC 8950 non-ENHE suppression | PASS | IPv4 route with IPv6 next-hop withheld from non-ENHE peer, restored when capability enabled |
...
| Administrative reset Cease/4 | PASS | ResetNeighbor sends Cease/4 with RFC 9003 text; FRR records reason and recovers after backoff |
| **Total** | **10/10** | |
