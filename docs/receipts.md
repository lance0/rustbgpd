...
| Dual-stack route-server reload (in progress) | Harness extension for one-session IPv4 + IPv6 unicast members with independent per-family completion, a filtering policy shape with named-withdrawal, leak, and bystander accounting, and wire- and bitmap-level empty-family, missing-prefix, and duplicate-prefix negative cases. The 20-member correctness rung passed (20/20 sessions, both families exact, zero decode errors, all operator probes exit 0); the pinned 200- and 700-member rungs (400,400 total = 200,200 per family) are not yet run | [`perf/ixp-dualstack-2026-09.md`](perf/ixp-dualstack-2026-09.md), [`pinned shape and raw cells`](perf/artifacts/ixp-dualstack-2026-09/README.md) |
