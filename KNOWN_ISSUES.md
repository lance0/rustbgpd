...
- **`match_rpki_validation` and `match_aspa_validation` only work in export
  policy.** Import policy evaluates in the transport layer before the route
  reaches the RIB, where RPKI and ASPA validation states are assigned. At
  import evaluation time, both states are always their defaults (`NotFound`
  for RPKI, `Unknown` for ASPA). Use best-path demotion and export policy
  for validation-based filtering instead.
