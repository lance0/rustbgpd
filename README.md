...
| Interop suites | Automated interop suite (see [docs/INTEROP.md](docs/INTEROP.md) for the full matrix), primarily against FRR 10.3.1 plus GoBGP 3.37.0 / 4.6.0 / 4.7.0 across labs and GoBGP 4.8.0 in M83, with StayRTR-backed RTR coverage; BIRD 2.0.12 covers M0 plus the remaining route-server, route-reflector, and differential labs, BIRD 2.19.2 covers M83, and BIRD 3.3.1 covers the TCP-AO smoke. A foundation tier is gated on every PR, privileged Linux dataplane smokes run in hosted kernel-dataplane CI, and longer soaks / platform-diversity scripts remain local. |
