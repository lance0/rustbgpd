...
- Full production EVPN **VTEP** deployments — rustbgpd now ships the
  ...
  multi-homing alpha support. Gate 9 IRB readiness foundation is
  live (`[[evpn_ip_vrfs]]` schema, IP-VRF readiness probe + Linux
  netlink dumps + `Dataplane::probe_ip_vrfs` end-to-end through the
  reconciler, ADR-0058), but Type 5 origination and L3 FIB
  programming are not yet shipped. Aliasing dataplane ECMP and
  production-default multi-homing enforcement are also still ahead
