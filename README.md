...
| Interop suites | Automated interop suite (see [docs/INTEROP.md](docs/INTEROP.md) for the full matrix), primarily against FRR 10.3.1 plus GoBGP 3.37.0 / 4.6.0 / 4.7.0 across established labs and GoBGP 4.8.0 in M83/M103/M104, with StayRTR-backed RTR coverage; BIRD 2.0.12 anchors M0 and immutable legacy/differential labs, BIRD 2.19.2 covers M83/M85/M93/M95/M104, BIRD 3.3.1 covers the TCP-AO smoke, and BIRD 3.3.2 covers M101. A foundation tier is gated on every PR, privileged Linux dataplane smokes run in hosted kernel-dataplane CI, and longer soaks / platform-diversity scripts remain local. |
