...
- Full production EVPN **VTEP** deployments — rustbgpd now ships the
  ...
  multi-homing alpha support. Gate 9 IRB readiness foundation
  is live (`[[evpn_ip_vrfs]]` schema, IP-VRF readiness probe + Linux
  netlink dumps + `Dataplane::probe_ip_vrfs` end-to-end through the
  reconciler, ADR-0058); slice 6 PR A also ships per-IP-VRF kernel
  route observation + local Type 5 origination gated on readiness.
  Remote Type 5 import, L3 FIB programming, and the M39
  containerlab smoke are still ahead (slice 6 PR B). Aliasing
...
| **Not yet supported** | Kernel FIB integration (unicast), EVPN duplicate-MAC quarantine action / Type 5 origination + L3 FIB programming (Gate 9 readiness foundation is live; dataplane is next) / aliasing dataplane ECMP / production-default multi-homing enforcement, VPNv4/v6, Confederation, TCP-AO |
...
| [docs/adr/](docs/adr/) | Architecture decision records (58 ADRs) |
