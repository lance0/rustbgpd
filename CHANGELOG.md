...
- **EventService session lifecycle events.** `EventService.WatchEvents`
  ...
  Ordinary non-collision FSM state-change lifecycle delivery uses a bounded
  transport channel that is separate from the lossless TCP collision-coordination
  path, so session-event observability cannot grow the collision queue under
  churn.
