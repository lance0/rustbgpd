...
/// IPv4 listener clamp, and likewise for IPv6. This prevents a
/// reduced IPv4 tunnel MTU from down-clamping IPv6 sessions below the MSS
/// implied by the IPv6 minimum MTU. Returns `(min_v4, min_v6)`, each `None`
/// when no neighbor of that family sets `tcp_mss`.
...
async fn run<T>(
    ...
    let listener_options =
        ListenerSocketOptions {
            ...
            // IPv4 listeners, and likewise for IPv6. This prevents a
            // reduced IPv4 tunnel MTU from down-clamping IPv6 sessions below the
            // MSS implied by the IPv6 minimum MTU.
    ...
    let blackhole_handle = blackhole::spawn(
        blackhole::BlackholeConfig {
            ...
            max_active: config.global.blackhole_discard_max_active,
            install_rate_per_minute: config.global.blackhole_discard_install_rate_per_minute,
    ...
    for neighbor in peer_configs {
        ...
        let mut peer_config = PeerManagerNeighborConfig {
            ...
            max_prefixes_received_ipv4: transport_config.max_prefixes_received_ipv4,
            max_prefixes_received_ipv6: transport_config.max_prefixes_received_ipv6,
            max_prefix_action: transport_config.max_prefix_action,
            max_prefix_warning_percent: transport_config.max_prefix_warning_percent,
            ...
            send_non_transitive_extended_communities: transport_config
            ...
            discard_path_attributes: transport_config.discard_path_attributes.clone(),
...
"#,
    ...
    /// When only an IPv4 neighbor has `tcp_mss = 536`, the IPv4 listener gets
    /// 536 and the IPv6 listener is unclamped.
    #[test]
    ...
    /// When both families have neighbors with different `tcp_mss` values,
    /// each listener gets its own family's minimum — an IPv4 tunnel
    ...
    /// When no neighbor sets `tcp_mss`, both listeners are unclamped.
    #[test]
