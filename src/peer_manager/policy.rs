...
use rustbgpd_rib::{
    ExportPolicyCohortOutcome, PeerExportPolicyReplacement, PeerExportPolicyRestoreReceipt,
    RibCommandError, RibUpdate,
};
use rustbgpd_telemetry::BgpMetrics;
use rustbgpd_transport::StateQueryOutcome;
...
/// the peer.
pub(super) struct PolicyApplyFailure {
    code: RuntimeConfigPolicyFailureCode,
    message: String,
...
#[derive(Default)]
pub(super) struct LazyRibBudget(Option<tokio::time::Instant>);
...
struct PolicySnapshotContext {
    ...
    /// Whether this transaction admits the bounded operator-read lane at
    /// read-only probes, RIB waits, and acknowledged policy-step boundaries.
    /// Decided at entry, so rollback inherits its forward apply's admission. Reads
...
struct RegisteredPolicyRollbackRib {
    ...
    task: tokio::task::JoinHandle<
        Result<Vec<PeerExportPolicyRestoreReceipt>, RuntimeConfigPolicyFailureCode>,
    >,
}

struct PolicyRollbackFailure {
    code: RuntimeConfigPolicyFailureCode,
    detail: String,
}

#[derive(Default)]
struct CleanStateQueryWindow {
    retry_deadline: Option<tokio::time::Instant>,
}

impl CleanStateQueryWindow {
    fn remaining_retry_budget(&mut self) -> Option<std::time::Duration> {
        let deadline = *self
            .retry_deadline
            .get_or_insert_with(|| tokio::time::Instant::now() + CLEAN_STATE_QUERY_WINDOW);
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        (!remaining.is_zero()).then_some(remaining)
    }
}

impl std::fmt::Display for PolicyRollbackFailure {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
impl PeerManager {
    ...
    async fn qualify_rfc8212_import_transition(
        ...
        let state = match state {
            ...
            StateQueryOutcome::SessionGone => {
                let retained = self
                    .query_peer_retained_stale(peer_key.address, rib_budget, operator_reads)
                    .await?;
                if retained == 0 {
                    return Ok(false);
                }
    ...
    /// servicing readiness and the owning transaction's admitted reads throughout.
    ///
    ...
    pub(super) async fn query_peer_retained_stale(
        &mut self,
        peer: IpAddr,
        rib_budget: Option<&mut LazyRibBudget>,
        operator_reads: OperatorReadAdmission,
        ...
        self.await_with_readiness(
            tokio::time::timeout_at(deadline, round_trip),
            operator_reads,
        )
        .await
        .map_err(|_| timed_out())?
    ...
    async fn preflight_rfc8212_import_transitions(
        ...
        for peer_key in &transitioning {
            if let Err(error) = self
                .qualify_rfc8212_import_transition(peer_key, Some(&mut *rib_budget), operator_reads)
                .await
            {
    ...
    pub(super) async fn update_runtime_policies_fatal(
        ...
        self.update_runtime_policies_for_peer_key(
            ...
            OperatorReadAdmission::Fenced {
                reason: "inline hot updates also restore a rejected generation's session configuration",
            },
        )
        .await
    ...
    #[cfg(test)]
    pub(super) async fn apply_resolved_policy_snapshot_classified(
        &mut self,
        targets: Vec<ResolvedPeerPolicy>,
    ...
    /// Forward policy and API compensation owners admit reads from preflight on.
    /// Generation compensation decides from earlier restoration results.
    /// Admission travels on the context through probes, acknowledged step
    /// boundaries, and RIB waits, including rollback. Unspecified owners use
    /// the fenced test wrapper above.
    ...
    async fn apply_resolved_policy_snapshot_classified_inner(
        ...
        let preflight = self
            .preflight_rfc8212_import_transitions(
                &targets,
                &mut context.forward,
        ...
        let cohort_mask = self
            .export_only_policy_cohort_mask(&targets, context.operator_reads)
            .await;
        phases.cohort_selection_us = elapsed_us(cohort_selection_started);
            ...
            Err(remainder_error) => {
                    ...
                    || rollback.is_err()
                {
                    let code = rollback
                        .as_ref()
                        .err()
                        .map_or(remainder_error.code, |failure| failure.code);
                    Err(PolicySnapshotFailure::ambiguous_code(code, message))
                } else {
                    Err(PolicySnapshotFailure::compensated_code(
                        remainder_error.code,
                        message,
                    ))
                }
            }
    ...
    async fn complete_policy_snapshot(
        ...
        if convergence_debt {
                ...
                .await
            {
                ...
                Err(error) => Err(PolicySnapshotFailure::ambiguous_code(
                    error.code,
                    format!(
        ...
        for target in targets {
                ...
                .await
            {
                    ...
                    .await
                {
                    ...
                    Err(restore_error) => Err(PolicySnapshotFailure::ambiguous_code(
                        restore_error.code,
                        format!(
                            "failed to apply resolved policy to {}: {apply_error}; \
    ...
    /// Query a post-delivery session state without treating one short actor
    /// miss as session loss. Every peer gets the ordinary 100 ms probe; only a
    /// typed timeout retries, at most once, and every retry shares one lazy
    /// absolute two-second window for the complete clean-convergence cohort.
    async fn query_clean_session_state(
        &mut self,
        commands: tokio::sync::mpsc::Sender<rustbgpd_transport::PeerCommand>,
        ...
        let Some(remaining) = window.remaining_retry_budget() else {
            ...
            targets.iter().zip(&peer_keys).zip(&import_deltas)
        {
            ...
            if import_changed {
                ...
                if let Err(error) = import_result {
                    ...
                    return Some(Err(match prior_restore {
                        ...
                        Err(restore_error) => PolicySnapshotFailure::ambiguous_code(
                            restore_error.code,
                            format!(
            ...
            if let Err(error) = apply_result {
                ...
                return Some(Err(match restore_error {
                    ...
                    Some(restore_error) => PolicySnapshotFailure::ambiguous_code(
                        prior_restore_code
                            .unwrap_or(RuntimeConfigPolicyFailureCode::RollbackSessionRejected),
                        format!(
        ...
        for (index, (target, peer_key)) in targets.iter().zip(&peer_keys).enumerate() {
            ...
            let state = if require_clean_convergence {
                self.query_clean_session_state(
                    commands,
                    &mut context.clean_state_window,
                    context.operator_reads,
                )
                .await
            } else {
    ...
    pub(super) async fn replace_peer_export_policy_in_rib(
        &mut self,
        peer: IpAddr,
        export_policy: Option<PolicyChain>,
        rib_budget: Option<&mut LazyRibBudget>,
    ...
    async fn register_policy_rollback_rib(
        ...
        // supply the original forward order and require unique RIB identities
        // before launching any detached work.
        let replacements = plans
            ...
            .filter_map(|plan| {
                plan.export_restore
                    .as_ref()
                    .map(|(export_policy, _)| PeerExportPolicyReplacement {
        ...
        let task = tokio::spawn(async move {
            ...
            if rib_tx
                ...
                .is_err()
            {
                return Err(RuntimeConfigPolicyFailureCode::RollbackBatchUnavailable);
            }
            let _ = enqueued_tx.send(());
            match reply_rx.await {
                Err(_) => Err(RuntimeConfigPolicyFailureCode::RollbackBatchReplyLost),
                Ok(Err(_)) => Err(RuntimeConfigPolicyFailureCode::RollbackBatchRejected),
        ...
        .await
        {
            ...
            Err(_) => {
                ...
                // task still owns both the queued send and its late receiver,
                // so exact repair can finish while pending flags stay armed.
                drop(task);
    ...
    async fn finish_policy_rollback_refreshes(
        ...
        for plan in plans {
            let refreshable = plan.export_restore.is_none() || batch_enqueued;
            if refreshable {
    ...
    async fn restore_resolved_policies(
        ...
        for prior in priors.into_iter().rev() {
            ...
            if !self.peers.contains_key(&peer_key) {
                if require_exact_pending {
                    failure_code.get_or_insert(RuntimeConfigPolicyFailureCode::RollbackPeerGone);
                    errors.push(format!(
                ...
                .await
            {
                Err(error) => {
                    failure_code
                        .get_or_insert(RuntimeConfigPolicyFailureCode::RollbackSessionRejected);
        ...
        let registered = match self.register_policy_rollback_rib(&plans, context).await {
            ...
            Err(code) => {
                ...
                errors.push(code.as_str().to_string());
        ...
        self.finish_policy_rollback_refreshes(&plans, registered.is_some())
            .await;
        ...
        if let Some(registered) = registered {
            ...
            .await
            {
                Err(_) => {
                    ...
                    let code = RuntimeConfigPolicyFailureCode::RollbackBatchTimeout;
                    failure_code.get_or_insert(code);
                    errors.push(code.as_str().to_string());
                }
                Ok(Err(_)) => {
                    ...
                    let code = RuntimeConfigPolicyFailureCode::RollbackBatchReplyLost;
                    failure_code.get_or_insert(code);
                    errors.push(code.as_str().to_string());
                }
                Ok(Ok(Err(code))) => {
                ...
                Ok(Ok(Ok(receipts))) => {
                    let export_plans = plans
                        .iter()
                        .filter(|plan| plan.export_restore.is_some())
                        .collect::<Vec<_>>();
                    if receipts.len() == export_plans.len() {
                        for (plan, receipt) in export_plans.into_iter().zip(receipts) {
                            if receipt.peer() != plan.peer_key.address {
                                let code = RuntimeConfigPolicyFailureCode::RollbackBatchReplyLost;
                                failure_code.get_or_insert(code);
                                errors.push(code.as_str().to_string());
                                self.retain_policy_rollback_rib_intent(plan);
                            ...
                            match receipt {
                                    ...
                                    ) =>
                                {
                                    info!(
                                        peer = %plan.peer_key.address,
                                        "policy rollback acknowledged absent RIB registration; restored session policy is authoritative on PeerUp"
                                    );
                                    self.finish_policy_rollback_rib_success(plan);
                                }
                                PeerExportPolicyRestoreReceipt::NotFound { .. } => {
        ...
        for (peer_key, prior_refresh, prior_export) in pending_priors {
            ...
            {
                failure_code.get_or_insert(RuntimeConfigPolicyFailureCode::RollbackPendingDebt);
                errors.push(format!(
        ...
        if errors.is_empty() {
            ...
            Err(PolicyRollbackFailure {
                code: failure_code.unwrap_or(RuntimeConfigPolicyFailureCode::RollbackBatchRejected),
                detail: errors.join("; "),
            })
        }
    }
    ...
    pub(super) async fn prepare_dataset_dependents(
        ...
        for dependent in &dependents {
            self.qualify_dataset_dependent(
                dependent,
                &mut window,
                &mut rib_budget,
                OperatorReadAdmission::Served,
    ...
    async fn qualify_dataset_dependent(
            ...
            .await
        {
            ...
            StateQueryOutcome::State(_) => {
                ...
                let retained = self
                    .query_peer_retained_stale(peer.address, Some(&mut *rib_budget), operator_reads)
                    .await
    ...
    pub(super) async fn reevaluate_dataset_exports(
        &mut self,
        peers: Vec<IpAddr>,
        known_down: bool,
        operator_reads: OperatorReadAdmission,
        wall_reply_budget: bool,
    ) -> Result<(), DatasetRefreshFailure> {
        if peers.is_empty() {
            return Ok(());
        }
        let rib_tx = self.rib_tx.clone();
        let capacity_deadline = tokio::time::Instant::now() + RIB_REPLY_TIMEOUT;
        let permit = match self
            .await_with_readiness(
                tokio::time::timeout_at(capacity_deadline, rib_tx.reserve()),
                operator_reads,
            )
            .await
        {
        ...
        // Legacy refresh charged wall time; generation settlement excludes
        // read servicing from its acknowledgement budget. Keep both contracts.
        let response = async {
            if wall_reply_budget {
                tokio::time::timeout(RIB_REPLY_TIMEOUT, response).await.ok()
            } else {
                Some(response.await)
            }
        };
        match self
            .await_with_readiness_budget(response, RIB_REPLY_TIMEOUT, operator_reads)
            .await
            .flatten()
        {
            Some(Ok(Ok(()))) => Ok(()),
    ...
    #[expect(
        clippy::too_many_lines,
        reason = "dataset settlement keeps qualification, export replay, import refresh, and cleanup in one ordered operation"
    )]
    pub(super) async fn refresh_dataset_generation_dependents(
        ...
        for (dependent, is_established) in dependents.iter().zip(&established) {
            if dependent.export {
                if *is_established {
                    ...
                    self.reevaluate_dataset_exports(
                        vec![dependent.peer.address],
                        true,
                        operator_reads,
                        false,
                    )
        ...
        self.reevaluate_dataset_exports(exports, false, operator_reads, false)
            .await?;
        for (dependent, is_established) in dependents.iter().zip(&established) {
            if dependent.import
                && *is_established
        ...
        for (dependent, was_established) in dependents.iter().zip(&established) {
            let now_established = self
                .qualify_dataset_dependent(dependent, &mut window, &mut rib_budget, operator_reads)
                .await
                .map_err(DatasetRefreshFailure::Ambiguous)?;
    ...
    async fn apply_runtime_policies_for_peer_key(
        ...
        let rfc8212_refresh_required = if rfc8212_fatal_transition {
            self.qualify_rfc8212_import_transition(
                &peer_key,
                rib_budget.as_deref_mut(),
                operator_reads,
            )
            .await?
        } else {
        ...
        self.drain_operator_queries(state_reads).await;
        let require_clean_state = clean_state_window.is_some()
            && matches!(refresh_failure, RefreshFailureHandling::Fatal);
        let session_state = if let Some(window) = clean_state_window {
            self.query_clean_session_state(commands, window, state_reads)
                .await
        } else {
            self.await_with_readiness(
                rustbgpd_transport::PeerHandle::query_state_outcome_with(
        ...
        let is_known_non_established = matches!(
            session_state,
            StateQueryOutcome::State(ref state)
                if state.fsm_state != SessionState::Established
        );
        ...
        if require_clean_state && (needs_refresh || needs_export_apply) && !is_established {
            ...
            let code = match session_state {
                StateQueryOutcome::State(_) => RuntimeConfigPolicyFailureCode::StateNonEstablished,
                StateQueryOutcome::SessionGone => RuntimeConfigPolicyFailureCode::StateSessionGone,
                StateQueryOutcome::TimedOut => RuntimeConfigPolicyFailureCode::StateRetryExhausted,
            };
            return Err(PolicyApplyFailure {
                code,
        ...
        if needs_export_apply && (is_established || is_rollback) {
            ...
            match rib_outcome {
                ...
                Err(error) => {
                    ...
                    return Err(PolicyApplyFailure {
                        code: RuntimeConfigPolicyFailureCode::RibApplyRejected,
                        message: error.to_string(),
                        refresh_delivery_began: false,
                    });
                }
        ...
        if needs_refresh && is_established {
                ...
                .await
            {
                ...
                match refresh_failure {
                    RefreshFailureHandling::Fatal => {
                        ...
                        return Err(PolicyApplyFailure {
                            code: RuntimeConfigPolicyFailureCode::RouteRefreshRejected,
                            message: error.to_string(),
            ...
            // a typed state query timed out or lost its session task for an
            // actually-Established peer and the ordinary best-effort path does
            // not require a clean-state proof. Re-arm `pending_refresh`
            // unconditionally so that a subsequent call — once the session unblocks
