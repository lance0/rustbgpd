...
fn lossless_path_bytes(path: &Path) -> (&'static [u8], Vec<u8>) {
    ...
    (b"unix-bytes-hex", path.as_os_str().as_bytes().to_vec())
}
...
"#,
    ...
    fn digest_canonical_framing_is_stable() {
        ...
        assert_eq!(
            ...
            [
                0xdd, 0x86, 0x36, 0x8e, 0x9e, 0x1e, 0x69, 0x41, 0xe3, 0xdd, 0xa4, 0xc9, 0x6a, 0xc4,
                0x79, 0x08, 0x26, 0x3f, 0xc4, 0x6f, 0x67, 0xf2, 0x09, 0xcc, 0x95, 0xe5, 0xad, 0xb8,
                0xb5, 0x41, 0x86, 0x65,
            ]
        );
    }
