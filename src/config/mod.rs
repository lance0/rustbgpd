...
pub(crate) use resolution::reserved_rfc8212_deny_chain;
#[allow(
    unused_imports,
    reason = "preserve the public crate::config API for library consumers; the binary mirror cannot observe external use"
)]
pub use resolution::{
    EffectivePolicyChains, ResolvedNeighbor, UnpolicedEbgpNeighbor, is_reserved_rfc8212_deny,
...
pub struct ConfigDiff {
    ...
    /// BFD profile definitions changed. Profiles remain startup-owned even
    /// when no running member currently references the changed profile.
    pub bfd_changed: bool,
    /// Member attachments need the SIGHUP BFD coordinator; v1 transactions
    /// cannot atomically settle the BFD actor with their peer mutations.
    #[serde(skip)]
    pub(crate) bfd_members_changed: bool,
...
pub fn classify_config_transaction_v1(diff: &ConfigDiff) -> ConfigTransactionSectionClassification {
    ...
    if diff.bfd_members_changed {
        class
            .unsupported_sections
            .push("neighbor / peer-group BFD (apply via SIGHUP reload)".to_string());
    }
    if diff.bfd_changed {
        class
            .restart_required_sections
            .push("[[bfd_profiles]]".to_string());
    }
...
pub fn format_config_diff_with_style(diff: &ConfigDiff, style: &ConfigDiffTextStyle<'_>) -> String {
    ...
    if diff.bfd_changed {
        restart_sections.push("[[bfd_profiles]]");
    }
    if diff.policy_explain_changed {
...
pub fn diff_config(old: &Config, new: &Config) -> ConfigDiff {
    ...
    // Profiles stay startup-owned; member attachments remain reload-applied.
...
fn classify_evpn_runtime_change(old: &Config, new: &Config) -> EvpnRuntimeChangeClass {
    ...
    // ADR-0085 binding-only edits remain reload-applied when the typed domain
    // plan is a no-op so SIGHUP republishes the binding snapshot. Every
    // supported primitive plan is classified by the shared domain policy.
...
pub(crate) fn bfd_member_attachments_changed(old: &Config, new: &Config) -> bool {
    let groups = |config: &Config| -> BTreeMap<_, _> {
        config
            .peer_groups
            .iter()
            .filter_map(|(name, group)| group.bfd.clone().map(|bfd| (name.clone(), bfd)))
            .collect()
    };
    let members = |config: &Config| -> BTreeMap<_, _> {
        config
            .neighbors
            .iter()
            .filter_map(|neighbor| {
                let bfd = neighbor.bfd.as_ref().or_else(|| {
                    config
                        .peer_groups
                        .get(neighbor.peer_group.as_deref()?)?
                        .bfd
                ...
                Some((
                    ...
                    (neighbor.bfd.clone(), bfd.clone()),
                ))
            })
            .collect()
    };
    groups(old) != groups(new) || members(old) != members(new)
}
