...
#[test]
/// The current comparator selector is a closed pair. It changes only the two
/// version-bearing config comments, while provenance identity is rechecked
/// without requiring Docker in this executable fixture.
fn irr_reload_competitor_generation_is_closed_and_byte_stable() {
    use std::collections::BTreeSet;
    ...
    let generate = |cell: &str, generation: Option<&str>, name: &str| {
        let output_dir = temp.path().join(name);
        let mut command = std::process::Command::new("python3");
        command
            .args([&generator, cell, "10", "100"])
            .arg(&output_dir)
            .args(["--min-list", "100", "--max-list", "100"]);
        if let Some(generation) = generation {
            command.args(["--competitor-generation", generation]);
        }
        let output = command.output().expect("run IRR competitor generator");
        assert!(
            output.status.success(),
            "{cell}/{generation:?} generator failed\nstdout:\n{}\nstderr:\n{}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
        output_dir
    ...
    for (cell, main_config, historical_headers, current_headers) in [
        (
            "bird",
            "bird.conf",
            [
                "# IRR reload-matrix BIRD 3.3.1 route server - generated by gen-irr-scenario.py.",
                "# Run: docker run -d --network=host -v <out_dir>:/etc/bird bird:3.3.1 \\",
            ],
            [
                "# IRR reload-matrix BIRD 3.3.2 route server - generated by gen-irr-scenario.py.",
        ...
        (
            ...
            "bgpd.conf",
            [
                "# IRR reload-matrix OpenBGPD 9.1 route server - generated by gen-irr-scenario.py.",
                "# Run: docker run -d --network=host -v <out_dir>:/etc/bgpd openbgpd/openbgpd:9.1",
            ],
            [
                "# IRR reload-matrix OpenBGPD 9.2 route server - generated by gen-irr-scenario.py.",
        ...
        let default = generate(cell, None, &format!("{cell}-default"));
        let historical = generate(cell, Some("historical"), &format!("{cell}-historical"));
        let current = generate(cell, Some("current"), &format!("{cell}-current"));
        let roster = |directory: &std::path::Path| {
            std::fs::read_dir(directory)
                .expect("read generated directory")
                .map(|entry| entry.expect("directory entry").file_name())
                .collect::<BTreeSet<_>>()
        };
        assert_eq!(roster(&default), roster(&historical));
        assert_eq!(roster(&historical), roster(&current));
        for entry in std::fs::read_dir(&historical).expect("historical roster") {
            let filename = entry.expect("historical entry").file_name();
            assert_eq!(
                std::fs::read(default.join(&filename)).expect("default bytes"),
                std::fs::read(historical.join(&filename)).expect("historical bytes"),
                "default selector drifted for {}",
                filename.to_string_lossy()
            );
            if filename != main_config && filename != "manifest.json" {
                assert_eq!(
                    std::fs::read(historical.join(&filename)).expect("historical bytes"),
                    std::fs::read(current.join(&filename)).expect("current bytes"),
                    "operational bytes drifted for {}",
                    filename.to_string_lossy()
                );
            }
        }
        let normalized_manifest = |directory: &std::path::Path| {
        ...
        assert_eq!(
            normalized_manifest(&historical),
            normalized_manifest(&current)
        );
        let historical_lines = std::fs::read_to_string(historical.join(main_config))
            .expect("historical config")
            .lines()
            .map(str::to_owned)
            .collect::<Vec<_>>();
        let current_lines = std::fs::read_to_string(current.join(main_config))
            .expect("current config")
            .lines()
            .map(str::to_owned)
            .collect::<Vec<_>>();
        assert_eq!(&historical_lines[..2], &historical_headers);
        assert_eq!(&current_lines[..2], &current_headers);
        assert_eq!(historical_lines[2..], current_lines[2..]);
    }

    let rejected = std::process::Command::new("python3")
        .args([&generator, "bird", "10", "100"])
        .arg(temp.path().join("bad-generation"))
        .args(["--competitor-generation", "arbitrary"])
        .output()
        .expect("run rejected generation");
    assert!(!rejected.status.success());

    let id_a = format!("sha256:{}", "a".repeat(64));
    let id_b = format!("sha256:{}", "b".repeat(64));
    let identity = |generation: &str, live_bird: &str, stored_bird: &str| {
        std::process::Command::new("bash")
            .arg(&runner)
            .args(["--self-test-competitor-identity", stored_bird, &id_b])
            .env("COMPETITOR_GENERATION", generation)
            .env("IRRRELOAD_SELF_TEST_BIRD_ID", live_bird)
            .env("IRRRELOAD_SELF_TEST_OPENBGPD_ID", &id_b)
            .output()
            .expect("run image identity fixture")
    };
    assert!(identity("historical", &id_a, &id_a).status.success());
    assert!(identity("current", &id_a, &id_a).status.success());
    assert!(!identity("current", &id_b, &id_a).status.success());
    assert!(!identity("arbitrary", &id_a, &id_a).status.success());

    let dry_current = std::process::Command::new("bash")
        .arg(&runner)
        .env("DRY_RUN_PROTOCOL", "1")
        .env("COMPETITOR_GENERATION", "current")
        .output()
        .expect("run current dry protocol");
    assert!(dry_current.status.success());
    let dry_current = String::from_utf8(dry_current.stdout).expect("dry protocol UTF-8");
    assert!(dry_current.contains("competitor_generation=current\n"));
    assert!(dry_current.contains("bird:v3.3.2-m101"));
    assert!(dry_current.contains(
...
fn irr_reload_counterbalanced_receipt_protocol_is_load_bearing() {
    ...
    for proof in [
        ...
        "competitor-ref-arbitrary",
        "competitor-ref-drift",
        "competitor-mixed-generation",
        "repeat-competitor-generation",
        "repeat-image-identity",
        "cross-role-competitor-generation",
        "nonoverlap-order",
        "reused-identity",
        "quiet-spacing",
        "preflight-raw",
