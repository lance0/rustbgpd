...
/// only.
fn emit_config(path: &Path, rundir: &Path) {
...
/// Wait for a gRPC round trip and return the BGP endpoint the daemon
/// reported binding.
async fn wait_until_serving(rbgp: &Rbgp, sock: &Path, daemon: &mut Daemon) -> SocketAddr {
    ...
    while Instant::now() < deadline {
        ...
        if let Some(bgp) = support::bound_bgp_addr(&daemon.log())
            && sock.exists()
...
async fn atomic_aggregate_and_aggregator_survive_adj_rib_in_to_export() {
    ...
    emit_config(&config, &rundir.path);
    ...
    let bgp = wait_until_serving(&rbgp, &sock, &mut daemon).await;
