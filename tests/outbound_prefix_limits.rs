...
/// The daemon picks its own BGP and metrics ports (`0`) and the test reads
/// them back from its log. One pinned IPv4 listen address makes any BGP bind
/// failure fatal at startup instead of leaving a daemon that serves `[::]`
/// only. Every generation carries the same listener settings.
fn emit_config(path: &Path, rundir: &Path, v4: u32, v6: u32) {
...
/// Wait for a gRPC round trip and return the BGP and metrics endpoints the
/// daemon reported binding.
async fn wait_until_serving(
    ...
    while Instant::now() < deadline {
        ...
        let log = daemon.log();
        if let (Some(bgp), Some(metrics)) = (
            support::bound_bgp_addr(&log),
            support::bound_metrics_addr(&log),
        ) && sock.exists()
            && rbgp.run(&["global"]).await.is_ok()
        {
            return (bgp, metrics);
        }
...
async fn outbound_prefix_limits_bound_the_wire_and_survive_reload_edits() {
    ...
    // for the full table.
    emit_config(&config_live, &rundir.path, 8, 4);
    emit_config(&config_lower, &rundir.path, 2, 1);
    emit_config(&config_raise, &rundir.path, 12, 6);
    ...
    let (bgp, metrics) = wait_until_serving(&rbgp, &sock, &mut daemon).await;
