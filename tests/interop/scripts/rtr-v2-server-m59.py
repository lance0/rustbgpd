...
# ROAs make both test prefixes RPKI-Valid so origin validation never
# interferes with the ASPA verdict under test.
ROAS = [
    # (flags=announce, prefix_len, max_len, ip_bytes, asn)
    (1, 24, 24, socket.inet_aton("192.168.20.0"), 65010),  # downstream-Valid origin
    (1, 24, 24, socket.inet_aton("192.168.21.0"), 65008),  # downstream-Invalid origin
]

# ASPA records: (flags=announce, customer_asn, [provider_asns]).
# Chosen so role-aware DOWNSTREAM (customer-cone) verification yields a clean
# Valid and a clean Invalid for the AS_PATHs FRR (AS 65003) advertises. No path
# contains 65001 (rustbgpd's own ASN) — that would trip eBGP loop detection.
#   Valid   [65003, 65011, 65010]: 65010→65011 and 65011→65003 form an
#           unbroken up-ramp covering the whole path ⇒ Valid.
#   Invalid [65003, 65007, 65008]: 65008 authorizes only 65099 (the
#           65008→65007 up-hop is NotProviderPlus) and 65003 authorizes only
#           65099 (the 65003→65007 down-hop is NotProviderPlus), so neither the
#           up- nor the down-ramp can cover the path ⇒ Invalid.
ASPAS = [
    (1, 65010, [65011]),
    (1, 65011, [65003]),
    (1, 65003, [65099]),
