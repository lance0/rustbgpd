...
# ---------------------------------------------------------------------------
# Helpers (M66 lineage; drain-gauge helpers are new)
# ---------------------------------------------------------------------------
...
# series reads as 0 (the gauge is only instantiated on the first
# transition of that reason — ADR-0085 decision 2 metrics contract).
drain_gauge() {
    local container=${1:?}
    local reason=${2:?}
    local value
...
# Poll (1s grain) until the drain gauge for a reason reads $3.
wait_drain_gauge() {
...
# Millisecond wall clock for the hold-off / blackout bookkeeping.
now_ms() {
    date +%s%3N
}

# pe1's bound AC up/down (the failure injection — see header for why
# pe1's own end of the veth is the injection point).
pe1_ac_down() {
...
# address — the GARP pattern real CEs emit; M66-established). This is
# the recovered DF's legitimate AC re-learn stimulus: with the
# ADR-0085 same-ESI bias + the VXLAN-port move observation landed,
# pe1 originates no Type 2 for the CE MAC after recovery until its
# kernel relearns it on the AC.
...
wait_pe_grpc() {
    ...
    fail "$label gRPC not reachable within 60s"
...
wait_vtep_established() {
    ...
    fail "$label session did not reach Established within 120s"
    return 1
}
...
start_prober() {
    local tag=${1:?}
    docker exec "$HR" sh -c "rm -f /tmp/m67-prober-${tag}.log /tmp/m67-prober-${tag}.pid" || true
    docker exec -d "$HR" sh -c "ping -i 0.1 -O $CE_HOST_IP >/tmp/m67-prober-${tag}.log 2>&1 & echo \$! >/tmp/m67-prober-${tag}.pid"
}

stop_prober() {
    local tag=${1:?}
    docker exec "$HR" sh -c "kill -TERM \"\$(cat /tmp/m67-prober-${tag}.pid)\" 2>/dev/null" || true
    sleep 1
}
...
prober_max_gap_ms() {
    local tag=${1:?}
    local max_gap
    max_gap=$(docker exec "$HR" cat "/tmp/m67-prober-${tag}.log" 2>/dev/null \
        | grep 'bytes from' \
